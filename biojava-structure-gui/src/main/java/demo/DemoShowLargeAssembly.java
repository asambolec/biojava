...
public class DemoShowLargeAssembly {
	...
	private static final Logger logger = LoggerFactory.getLogger(DemoShowLargeAssembly.class);

	public static void main(String[] args) {

		// This loads the PBCV-1 virus capsid, one of, if not the biggest biological
		// assembly in terms on nr. of atoms.
		// The 1m4x.pdb1.gz file has 313 MB (compressed)
		// This Structure requires a minimum of 9 GB of memory to be able to be loaded
		// in memory.
		...
		logger.info(new StringBuilder().append("Memory consumption after ").append(pdbId)
				.append(" structure has been loaded into memory:").toString());

		String mem = String.format("Total %dMB, Used %dMB, Free %dMB, Max %dMB", r.totalMemory() / 1048576,
				(r.totalMemory() - r.freeMemory()) / 1048576, r.freeMemory() / 1048576, r.maxMemory() / 1048576);

		logger.info(mem);

		// 9693 atoms in the asymmetric unit * 1680 copies per assembly = 16284240 atoms
		logger.info("# atoms: " + StructureTools.getNrAtoms(bigStructure));
	...
	public static Structure readStructure(String pdbId, int bioAssemblyId) {
		...
		// note, this requires several GB of memory for some of the largest assemblies,
		// such a 1MX4
