...
/**
 * A JPanel that can display the underlying distance matrix data of the protein
 * structure alignment algorithm. It adds a JSlider to a JMatrixPanel.
 *
...
public class ScaleableMatrixPanel extends JPanel implements ChangeListener, ActionListener {
	...
	private static final Logger logger = LoggerFactory.getLogger(ScaleableMatrixPanel.class);
	...
	protected static Map<String, ContinuousColorMapper> createGradients() {
		 ...
		 * gradients.put((++i)+". Default", defaultMapper); defaultMapper = new
		 * DefaultMatrixMapper(5., .9f); gradients.put((++i)+". Sensitive",
		 * defaultMapper);
		 * 
		 * 
		 * gradients.put((++i)+". Rainbow",
		 * GradientMapper.getGradientMapper(GradientMapper.RAINBOW_INTENSITY_GRADIENT,
		 * 0, 10)); gradients.put((++i)+". Rainbow",
		 * GradientMapper.getGradientMapper(GradientMapper.RAINBOW_INTENSITY_GRADIENT,
		 * 10, 0));
		 */
	...
	/**
	 * A renderer for the the gradient dropdown menu at the top of
	 * scaleableMatrixPanes. Knows how to draw a gradient and nicely label it.
	 *
