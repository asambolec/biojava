...
public class DisplayAFP {
	...
	/**
	 * Return a list of pdb Strings corresponding to the aligned positions of the
	 * molecule. Only supports a pairwise alignment with the AFPChain DS.
	 *
	 * @param aligPos
	 * @param afpChain
	 * @param ca
	...
	public static final List<String> getPDBresnum(int aligPos, AFPChain afpChain, Atom[] ca) {
		...
		if (aligPos > 1) {
			logger.error("multiple alignments not supported yet!");
			return lst;
		}
	...
	/**
	 * Return the atom at alignment position aligPos. at the present only works with
	 * block 0
	 * 
	 * @param chainNr     the number of the aligned pair. 0... first chain, 1...
	 *                    second chain.
	 * @param afpChain    an afpChain object
	 * @param aligPos     position on the alignment
	 * @param getPrevious gives the previous position if false, gives the next
	 *                    posible atom
	 * @return a CA atom that is at a particular position of the alignment
	 */
	public static final Atom getAtomForAligPos(AFPChain afpChain, int chainNr, int aligPos, Atom[] ca,
			boolean getPrevious) throws StructureException {
		int[] optLen = afpChain.getOptLen();
		// int[][][] optAln = afpChain.getOptAln();
		...
		if (chainNr < 0 || chainNr > 1) {
			throw new StructureException(
					"So far only pairwise alignments are supported, but you requested results for alinged chain nr "
							+ chainNr);
		}
		...
		if (capos < 0) {
			...
			// System.out.println(" got next" + capos + " for " + chainNr + " alignedPos: "
			// + aligPos);
		} else {
			// System.out.println("got aligned fatcat position: " + capos + " " + chainNr +
			// " for alig pos: " + aligPos);
		}

		if (capos < 0) {
			logger.error(new StringBuilder().append("could not match position ").append(aligPos).append(" in chain ")
					.append(chainNr).append(". Returing null...").toString());
			return null;
		}
		if (capos <= ca.length) {
			return ca[capos];
		...
		logger.error(new StringBuilder().append("Atom array ").append(chainNr).append(" does not have ").append(capos)
				.append(" atoms. Returning null.").toString());
		return null;
		...
		for (int i = 0; i < blockNum; i++) {
			for (int j = 0; j < optLen[i]; j++) {
				...
				if (len > 0) {
					...
					// System.out.println(" pos "+ len+" p1-p2: " + p1 + " - " + p2 + " lmax: " +
					// lmax + " p1b-p2b:"+p1b + " " + p2b + " terminate? "+ terminateNextMatch);
		...
		for (int i = 0; i < blockNum; i++) {
			for (int j = 0; j < optLen[i]; j++) {
				...
				if (len > 0) {
					...
					// System.out.println(" p1-p2: " + p1 + " - " + p2 + " lmax: " + lmax + "
					// p1b-p2b:"+p1b + " " + p2b);
					for (int k = 0; k < lmax; k++) {
	...
	public static final Atom[] getAtomArray(Atom[] ca, List<Group> hetatms) throws StructureException {
		...
		// we only add atom nr 1, since the getAlignedStructure method actually adds the
		// parent group, and not the atoms...
		for (Group g : hetatms) {
			if (g.size() < 1) {
				continue;
			}
	...
	/**
	 * Note: ca2, hetatoms2 and nucleotides2 should not be rotated. This will be
	 * done here...
	 */

	public static final StructureAlignmentJmol display(AFPChain afpChain, Group[] twistedGroups, Atom[] ca1, Atom[] ca2,
			List<Group> hetatms1, List<Group> hetatms2) throws StructureException {
		...
		String title = new StringBuilder().append(afpChain.getAlgorithmName()).append(" V.")
				.append(afpChain.getVersion()).append(" : ").append(afpChain.getName1()).append(" vs. ")
				.append(afpChain.getName2()).toString();
		...
		// jmol.setTitle("Structure Alignment: " + afpChain.getName1() + " vs. " +
		// afpChain.getName2());
		jmol.setTitle(title);
		return jmol;
	}
	...
	public static void showAlignmentPanel(AFPChain afpChain, Atom[] ca1, Atom[] ca2, AbstractAlignmentJmol jmol)
		...
		frame.setTitle(new StringBuilder().append(afpChain.getName1()).append(" vs. ").append(afpChain.getName2())
				.append(" | ").append(afpChain.getAlgorithmName()).append(" V. ").append(afpChain.getVersion())
				.toString());
		me.setPreferredSize(
				new Dimension(me.getCoordManager().getPreferredWidth(), me.getCoordManager().getPreferredHeight()));
	...
	/**
	 * Create a "fake" Structure objects that contains the two sets of atoms aligned
