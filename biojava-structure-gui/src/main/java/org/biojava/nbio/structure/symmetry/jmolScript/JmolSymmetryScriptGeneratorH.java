...
public class JmolSymmetryScriptGeneratorH extends JmolSymmetryScriptGenerator {
	...
	private static double sideChainExtension = 6.0;
	...
	@Override
	public int getZoom() {
		int zoom = (int) Math.round(100.0 / axisScaleFactor);
		return zoom;
	}
	...
	/**
	 * Returns a Jmol script that sets a specific orientation and zoom to draw
	 * either axes or polyhedron
	 * 
	...
	public String drawPolyhedron() {
		...
		// helixAxisAligner.getHelixLayers().getByLargestContactsNotLowestAngle().getLayerLines();
		...
		// helixAxisTransformation.getHelixLayers().getByLowestAngle().getTransformation();
		//
	...
	public String drawAxes() {
		...
		v1.scale(axisScaleFactor * (helixAxisAligner.getDimension().y + sideChainExtension));
	...
	/**
	 * Returns a Jmol script that colors the subunits of a structure by different
	 * colors
	 * 
	 * @return
	 */
	...
	/**
	 * Returns a Jmol script that colors subunits to highlight the symmetry within a
	 * structure Different subunits should have a consistent color scheme or
	 * different shade of the same colors
	 * 
	 * @return Jmol script
	 */
	@Override
	public String colorBySymmetry() {
		...
		for (int i = 0; i < maxLen; i++) {
			for (List<Integer> unit1 : units) {
				int m = unit1.size();
				if (i < m) {
					int subunit = unit1.get(i);
	...
	private List<List<Integer>> orientLayerLines(List<List<Integer>> layerLines) {
		...
		layerLines.forEach(layerLine -> {
			// get center of first subunit in layerline and transform to standard
			// orientation (helix axis aligned with y-axis)
			int first = layerLine.get(0);
			Point3d firstSubunit = new Point3d(centers.get(first));
			transformation.transform(firstSubunit);

			// get center of last subunit in layerline and transform to standard orientation
			// (helix axis aligned with y-axis)
			...
			// a layerline should start at the lowest y-value, so all layerlines have a
			// consistent direction from -y value to +y value
	...
	private List<Point3d> extendUnitCenters(List<Point3d> points) {
		...
		radius += sideChainExtension; // add space for side chains
	...
	private List<Point3d> interiorCenters(List<Point3d> points) {
		...
		List<Point3d> ePoints = new ArrayList<>(points.size());
		points.stream().map(Point3d::new).forEach(q -> {
			transformation.transform(q);
			q.x = 0;
			q.z = 0;
			reversetransformation.transform(q);
