...
/**
 * A sequence display that can show the results of a protein structure
 * alignment.
 *
...
public class SequenceDisplay extends JPanel implements ChangeListener {
	...
	// LoggerFactory.getLogger(SequenceDisplay.class);
	...
	public SequenceDisplay(StructurePairAligner structurePairAligner) {
		...
		int RES_MAX = 100;
		int RES_INIT = 100;
		residueSizeSlider = new JSlider(JSlider.HORIZONTAL, RES_MIN, RES_MAX, RES_INIT);
	...
	public static void main(String[] args) {
		...
		try {
			...
			logger.info(new StringBuilder().append("aligning ").append(pdb1).append(" vs. ").append(pdb2).toString());
			...
			JFrame frame = new JFrame(new StringBuilder().append("Sequences for AlternativeAlignment [").append(0)
					.append("]").toString());
	...
	/**
	 * calculate the float that is used for display. 1 * scale = size of 1 amino
	 * acid (in pixel). maximum @see MAX_SCALE
	 * 
	 * @param zoomFactor
	 * @return a float that is the display "scale" - an internal value required for
	 *         paintin. user should only interact with the zoomfactor ...
	 */
	...
	/**
	 * a value of 100 means that the whole sequence should be displayed in the
	 * current visible window a factor of 1 means that one amino acid shoud be drawn
	 * as big as possible
	 *
