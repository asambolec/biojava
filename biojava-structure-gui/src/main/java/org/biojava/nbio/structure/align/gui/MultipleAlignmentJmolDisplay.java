 ...
 * <p>
 * Currently supported: Alignment Panel Display, select aligned residues in Jmol
 * by their PDB name, show a text Frame for any sequence alignment format, basic
 * Jmol display from a MultipleAlignment, generate an artificial PDB structure
 * with a new model for every aligned structure.
 *
 * @author Aleix Lafita
 * @since 4.2.0
 *
...
public class MultipleAlignmentJmolDisplay {
	...
	/**
	 * Utility method used in the {@link MultipleAlignmentJmol} Frame, when the
	 * aligned residues of a structure in the alignment have to be selected for
	 * formatting them (coloring and style).
	 *
	 ...
	 * @param ca        the atom array of the structure specified (corresponding to
	 *                  the structure index)
	 * @return List of pdb Strings corresponding to the aligned positions of the
	 *         structure.
	 */
	...
	/**
	 * Creates a new Frame with the MultipleAlignment Sequence Panel. The panel can
	 * communicate with the Jmol 3D visualization by selecting the aligned residues
	 * of every structure.
	 *
	 * @param multAln
	 * @param jmol
	 ...
	 */
	public static void showMultipleAligmentPanel(MultipleAlignment multAln, AbstractAlignmentJmol jmol)
			throws StructureException {
	...
	public static void showAlignmentImage(MultipleAlignment multAln, String result) {
		...
		String title = new StringBuilder().append(multAln.getEnsemble().getAlgorithmName()).append(" V.")
				.append(multAln.getEnsemble().getVersion()).toString();
	...
	/**
	 * Display a MultipleAlignment with a JmolPanel. New structures are downloaded
	 * if they were not cached in the alignment and they are entirely transformed
	 * here with the superposition information in the Multiple Alignment.
	 *
	 * @param multAln
