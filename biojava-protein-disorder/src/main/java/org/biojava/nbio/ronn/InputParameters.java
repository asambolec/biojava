...
final class InputParameters {
	...
	ResultLayout parseFormat(String format) {
		...
		throw new IllegalArgumentException(new StringBuilder().append("Unrecognised format: '").append(format)
				.append("' Output format for results must be either H or V.").toString());
	}
	...
	public String toString() {
		...
		String value = new StringBuilder().append("input=").append(input).append(newLine).toString();

		if (output != null) {
			value += new StringBuilder().append("output=").append(output).append(newLine).toString();
		} else {
			value += "output= standard out" + newLine;
		}
		if (statistics != null) {
			value += new StringBuilder().append("statistics=").append(statistics).append(newLine).toString();
		} else {
		...
		value += newLine;
		value += new StringBuilder().append("thread number=").append(getThreadNum()).append(newLine).toString();
	...
	void setDisorder(final String prm) {
		final float disorder = Float.parseFloat(prm.substring(InputParameters.disorderKey.length()));
	...
	void setThreadNum(final String prm) {
		...
		if ((tnum < 1) || (tnum > procNum * 2)) {
			throw new IllegalArgumentException("Number of threads must be between 1 "
