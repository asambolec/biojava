...
public final class ORonnModel {
	 ...
	 */
	private final float disorderWeight;
	...
	public ORonnModel(final String sequence, final Model model, final float disorder) {
		this.disorderWeight = disorder;
		this.model = model;
	...
	final float[] detect() {
		...
		/*
		 * 19 looks like a size of the sliding window. So for any sequences shorted than
		 * 19 AA the score will be NaN. Original RONN segfault in such condition
		 */
		for (sResidue = 0; sResidue <= query.length - ORonnModel.AA_ALPHABET; sResidue++) {
			...
			fOrder = (float) (Math.exp(-0.5 * Math.pow(est - thold.mu0, 2.0) / thold.sigma0)
					/ (Math.sqrt(6.28) * thold.sigma0));

			fDisor = (float) (Math.exp(-0.5 * Math.pow(est - thold.mu1, 2.0) / thold.sigma1)
					/ (Math.sqrt(6.28) * thold.sigma1));

			pDisor = (float) (disorderWeight * fDisor / ((1.0 - disorderWeight) * fOrder + disorderWeight * fDisor));
