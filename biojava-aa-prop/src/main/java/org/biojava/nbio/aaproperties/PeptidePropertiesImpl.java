...
/**
 * This class contains the actual implementation of IPeptideProperties and is
 * wrapped around by PeptideProperties for ease of use.
 *
 * @author kohchuanhock
 * @version 2011.08.22
 * @since 3.0.2
...
public class PeptidePropertiesImpl implements IPeptideProperties {
	...
	/*
	 * Pseudo code obtained from email correspondance with ExPASy Helpdesk, Gregoire
	 * Rossier
	 */
	//
	// Table of pk values :
	...
	// Ct Nt Sm Sc Sn
	//
	private final double[][] cPk = { { 3.55, 7.59, 0.0 }, // A
			{ 3.55, 7.50, 0.0 }, // B
			{ 3.55, 7.50, 9.00 }, // C
			// {4.55, 7.50, 4.05}, // D
			// {4.75, 7.70, 4.45}, // E
	...
	public double getMolecularWeight(ProteinSequence sequence, File aminoAcidCompositionFile)
		...
		return getMolecularWeightBasedOnXML(sequence,
				obtainAminoAcidCompositionTable(elementMassFile, aminoAcidCompositionFile));
	}
	...
	public double getExtinctionCoefficient(ProteinSequence sequence, boolean assumeCysReduced) {
		...
		// Trp => W
		// Cys => C
		// E(Prot) = Numb(Tyr)*Ext(Tyr) + Numb(Trp)*Ext(Trp) +
	...
	public double getInstabilityIndex(ProteinSequence sequence) {
		...
		logger.warn(
				"Valid length of sequence is 0, can't divide by 0 to calculate instability index: setting instability index value to 0.0");
		return 0.0;
	}
	...
	public double getAvgHydropathy(ProteinSequence sequence) {
		...
		logger.warn(
				"Valid length of sequence is 0, can't divide by 0 to calculate average hydropathy: setting average hydropathy to 0");
		return 0.0;
	}
	...
	private double getIsoelectricPointInnovagen(ProteinSequence sequence) {
		...
		while (true) {
			...
			if (margin <= difference && margin >= -difference) {
				break;
			}
			changeSize /= 2.0;
			if (margin > 0) {
	...
	private double getNetChargeInnovagen(ProteinSequence sequence, double pHPoint) {
		...
		return getNetChargeInnovagen(chargedAA2Count, pHPoint, sequenceString.charAt(0),
				sequenceString.charAt(sequenceString.length() - 1));
	}

	private double getNetChargeInnovagen(Map<AminoAcidCompound, Integer> chargedAA2Count, double ph, char nTerminalChar,
			char cTerminalChar) {
		// Constraints.aa2PKa is aleady reinitialized in getChargedAACount hence no need
		// to do it again
		...
		double kCharge = chargedAA2Count.get(aaSet.getCompoundForString("K"))
				* this.getPosCharge(Constraints.aa2PKa.get(aaSet.getCompoundForString("K")), ph);
		double rCharge = chargedAA2Count.get(aaSet.getCompoundForString("R"))
				* this.getPosCharge(Constraints.aa2PKa.get(aaSet.getCompoundForString("R")), ph);
		double hCharge = chargedAA2Count.get(aaSet.getCompoundForString("H"))
				* this.getPosCharge(Constraints.aa2PKa.get(aaSet.getCompoundForString("H")), ph);
		double dCharge = chargedAA2Count.get(aaSet.getCompoundForString("D"))
				* this.getNegCharge(Constraints.aa2PKa.get(aaSet.getCompoundForString("D")), ph);
		double eCharge = chargedAA2Count.get(aaSet.getCompoundForString("E"))
				* this.getNegCharge(Constraints.aa2PKa.get(aaSet.getCompoundForString("E")), ph);
		double cCharge = chargedAA2Count.get(aaSet.getCompoundForString("C"))
				* this.getNegCharge(Constraints.aa2PKa.get(aaSet.getCompoundForString("C")), ph);
		double yCharge = chargedAA2Count.get(aaSet.getCompoundForString("Y"))
				* this.getNegCharge(Constraints.aa2PKa.get(aaSet.getCompoundForString("Y")), ph);
		// if((kCharge + rCharge + hCharge) == 0.0 && (dCharge + eCharge + cCharge +
		// yCharge) == 0.0){
		// return 0.0;
		// }
		return (nTerminalCharge + kCharge + rCharge + hCharge)
				- (dCharge + eCharge + cCharge + yCharge + cTerminalCharge);
	}
	...
	public Map<AminoAcidCompound, Double> getAAComposition(ProteinSequence sequence) {
		...
		AminoAcidCompoundSet aaSet = new AminoAcidCompoundSet();
		aaSet.getAllCompounds().forEach(aa -> aa2Composition.put(aa, 0.0));
		char[] seq = this.getSequence(sequence.toString(), true);
		for (char aa : seq) {
