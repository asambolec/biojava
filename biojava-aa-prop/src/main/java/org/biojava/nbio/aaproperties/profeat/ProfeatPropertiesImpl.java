...
public class ProfeatPropertiesImpl implements IProfeatProperties {
	...
	public Map<GROUPING, Double> getComposition(ProteinSequence sequence, ATTRIBUTE attribute) throws Exception {
		...
		for (GROUPING group : GROUPING.values()) {
			grouping2Composition.put(group, getComposition(sequence, attribute, group));
		}
	...
	public Map<ATTRIBUTE, Map<GROUPING, Double>> getComposition(ProteinSequence sequence) throws Exception {
		...
		for (ATTRIBUTE attribute : ATTRIBUTE.values()) {
			attribute2Grouping2Composition.put(attribute, getComposition(sequence, attribute));
		}
	...
	public Map<TRANSITION, Double> getTransition(ProteinSequence sequence, ATTRIBUTE attribute) throws Exception {
		...
		for (TRANSITION transition : TRANSITION.values()) {
			transition2Double.put(transition, getTransition(sequence, attribute, transition));
		}
	...
	public Map<ATTRIBUTE, Map<TRANSITION, Double>> getTransition(ProteinSequence sequence) throws Exception {
		...
		for (ATTRIBUTE attribute : ATTRIBUTE.values()) {
			attribute2Transition2Double.put(attribute, getTransition(sequence, attribute));
		}
	...
	public Map<ATTRIBUTE, Map<GROUPING, Map<DISTRIBUTION, Double>>> getDistributionPosition(ProteinSequence sequence)
			throws Exception {
		Map<ATTRIBUTE, Map<GROUPING, Map<DISTRIBUTION, Double>>> attribute2Grouping2Distribution2Double = new HashMap<>();
		for (ATTRIBUTE attribute : ATTRIBUTE.values()) {
