 ...
 * {@link MultipleSequenceAlignment} or other indirect distance infomation
 * (RMSD).
 *
 * @author Aleix Lafita
 * @since 4.1.1
...
public class DistanceMatrixCalculator {
	...
	/**
	 * The fractional dissimilarity (D) is defined as the percentage of sites that
	 * differ between two aligned sequences. The percentage of identity (PID) is the
	 * fraction of identical sites between two aligned sequences.
	 *
	 * <pre>
	 * D = 1 - PID
	 * </pre>
	 ...
	 * dissimilarity (D) specially needed for large evolutionary distances. It is
	 * given by:
	 *
	 * <pre>
	 ...
	 * </pre>
	 *
	 * The equation is derived by fitting the relationship between the evolutionary
	 * distance (d) and the fractional dissimilarity (D) according to the PAM model
	 * of evolution (it is an empirical approximation for the method
	 * {@link #pamDistance(MultipleSequenceAlignment}). The gapped positons in the
	 * alignment are ignored in the calculation. This method is a wrapper to the
	 * forester implementation of the calculation:
	...
	/**
	 * BioJava implementation for percentage of identity (PID). Although the name of
	 * the method is percentage of identity, the DistanceMatrix contains the
	 * fractional dissimilarity (D), computed as D = 1 - PID.
	 * <p>
	 ...
	 * </pre>
	 *
	 * Where the sum through i runs for all the alignment positions, ai and bi are
	 * the AA at position i in the first and second aligned sequences, respectively,
	 * and L is the total length of the alignment (normalization).
	 * <p>
	 * The fractional dissimilarity score (Ds) is in the interval [0, 1], where 0
	 * means that the sequences are identical and 1 that the sequences are
	...
	/**
	 * The dissimilarity score is the additive inverse of the similarity score (sum
	 * of scores) between two aligned sequences using a substitution model
	 ...
	 * </pre>
	 *
	 * Where the sum is for all 20 AA, fi denotes the natural fraction of the given
	 * AA and M is the substitution matrix (in this case the PAM1 matrix).
	 * <p>
	 * To calculate the PAM distance between two aligned sequences the maximum
	 * likelihood (ML) approach is used, which consists in finding d that maximazies
	 * the function:
	 *
	 * <pre>
	 ...
	 * </pre>
	 *
	 * Where the product is for every position i in the alignment, and ai and bi are
	 * the AA at position i in the first and second aligned sequences, respectively.
	 *
	 * @param msa MultipleSequenceAlignment
	 * @return
	...
	public static <C extends Sequence<D>, D extends Compound> DistanceMatrix pamMLdistance(
		...
		// SubstitutionMatrix<AminoAcidCompound> PAM1 =
	...
	/**
	 * The structural distance (d<sub>S</sub>) uses the structural similarity (or
	 * dissimilarity) from a the structural alignment of two protein strutures. It
	 * is based on the diffusive model for protein fold evolution (Grishin 1995).
	 ...
	 * </pre>
	 *
	 * @param rmsdMat RMSD matrix for all structure pairs (symmetric matrix)
	 * @param alpha   change in CA positions introduced by a single AA substitution
	 *                (Grishin 1995: 1 A)
	 * @param rmsdMax estimated RMSD between proteins of the same fold when the
	 *                percentage of identity is infinitely low (the maximum allowed
	 *                RMSD of proteins with the same fold). (Grishin 1995: 5 A)
	 * @param rmsd0   arithmetical mean of squares of the RMSD for identical
	 *                proteins (Grishin 1995: 0.4 A)
	 * @return DistanceMatrix
	 */
	public static <C extends Sequence<D>, D extends Compound> DistanceMatrix structuralDistance(double[][] rmsdMat,
			double alpha, double rmsdMax, double rmsd0) {
		...
		for (int i = 0; i < n; i++) {
			for (int j = i; j < n; j++) {
				if (i == j) {
					...
					double d = (rmsdMax * rmsdMax) / (alpha * alpha) * Math.log(
							(rmsdMax * rmsdMax - rmsd0 * rmsd0) / (rmsdMax * rmsdMax - rmsdMat[i][j] * rmsdMat[i][j]));
					d = Math.max(d, 0.0);
					DM.setValue(i, j, d);
					DM.setValue(j, i, d);
				}
