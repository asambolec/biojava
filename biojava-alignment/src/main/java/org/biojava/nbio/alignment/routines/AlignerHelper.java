...
/**
 * Static utility to construct alignment routines from a common library of
 * methods.
 *
 * @author Mark Chapman
...
public class AlignerHelper {
	...
	/**
	 * Calculate the optimal alignment score for the given sequence positions with
	 * an affine or constant gap penalty
	 * 
	...
	/**
	 * Calculates the optimal alignment score for the given sequence positions and a
	 * linear gap penalty
	 * 
	 * @param x      position in query
	 * @param y      position in target
	 ...
	 * @param sub    compound match score
	 * @param scores dynamic programming score matrix to fill at the given position
	 * @return traceback directions for substitution, deletion and insertion
	 *         respectively
	 */
	...
	public static Last[][] setScoreVector(int x, Subproblem subproblem, int gop, int gep, int[] subs, boolean storing,
			int[][][] scores) {
		return setScoreVector(x, subproblem.getQueryStartIndex(), subproblem.getTargetStartIndex(),
				subproblem.getTargetEndIndex(), gop, gep, subs, storing, scores, subproblem.isStartAnchored());
	}
	 ...
	 * @return
	 */
	public static Last[][] setScoreVector(int x, int xb, int yb, int ye, int gop, int gep, int[] subs, boolean storing,
	...
	/**
	 * Score global alignment for a given position in the query sequence for a
	 * linear gap penalty
	 * 
	 * @param x
	 * @param subproblem
	...
	public static Last[][] setScoreVector(int x, Subproblem subproblem, int gep, int[] subs, boolean storing,
			int[][][] scores) {
		return setScoreVector(x, subproblem.getQueryStartIndex(), subproblem.getTargetStartIndex(),
				subproblem.getTargetEndIndex(), gep, subs, storing, scores, subproblem.isStartAnchored());
	}
	 ...
	 * @return
	 */
	public static Last[][] setScoreVector(int x, int gop, int gep, int[] subs, boolean storing, int[][][] scores,
			int[] xyMax, int score) {
	...
	/**
	 * Score local alignment for a given position in the query sequence for a linear
	 * gap penalty
	 * 
	 * @param x
	 * @param gep
	...
	public static int[] setSteps(Last[][][] traceback, int[][][] scores, List<Step> sx, List<Step> sy) {
		...
		Last last =
				...
				linear ? traceback[xMax][yMax][0] :

						(scores[xMax][yMax][1] > scores[xMax][yMax][0] && scores[xMax][yMax][1] > scores[xMax][yMax][2])
								?

								Last.DELETION
								: (scores[xMax][yMax][0] > scores[xMax][yMax][2]) ? Last.SUBSTITUTION : Last.INSERTION;
	...
	public static String tracebackToString(Last[][][] traceback) {
		...
		for (int z = 0; z < 3; z++) {
			for (Last[][] aTraceback : traceback) {
				if (aTraceback != null) {
					for (int j = 0; j < aTraceback.length; j++) {
						if (aTraceback[j] == null || z >= aTraceback[j].length || aTraceback[j][z] == null) {
							sb.append('.');
						} else {
							switch (aTraceback[j][z]) {
							case DELETION:
								sb.append('^');
								break;
							case SUBSTITUTION:
	...
	// LoggerFactory.getLogger(AlignerHelper.class);

	// types

	/**
	 * Define a traceback pointer for the three edit operations: substitution
	 * (match/replacement of a query compound with a target compound), deletion
	 * (removal of a query compound leaving a gap in the target sequence), and
	 * insertion (addition of a target compound opening a gap in the query
	 * sequence).
	 */
	public enum Last {
	...
	/**
	 * Defines a 'cut' row for divide-and-conquer alignment in which a new anchor is
	 * found.
	 */
	...
	/**
	 * Alignment subproblem. The bounds of the subproblem are the indicies
	 * representing the inclusive bounds of the dynamic programming alignment
	 * problem.
	 * 
	 * @author Daniel Cameron
	 */
	public static class Subproblem {
		...
		/**
		 * Indicates whether the start query and start target index compounds are
		 ...
		 */
		public static List<Subproblem> getSubproblems(List<Anchor> anchors, int querySequenceLength,
			...
			for (AlignerHelper.Anchor anchor : anchors) {
				if (anchor.targetIndex <= last.targetIndex || anchor.queryIndex <= last.queryIndex) {
					throw new IllegalArgumentException("Anchor set must allow at least one possible alignment.");
				}
				list.add(new Subproblem(last.queryIndex + 1, last.targetIndex + 1, anchor.queryIndex,
						anchor.targetIndex, isAnchored));
				last = anchor;
				isAnchored = true;
			}
			list.add(new Subproblem(last.queryIndex + 1, last.targetIndex + 1, querySequenceLength,
					targetSequenceLength, isAnchored));
			return list;
		}
	}
}
