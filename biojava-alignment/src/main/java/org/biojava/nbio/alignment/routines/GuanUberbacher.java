...
/**
 * Guan and Uberbacher defined an algorithm for pairwise global sequence
 ...
 * efficiently by dynamic programming with a space requirement reduced from
 * quadratic (a multiple of query sequence length times target sequence length)
 * to only linear (a multiple of query sequence length). The counterpoint to
 * this reduction in space complexity is a modest (a multiple < 2) increase in
 * time.
 *
 * @author Mark Chapman
...
public class GuanUberbacher<S extends Sequence<C>, C extends Compound> extends AnchoredPairwiseSequenceAligner<S, C> {
	 ...
	 * 
	 * @param defaultCutsPerSection the default number of cuts added to each section
	 *                              during each pass
	 */
