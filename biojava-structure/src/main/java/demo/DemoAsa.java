...
public class DemoAsa {
	...
	private static void demoAsa(String pdbCode, int numThreads) throws IOException, StructureException {
		...
		AsaCalculator asaCalc = new AsaCalculator(structure, AsaCalculator.DEFAULT_PROBE_SIZE, 1000, numThreads,
				hetAtoms);
		...
		for (GroupAsa groupAsa : groupAsas) {
			logger.info("%1s\t%5s\t%3s\t%6.2f\n", groupAsa.getGroup().getChainId(),
					groupAsa.getGroup().getResidueNumber(), groupAsa.getGroup().getPDBName(), groupAsa.getAsaU());
			tot += groupAsa.getAsaU();
		}

		logger.info("Total area: %9.2f\n", tot);
		logger.info("Time: %4.1fs\n", ((end - start) / 1000.0));
		...
		for (int nThreads = 1; nThreads <= numThreads; nThreads++) {
			...
			asaCalc = new AsaCalculator(structure, AsaCalculator.DEFAULT_PROBE_SIZE, 1000, numThreads, hetAtoms);
		...
		for (int nThreads = 1; nThreads <= numThreads; nThreads++) {
			logger.info(nThreads + " threads, time: %4.1fs -- x%2.1f\n", runTimes[nThreads - 1],
					runTimes[0] / runTimes[nThreads - 1]);
		}
