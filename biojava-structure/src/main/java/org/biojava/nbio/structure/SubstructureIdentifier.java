 ...
 *
 * <p>
 * The current syntax allows the specification of a set of residues from the
 * first model of a structure. Future versions may be extended to represent
 * additional properties.
 *
 * <p>
 * Identifiers should adhere to the following specification, although some
 * additional forms may be tolerated where unambiguous for backwards
 * compatibility.
 * 
 * <pre>
 * 		name          := pdbID
...
public class SubstructureIdentifier implements StructureIdentifier {
	...
	public SubstructureIdentifier(String id) {
		...
		if (idRange[0].length() != 4) {
			...
			// Changed from Exception to a warning to support files and stuff -sbliven
			// 2015/01/22
	 ...
	 *
	 * This provides the canonical form for a StructureIdentifier and has all the
	 * information needed to recreate a particular substructure.
	 *
	...
	public String getIdentifier() {
		...
		return new StringBuilder().append(pdbId).append(".").append(ResidueRange.toString(ranges)).toString();
	}
	...
	/**
	 * Takes a complete structure as input and reduces it to residues present in the
	 * specified ranges
	 *
	 * <p>
	 ...
	 * 
	 * <p>
	 * Ligands are handled in a special way. If a full chain is selected (e.g.
	 * '1ABC.A') then any waters and ligands with matching chain name are included.
	 * If a residue range is present ('1ABC.A:1-100') then any ligands (technically
	 * non-water non-polymer atoms) within
	 * {@link StructureTools#DEFAULT_LIGAND_PROXIMITY_CUTOFF} of the selected range
	 * are included, regardless of chain.
	 * 
	 * @param input A full structure, e.g. as loaded from the PDB. The structure ID
	 *              should match that returned by getPdbId().
	 * @return
	...
	public Structure reduce(Structure s) throws StructureException {
		...
		newS.getPDBHeader().setDescription(new StringBuilder().append("sub-range ").append(ranges).append(" of ")
				.append(newS.getPDBCode()).append(" ").append(s.getPDBHeader().getDescription()).toString());
		newS.setEntityInfos(new ArrayList<>());
		// TODO The following should be only copied for atoms which are present in the
		// range.
			...
			if (getResidueRanges().isEmpty()) {
				...
				for (ResidueRange range : getResidueRanges()) {
					...
					if ("_".equals(chainName)) {
						...
						if (polyChain == null) {
							...
							try {
								...
								logger.warn(
										"No chain found for {}. Interpretting it as an index, using chain {} instead",
										chainName, polyChain.getId());
							} catch (NumberFormatException e3) {
								logger.error(e3.getMessage(), e3);
								// Not an index. Throw the original exception
								throw new StructureException(
										String.format("Unrecognized chain %s in %s", chainName, getIdentifier()));
							}
						}
					}
					...
					if (pdbresnum1 == null && pdbresnum2 == null) {
						...
						copyLigandsByProximity(s, newS, StructureTools.DEFAULT_LIGAND_PROXIMITY_CUTOFF, modelNr,
								modelNr);
					}
				} // end range
			}
	 ...
	 * on a distance cutoff. Ligand groups are moved (destructively) from full to
	 * reduced if they fall within the cutoff of any atom in the reduced structure.
	 * The {@link StructureTools#DEFAULT_LIGAND_PROXIMITY_CUTOFF default cutoff} is
	 * used.
	 * 
	 * @param full    Structure containing all ligands
	 * @param reduced Structure with a subset of the polymer groups from full
	 * @see StructureTools#getLigandsByProximity(java.util.Collection, Atom[],
	 *      double)
	 */
	 ...
	 *      double)
	 */
	protected static void copyLigandsByProximity(Structure full, Structure reduced, double cutoff, int fromModel,
			int toModel) {
				...
				.reduce((Chain) null, // reduction updates the chain guess
						(guess, g) -> {
							...
							try {
								...
								wasAdded = reduced.findGroup(g.getChainId(), g.getResidueNumber().toString(),
										toModel) != null;
							...
							if (!wasAdded) {
								...
								logger.info("Adding ligand group {} {} by proximity", g.getPDBName(),
										g.getResidueNumber().toPDB());
