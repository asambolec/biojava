 ...
 *
 * A structure object allows to access the PDB header information as well as to
 * the data from the ATOM records. The header information is currently available
 ...
 * </ul>
 *
 * The structure object provides access to the data from the ATOM records
 * through a hierarchy of sub-object:
 * 
 * <pre>
 * Structure
 *         |
 ...
 *
 * <p>
 * The tutorial for the BioJava structure modules can be found at <a href=
 * "https://github.com/biojava/biojava3-tutorial/tree/master/structure">github</a>.
 * </p>
 *
 *
 * <hr/>
...
public interface Structure extends Cloneable, Serializable {
	 ...
	 * 
	 * @param structureIdentifier the structureIdentifier corresponding to this
	 *                            structure
	 */
	...
	/**
	 * Return the number of models . In this implementation also XRAY structures
	 * have "1 model", since model is the container for the chains. to test if a
	 * Structure is an NMR structure use {@link #isNmr()}.
	 *
	...
	/**
	 * Test if this structure is a crystallographic structure, i.e. it is an
	 * asymmetric unit from which it is possible to reconstruct the crystal lattice
	 * given cell parameters and space group.
	 *
	...
	/**
	 * A convenience function if one wants to edit and replace the models in a
	 * structure. Allows to set (replace) the model at position with the new List of
	 * Chains.
	 * 
	...
	/**
	 * Set the chains of a structure, if this is a NMR structure, this will only set
	 * model 0.
	 *
	...
	/**
	 * Request a particular chain from a structure. by default considers only the
	 * first model.
	 * 
	 * @param authId name of a chain that should be returned
	 * @return Chain the requested chain
	 * @throws StructureException
	 * @Deprecated use {@link #getPolyChainByPDB(String)} or
	 *             {@link #getNonPolyChainsByPDB(String)} instead
	 */
	@Deprecated
	 ...
	 * @return Chain the requested chain
	 * @throws StructureException
	 * @Deprecated use {@link #getPolyChainByPDB(String, int)} or
	 *             {@link #getNonPolyChainsByPDB(String, int)} instead
	 */
	@Deprecated
	...
	/**
	 * Check if a non polymeric chain with chainId asymId is contained in the
	 * structure.
	 *
	 * @param asymId the id of the chain
	...
	/**
	 * Request a particular group from a structure. by default considers only the
	 * first model in the structure.
	 * 
	...
	/**
	 * Request a particular group from a structure. considers only model nr X. count
	 * starts with 0.
	 * 
	...
	/**
	 * Request a chain by its public id (author id) for the first model. Before 5.0
	 * it returned a Chain that had both polymeric and non-polymeric groups
	 * following the PDB-file data model. Since 5.0 it only returns the polymeric
	 * part of the chain.
	 *
	 ...
	 * Before 5.0 it returned a Chain that had both polymeric and non-polymeric
	 * groups following the PDB-file data model. Since 5.0 it only returns the
	 * polymeric part of the chain.
	 * 
	 * @param authId   the author id (chainName, public chain id)
	...
	/**
	 * Retrieve a Chain (polymeric, non-polymeric or water) based on the 'internal'
	 * chain id (asymId) for the first model
	 * 
	...
	/**
	 * Retrieve a Chain (polymeric, non-polymeric or water) based on the 'internal'
	 * chain id (asymId) for the given model index
	 * 
	 * @param asymId   the asymId (chainId)
	 * @param modelIdx the index of the required model (0-based)
	 * @return
	...
	/**
	 * Retrieve a polymeric Chain based on the 'internal' chain id (asymId) for the
	 * first model
	 * 
	 * <p>
	 * See {@link #getPolyChainByPDB(String)} for a similar method using the chain
	 * name (authId).
	 * 
	 * @param asymId the asymId (chainId)
	 ...
	 * 
	 * <p>
	 * See {@link #getPolyChainByPDB(String, int)} for a similar method using the
	 * chain name (authId).
	 * 
	 * @param asymId   the asymId (chainId)
	 * @param modelIdx the index of the required model (0-based)
	...
	/**
	 * Retrieve a polymeric Chain based on the 'public' chain name (authId) for the
	 * first model
	 * 
	 * <p>
	 * See {@link #getPolyChain(String)} for a similar method using the chain id
	 * (asymId).
	 * 
	 * @param authId the author id (chainName, public chain id)
	 * @return a polymeric Chain or null if it can't be found
	 ...
	 * 
	 * <p>
	 * See {@link #getPolyChain(String, int)} for a similar method using the chain
	 * id (asymId).
	 * 
	 * @param authId   the author id (chainName, public chain id)
	 * @param modelIdx the index of the required model (0-based)
	...
	/**
	 * Retrieve a non-polymeric Chain based on the 'internal' chain id (asymId) for
	 * the first model
	 * 
	 * @param asymId the asymId (chainId)
	...
	/**
	 * Retrieve all non-polymeric Chains corresponding to the 'public' chain name
	 * (authId) and the given model index.
	 * 
	 * @param authId   the author id (chainName, public chain id)
	 * @param modelIdx the index of the required model (0-based)
	 * @return a list of non-polymeric Chains, if none found the list will be empty
	...
	/**
	 * Retrieve a water Chain based on the 'internal' chain id (asymId) for the
	 * first model
	 * 
	 * @param asymId the asymId (chainId)
	...
	/**
	 * Retrieve a water Chain based on the 'public' chain name (authId) for the
	 * first model
	 * 
	 * @param authId the author id (chainName, public chain id)
	 * @return
	 ...
	 * The JRNL section is not mandatory and thus may not be present.
	 * 
	...
	/**
	 * Returns the list of {@link ResidueRange ResidueRanges} that this
	 * StructureIdentifier defines. This is a unique representation.
	 * 
	...
	/**
	 * Get a string representing this structure's contents. The following places are
	 * searched for a non-null value, with the first being returned:
	 * <ol>
	 * <li>{@link #getStructureIdentifier()}.getIdentifier(), which should give the
	 * string originally used to create the structure
	 * <li>{@link #getName()}
	 * <li>A combination of {@link #getPDBCode()} with a heuristic description of
	 * the residue ranges, in {@link SubstructureIdentifier} format.
	 * </ol>
	 * 
	 * @return A {@link SubstructureIdentifier}-format string describing the residue
	 *         ranges in this structure
