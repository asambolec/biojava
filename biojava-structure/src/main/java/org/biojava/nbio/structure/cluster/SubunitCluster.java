...
public class SubunitCluster {
	...
	/**
	 * A constructor from a single Subunit. To obtain a SubunitCluster with multiple
	 * Subunits, initialize different SubunitClusters and merge them.
	 * 
	 * @param subunit initial Subunit
	 */
	...
	/**
	 * A copy constructor with the possibility of removing subunits. No
	 * re-clustering is done.
	 *
	 * @param other            reference SubunitCluster
	 * @param subunitsToRetain which subunits to copy to this cluster
	 */
	...
	/**
	 * Merges the other SubunitCluster into this one if it contains exactly the same
	 * Subunit. This is checked by {@link #isIdenticalTo(SubunitCluster)}.
	 * 
	 * @param other SubunitCluster
	 ...
	 * 
	 * @param other  SubunitCluster
	 * @param params SubunitClustererParameters, with information whether to use
	 *               local or global alignment, sequence identity and coverage
	 *               thresholds. Threshold values lower than 0.7 are not
	 *               recommended. Use {@link #mergeStructure} for lower values.
	 * @return true if the SubunitClusters were merged, false otherwise
	...
	public boolean mergeSequence(SubunitCluster other, SubunitClustererParameters params)
		...
		return mergeSequence(other, params, alignerType, new SimpleGapPenalty(),
	 ...
	 * 
	 * @param other       SubunitCluster
	 * @param params      {@link SubunitClustererParameters}, with information
	 *                    whether to use local or global alignment, sequence
	 *                    identity and coverage thresholds. Threshold values lower
	 *                    than 0.7 are not recommended. Use {@link #mergeStructure}
	 *                    for lower values.
	...
	public boolean mergeSequence(SubunitCluster other, SubunitClustererParameters params,
			PairwiseSequenceAlignerType alignerType, GapPenalty gapPenalty,
			SubstitutionMatrix<AminoAcidCompound> subsMatrix) throws CompoundNotFoundException {
		...
		PairwiseSequenceAligner<ProteinSequence, AminoAcidCompound> aligner = Alignments
				.getPairwiseAligner(thisSequence, otherSequence, alignerType, gapPenalty, subsMatrix);
		...
		logger.info(String.format(
				"SubunitClusters are similar in sequence " + "with %.2f sequence identity and %.2f coverage",
		...
		thisRemove.forEach(column -> this.subunitEQR.forEach(eqr -> eqr.remove(column)));

		otherRemove.forEach(column -> other.subunitEQR.forEach(eqr -> eqr.remove(column)));
	...
	/**
	 * Merges the other SubunitCluster into this one if their representative Atoms
	 * are structurally similar (according to the criteria in params).
	 * <p>
	 *
	 * @param other  SubunitCluster
	 * @param params {@link SubunitClustererParameters}, with information on what
	 *               alignment algorithm to use, RMSD/TMScore and structure coverage
	 *               thresholds.
	 * @return true if the SubunitClusters were merged, false otherwise
	...
	public boolean mergeStructure(SubunitCluster other, SubunitClustererParameters params) throws StructureException {
		...
		MultipleAlignment msa = new MultipleAlignmentEnsembleImpl(afp,
				this.subunits.get(this.representative).getRepresentativeAtoms(),
				other.subunits.get(other.representative).getRepresentativeAtoms(), false).getMultipleAlignment(0);
		...
		logger.info(String.format("SubunitClusters are structurally similar with " + "%.2f RMSD %.2f coverage", rmsd,
				structureCoverage));
	 ...
	 * {@link Subunit} into the internal repeats (domains) if they are internally
	 * symmetric.
	 * 
	 * @param clusterParams {@link SubunitClustererParameters} with fields used as
	 *                      follows: structureCoverageThreshold the minimum coverage
	 *                      of all repeats in the Subunit rmsdThreshold the maximum
	 *                      allowed RMSD between the repeats minimumSequenceLength
	...
	public boolean divideInternally(SubunitClustererParameters clusterParams) throws StructureException {
		...
		logger.info("SubunitCluster is internally symmetric with {} repeats, " + "{} RMSD and {} coverage",
				result.getNumRepeats(), rmsd, coverage);
		...
		alignedRes.forEach(alignedRe -> columns.add(new ArrayList<Integer>(alignedRe.size())));
		...
		for (int col = 0; col < alignedRes.get(0).size(); col++) {
			...
			for (List<Integer> alignedRe : alignedRes) {
				if (!subunitEQR.get(representative).contains(alignedRe.get(col))) {
					missing = true;
					break;
				}
			}
		...
		List<Subunit> newSubunits = new ArrayList<>(subunits.size() * columns.size());
		List<List<Integer>> newSubunitEQR = new ArrayList<>(subunits.size() * columns.size());

		for (int s = 0; s < subunits.size(); s++) {
			for (List<Integer> column : columns) {

				// Calculate start and end residues of the new Subunit
				int start = subunitEQR.get(s).get(column.get(0));
				int end = subunitEQR.get(s).get(column.get(column.size() - 1));

				Atom[] reprAtoms = Arrays.copyOfRange(subunits.get(s).getRepresentativeAtoms(), start, end + 1);

				newSubunits.add(new Subunit(reprAtoms, subunits.get(s).getName(), subunits.get(s).getIdentifier(),
						subunits.get(s).getStructure()));

				// Recalculate equivalent residues
				List<Integer> eqr = new ArrayList<>();
				for (int p = 0; p < column.size(); p++) {
					eqr.add(subunitEQR.get(s).get(column.get(p)) - start);
				}
	...
	/**
	 * @return the {@link SubunitClustererMethod} used for clustering the Subunits
	 */
	...
	/**
	 * @param index Subunit index in the Cluster
	 * @return An Atom array of length {@link #length()} with the aligned Atoms from
	 *         the selected Subunit in the Cluster
	 */
	 ...
	 * SubunitCluster. The alignment is recalculated every time the method is called
	 * (no caching).
	 * 
	...
	public MultipleAlignment getMultipleAlignment() throws StructureException {
		...
		msa.getEnsemble()
				.setAtomArrays(subunits.stream().map(Subunit::getRepresentativeAtoms).collect(Collectors.toList()));
	...
	public String toString() {
		return new StringBuilder().append("SubunitCluster [Size=").append(size()).append(", Length=").append(length())
				.append(", Representative=").append(representative).append(", Method=").append(method).append("]")
				.toString();
	}

	/**
	 * @return true if this cluster is considered pseudo-stoichiometric (i.e., was
	 *         either clustered by structure, or by sequence with low scores), false
	 *         otherwise.
	 */
