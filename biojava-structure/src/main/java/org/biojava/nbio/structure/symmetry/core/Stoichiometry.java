...
/**
 * A utility object that describes Stoichiometry (composition of a protein
 * assembly), determined via clustering procedure
 * {@link org.biojava.nbio.structure.cluster.SubunitClusterer}, and implements
 * human-readable representation using various strategies.
 *
...
public class Stoichiometry {
	...
	/**
	 * Alphabet (a sequence of characters) used in this stoichiometry to construct
	 * human-readable representation.
	 */
	private String alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";

	/**
	 * Strategy determines how this stoichiometry will construct human-readable
	 * representation in case number of clusters exceeds number of letters in the
	 * alphabet.
	 */
	private StringOverflowStrategy strategy = StringOverflowStrategy.CYCLE;

	/**
	 * Full customisation of the string generation is supported via an external
	 * function
	 */
	...
	/**
	 * Constructor for Stoichiometry. The default strategy is CYCLE, the letters
	 * assigned for each cluster will be reset.
	 *
	 * @param clusters List of {@link SubunitCluster} that defines assembly
	 *                 composition.
	 */
	 ...
	 * @param clusters    List of {@link SubunitCluster} that defines assembly
	 *                    composition.
	 * @param resetAlphas Whether to keep alphas assigned to {@link SubunitCluster}
	 *                    object (useful for local symmetry detection) or to
	 *                    generate them anew.
	 */
	...
	/**
	 * Constructor for Stoichiometry. The alphas assigned to {@link SubunitCluster}
	 * objects will be reset.
	 *
	 * @param clusters List of {@link SubunitCluster} that defines assembly
	 *                 composition.
	 * @param strategy What to do if number of {@link SubunitCluster} exceeds the
	 *                 alphabet length.
	 */
	...
	public Stoichiometry(List<SubunitCluster> clusters, StringOverflowStrategy strategy, boolean resetAlphas) {
		...
		this.orderedClusters = clusters.stream().sorted(Comparator.comparing(SubunitCluster::size).reversed())
	 ...
	 * Constructor for Stoichiometry.
	 *
	 * @param clusters              List of {@link SubunitCluster} that defines
	 *                              assembly composition.
	 * @param customStringGenerator A function which produces a string for a
	 *                              composition (list of subunit clusters).
	 */
	...
	/**
	 * Produce a string ("alpha") that describes each component depending on the
	 * current strategy.
	 * 
	 * @param clusterInd component index
	...
	/**
	 * @return list of {@link SubunitCluster}, ordered by the number of subunits
	 *         (decreasing).
	 */
	...
	/**
	 * Make a combined Stoichiometry object of <i>this</> and the <i>other</>. The
	 * combined list of clusters will be ordered by the number of subunits.
	 * 
	...
	/**
	 * @return {@link StringOverflowStrategy} used in this stoichiometry to
	 *         construct human-readable representation in case number of clusters
	 *         exceeds number of letters in the alphabet.
	 */
	...
	/**
	 * Change string representation of a stoichiometry in case number of clusters
	 * exceeds number of letters in the alphabet. This action may invalidate alphas
	 * already assigned to the clusters.
	 * 
	 * @param strategy {@link StringOverflowStrategy} used in this stoichiometry to
	 *                 construct human-readable representation in case number of
	 *                 clusters exceeds number of letters in the alphabet.
	 */
	...
	/**
	 * Let a user-defined function handle the entire string representation of a
	 * stoichiometry.
	 * 
	 * @param customStringGenerator A function which accepts a list of subunit
	 *                              clusters and returns a string.
	 */
	...
	/**
	 * @return Alphabet (a sequence of characters) used in this stoichiometry to
	 *         construct human-readable representation.
	 */
	...
	/**
	 * Change alphabet used for string representation of a stoichiometry. This
	 * action invalidates alphas already assigned to the clusters.
	 * 
	 * @param alphabet a sequence of characters used in this stoichiometry to
	 *                 construct human-readable representation.
	 */
		...
		if (strategy == StringOverflowStrategy.CUSTOM) {
			if (customStringGenerator == null) {
				throw new IllegalStateException(
						"The strategy is CUSTOM, yet the string generator function is not defined.");
			}
	 ...
	 * {@link SubunitClustererMethod#STRUCTURE} similarity, or
	 * {@link SubunitClustererMethod#SEQUENCE} similarity with low scores.
	 *
	 * @return true if any of the clusters is pseudostoichiometric, false otherwise
	 */
	public boolean isPseudoStoichiometric() {
		return orderedClusters.stream().anyMatch(SubunitCluster::isPseudoStoichiometric);
	}

	/**
	 * What to do when the number of {@link SubunitCluster} exceeds the length of
	 * the alphabet.
	 */
	public enum StringOverflowStrategy {
		...
		CYCLE,
		/**
		 * Represent every cluster with two symbols from the alphabet, this forces us to
		 * specify number of subunits for every subunit (e.g., AA1AB1AC1...). This
		 * strategy will not work correctly if there are more than alphabet.length^2
		 * subunit clusters.
		 */
		DOUBLE,
		/**
		 * The strategy is defined via an external function, we do not have to do
		 * anything.
		 */
		CUSTOM
	}
