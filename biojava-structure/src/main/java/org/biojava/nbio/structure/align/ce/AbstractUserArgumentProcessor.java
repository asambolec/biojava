 ...
 * <li>Implement ConfigStrucAligParams. This provides the parameters for the
 * GUI.
 * <li>Subclass StartupParameters (can be an inner class) with the same
 * parameters described in the ConfigStrucAligParams.
 * <li>Subclass AbstractUserArgumentProcessor, with the getStartupParams()
 * method returning a fresh instance of the custom StartupParameters
 * <li>Implement the getParameters() method to copy values from the
 * StartupParameters to the ConfigStrucAligParams.
 * </ol>
 *
 * <p>
 * Note that reflection is used in a number of places, so the CLI argument names
 ...
 * <li>AbstractUserArgumentProcessor also takes parameter names and help
 * descriptions from ConfigStrucAligParams, but it saves arguments to the
 * StartupParameter bean.
 * </ul>
...
public abstract class AbstractUserArgumentProcessor implements UserArgumentProcessor {
	...
	/**
	 * StartupParameters is a bean to store all the possible command line
	 * parameters.
	 *
	 * The `StartupParameter` class contains the basic set of CLI parameters common
	 * to all `StructureAligmnent`s. This method should return a subclass of
	 * StartupParameters which has been extended to store values for all additional
	 * parameters.
	 * 
	...
	public void process(String[] argv) {
		...
		for (int i = 0; i < argv.length; i++) {
			...
			if ("-version".equalsIgnoreCase(arg) || "--version".equalsIgnoreCase(arg)) {
				...
				logger.info(new StringBuilder().append(alg.getAlgorithmName()).append(" v.").append(alg.getVersion())
						.toString());
				return;
			}
		...
		if (params.getShowDBresult() != null) {
			...
			logger.error("showing DB results...");
			try {
	...
	public static void printAboutMe() {
		try {
			...
			logger.info(new StringBuilder().append("Protein Comparison Tool ").append(version).append(" ").append(build)
					.toString());
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
		}
	...
	private void runDBSearch() throws ConfigurationException {
		...
		if (pdbFilePath == null || "".equals(pdbFilePath)) {

			UserConfiguration c = new UserConfiguration();
			pdbFilePath = c.getPdbFilePath();
			logger.error(new StringBuilder().append("You did not specify the -pdbFilePath parameter. Defaulting to ")
					.append(pdbFilePath).append(".").toString());
		}

		String cacheFilePath = params.getCacheFilePath();

		if (cacheFilePath == null || "".equals(cacheFilePath)) {
		...
		if (alignPairs == null || "".equals(alignPairs)) {
	 ...
	 */
	private void runDbSearch(AtomCache cache, String searchFile, String outputFile, int useNrCPUs,
			StartupParameters params) throws ConfigurationException {

		logger.info(new StringBuilder().append("will use ").append(useNrCPUs).append(" CPUs.").toString());
		...
		MultiThreadedDBSearch dbSearch = new MultiThreadedDBSearch(name1, structure1, outputFile, algorithm, useNrCPUs,
	...
	private void runPairwise() throws ConfigurationException {
		...
		if (name1 == null && file1 == null) {
			throw new ConfigurationException(
					"You did not specify the -pdb1 or -file1 parameter. Can not find query PDB id for alignment.");
		}

		if (file1 == null) {
			if (name1.length() < 4) {
				throw new ConfigurationException(
						"-pdb1 does not look like a PDB ID. Please specify PDB code or PDB.chainName.");
			}
		}
		...
		if (name2 == null && file2 == null) {
			throw new ConfigurationException(
					"You did not specify the -pdb2 or -file2 parameter. Can not find target PDB id for alignment.");
		}

		if (file2 == null) {
			if (name2.length() < 4) {
				throw new ConfigurationException(
						"-pdb2 does not look like a PDB ID. Please specify PDB code or PDB.chainName.");
			}
		}
		...
		if (file1 == null || file2 == null) {
			if (path == null) {
				...
				logger.error(
						new StringBuilder().append("You did not specify the -pdbFilePath parameter. Defaulting to ")
		...
		// default: new:
		// 1buz - 1ali : time: 8.3s eqr 68 rmsd 3.1 score 161 | time 6.4 eqr 58 rmsd 3.0
		// scre 168
		// 5pti - 1tap : time: 6.2s eqr 48 rmsd 2.67 score 164 | time 5.2 eqr 49 rmsd
		// 2.9 score 151
		// 1cdg - 8tim
		// 1jbe - 1ord
		// 1nbw.A - 1kid
		...
		try {
			...
			if (params.isShow3d()) {
				...
				if (!GuiWrapper.isGuiModuleInstalled()) {
					...
					// StructureAlignmentJmol jmol = algorithm.display(afpChain,ca1,ca2,hetatms1,
					// nucs1, hetatms2, nucs2);
		...
		} catch (StructureException | IllegalAccessException | InvocationTargetException | NoSuchMethodException
	 ...
	 * @param ca1
	 * @param ca2
	 * @throws IOException               If an error occurs when writing the
	 *                                   afpChain to XML
	...
	private void checkWriteFile(AFPChain afpChain, Atom[] ca1, Atom[] ca2, boolean dbsearch)
			...
			IllegalAccessException, StructureException {
		String output = null;
		if (params.isOutputPDB()) {
			if (!GuiWrapper.isGuiModuleInstalled()) {
				...
				output = new StringBuilder().append("TITLE  ").append(afpChain.getAlgorithmName()).append(" ")
						.append(afpChain.getVersion()).append(" ").toString();
				output += new StringBuilder().append(afpChain.getName1()).append(" vs. ").append(afpChain.getName2())
						.toString();
				output += newline;
				output += tmp.toPDB();
			}
		...
		if (dbsearch) {
			if (params.getSaveOutputDir() != null) {
				...
				// we currently don't have a naming convention for how to store results for
				// custom files
		...
		if (fileName == null) {
			logger.error(
					"Can't write outputfile. Either provide a filename using -outFile or set -autoOutputFile to true .");
			System.exit(1);
			return;
		}
		// System.out.println("writing results to " + fileName + " " +
		// params.getSaveOutputDir());
	...
	private String getAutoFileName(AFPChain afpChain) {
		String fileName = new StringBuilder().append(afpChain.getName1()).append("_").append(afpChain.getName2())
				.append("_").append(afpChain.getAlgorithmName()).toString();
	...
	private Structure getStructure(AtomCache cache, String name1, String file) {
		...
		try {
			...
			logger.error(e.getMessage(), e);
			logger.error(new StringBuilder().append("unable to load structure from dir: ").append(cache.getPath())
					.append("/").append(name1).toString());
			return null;
		}
	...
	/**
	 * apply a number of rules to fix the name of the structure if it did not get
	 * set during loading.
	 *
	 * @param s
	 * @param file
	...
	public String printHelp() {
		...
		buf.append(new StringBuilder().append(alg.getAlgorithmName()).append(" v.").append(alg.getVersion())
				.append(" help: ").append(newline).toString());
		buf.append("-------------------").append(newline);
		buf.append(newline);
		...
		buf.append("   -pdbFilePath  Path to the directory in your file system that contains the PDB files.")
				.append(newline);
		buf.append(
		...
		buf.append("   -show3d print a 3D visualisation of the alignment (requires jmolapplet.jar in classpath)")
				.append(newline);
		...
		buf.append("   -outFile (mandatory) a directory that will contain the results of the DB search")
				.append(newline);
		buf.append(
				"   -nrCPU (optional) Number of CPUs to use for the database search. By default will use the all, but one CPU in the system.")
				.append(newline);
		buf.append("   -pdbFilePath (mandatory) Path to the directory in your file system that contains the PDB files.")
				.append(newline);
		buf.append(
		...
		buf.append(" If only a PDB code is provided, the whole structure will be used for the alignment.")
				.append(newline);
		buf.append(" To specify a particular chain write as: 4hhb.A (chain IDs are case sensitive, PDB ids are not)")
				.append(newline);
