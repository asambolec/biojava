...
public enum ExperimentalTechnique {
	...
	SOLUTION_NMR("SOLUTION NMR", false, true), SOLID_STATE_NMR("SOLID-STATE NMR", false, true),
	...
	// from here, not in "official list" in pdb.org advanced search: they call these
	// "OTHER"
	...
	/**
	 * Returns the ExpTechnique given an experimental technique name as used in the
	 * PDB, e.g. "X-RAY DIFFRACTION" returns
	 * {@link ExperimentalTechnique#XRAY_DIFFRACTION}
	 * 
	 * @param expTechniqueName the ExpTechnique value or null if string doesn't
	 *                         match one of the known PDB experimental strings
	 * @return
	 */
	...
	/**
	 * Given a Set of ExperimentalTechniques returns true if at least one is
	 * crystallographic
	 * 
	...
	public static boolean isCrystallographic(Set<ExperimentalTechnique> techniques) {
		...
		return techniques.stream().anyMatch(ExperimentalTechnique::isCrystallographic);
	 ...
	 * 
	 * @return true if at least 1 of the techniques is NMR, false if none of the
	 *         techniques are NMR
	 * @throws NullPointerException if input is null
	 */
	public static boolean isNmr(Set<ExperimentalTechnique> techniques) {

		return techniques.stream().anyMatch(ExperimentalTechnique::isNmr);
	}
