...
public class AFPOptimizer {
	...
	public static void optimizeAln(FatCatParameters params, AFPChain afpChain, Atom[] ca1, Atom[] ca2)
		...
		if (debug) {
			logger.info("AFPOptimizer got blockNum: " + blockNum);
		}
		...
		for (bk = 0; bk < blockNum; bk++) {
			...
			} // use the middle point of the current and next AFPs. old (starting point of
				// next AFP)
			else {
				e1 = ca1.length;
				e2 = ca2.length;
			...
			// optimize the align by dynamic programming & constraint the optimization
			// region
			if (debug) {
				logger.error(String.format("optimize block %d (%d afp), region %d-%d(len %d), %d-%d(len %d)%n", bk,
						blockSize[bk], b1, e1, e1 - b1, b2, e2, e2 - b2));
			...
			// SALNOPT *opt = new SALNOPT(e1-b1, &pro1->caCod[3 * b1], e2-b2, &pro2->caCod[3
			// * b2], iniLen, iniSet, maxi);
			// optRmsd[bk] = opt->OptimizeResult(&optLen[bk], optAln[bk]);

			if (debug) {
				logger.info(String.format(" optimized len=%d, rmsd %f%n", optLen[bk], optRmsd[bk]));
			}
		...
		if (debug) {
			logger.info(
					new StringBuilder().append("complete AlignOpt ").append(optEnd - optStart).append("\n").toString());
		}
	...
	/**
	 * to update the chaining score after block delete and merge processed the
	 * blockScore value is important for significance evaluation
	 */
