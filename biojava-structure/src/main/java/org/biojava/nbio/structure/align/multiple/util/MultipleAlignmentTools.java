...
public class MultipleAlignmentTools {
	...
	/**
	 * Calculate the sequence alignment Strings for the whole alignment. This method
	 * creates a sequence alignment where aligned residues are in uppercase and
	 * unaligned residues are in lowercase, thus providing a more compact way to
	 * represent the alignment.
	 * <p>
	 * Blocks are concatenated in the order returned by
	 * {@link MultipleAlignment#getBlocks()}, so sequences may not be sequential.
	 * Gaps are represented by '-'. Separation between different Blocks is indicated
	 * by a gap in all positions, meaning that there is a possible discontinuity.
	 *
	 * @param alignment      input MultipleAlignment
	 * @param mapSeqToStruct provides a link from the sequence alignment position to
	 *                       the structure alignment position. Specially designed
	 *                       for the GUI. Has to be initialized previously and will
	 *                       be overwritten.
	 * @return a string for each row in the alignment, giving the 1-letter code for
			...
			for (int pos = 0; pos < alignment.getBlocks().get(b).length(); pos++) {
				...
				while (gaps) {
					...
					if (gaps) {
						for (int str = 0; str < alignment.size(); str++) {
							if (provisionalChar[str] == ' ') {
								...
								alnSequences.set(str,
										(alnSequences.get(str) + String.valueOf(Character.toLowerCase(aa))));
								previousPos[str]++;
							} else {
						...
						for (int str = 0; str < alignment.size(); str++) {
							alnSequences.set(str, (alnSequences.get(str) + String.valueOf(provisionalChar[str])));
	 ...
	 *
	 * @param alignment input MultipleAlignment
	 * @return String for each row in the alignment, giving the 1-letter code for
	 *         each aligned residue.
	 */
	 ...
	 * alignment. This method creates a sequence alignment where all residues are in
	 * uppercase and a residue alone with gaps in all the other structures
	 * represents unaligned residues. Because of this latter constraint only the
	 * residues within the Blocks are represented, for a more compact alignment. For
	 * a sequence alignment of the full protein use
	 * {@link #getSequenceAlignment(MultipleAlignment)}.
	 * <p>
	 * Blocks are concatenated in the order returned by
	 * {@link MultipleAlignment#getBlocks()}, so sequences may not be sequential.
	 * Gaps between blocks are omitted, while gaps within blocks are represented by
	 * '-'. Separation between different Blocks is indicated by a gap in all
	 * positions, meaning that there is something unaligned inbetween.
	 *
	 * @param alignment      input MultipleAlignment
	 * @param mapSeqToStruct provides a link from the sequence alignment position to
	 *                       the structure alignment position. Specially designed
							...
							if (provisionalChar[str] == ' ') {
								...
								for (int s2 = 0; s2 < alignment.size(); s2++) {
									if (str == s2) {
										...
										String g = a.getGroup().getPDBName();
										char aa = StructureTools.get1LetterCode(g);
										alnSequences.set(s2, (alnSequences.get(s2) + String.valueOf(aa)));
									} else {
										alnSequences.set(s2, (alnSequences.get(s2) + "-"));
									}
								}
	...
	/**
	 * Returns the Atom of the specified structure that is aligned in the sequence
	 * alignment position specified.
	 *
	 * @param multAln        the MultipleAlignment object from where the sequence
	 *                       alignment has been generated
	 * @param mapSeqToStruct the mapping between sequence and structure generated
	 ...
	 * given the mapping from structure to function.
	 *
	 * @param multAln        the MultipleAlignment object from where the sequence
	 *                       alignment has been generated.
	 * @param mapSeqToStruct the mapping between sequence and structure generated
	...
	/**
	 * The average residue distance Matrix contains the average distance from each
	 * residue to all other residues aligned with it.
	 * <p>
	 ...
	 * length.
	 *
	 * @param transformed List of Atom arrays containing only the aligned atoms of
	 *                    each structure, or null if there is a gap.
	 * @return Matrix containing all average residue distances. Entry -1 means there
	 *         is a gap in the position.
	 */
	 ...
	 * <p>
	 * For each structure in the alignment, returns an atom for each representative
	 * atom in the aligned columns, omitting unaligned residues (i.e. an array of
	 * length <tt>alignment.length()</tt> ).
	 * <p>
	 ...
	 * residues (gaps), then the returned array will also contain null Atoms in the
	 * same positions.
	 *
	 * @param alignment MultipleAlignment
	...
	public static void sortBlocks(List<Block> blocks, final int sortedIndex) {
		Collections.sort(blocks, new Comparator<Block>() {
			...
			public int compare(Block o1, Block o2) {
				...
				Integer res1 = alignres1.stream().filter(r -> r != null).findFirst().orElse(null);
				Integer res2 = alignres2.stream().filter(r -> r != null).findFirst().orElse(null);
	...
	public static Structure toMultimodelStructure(MultipleAlignment multAln, List<Atom[]> transformedAtoms)
		...
		String title = new StringBuilder().append(multAln.getEnsemble().getAlgorithmName()).append(" V.")
				.append(multAln.getEnsemble().getVersion()).append(" : ").toString();
	 ...
	 * structure, so that the alignment result can be viewed in Jmol. The Atoms have
	 * to be rotated beforehand.
	 *
	...
	public static final Structure getAlignedStructure(List<Atom[]> atomArrays) throws StructureException {
		...
		atomArrays.stream().map(AlignmentTools::getAlignedModel).forEach(s::addModel);
		return s;
	}

	/**
	 * Calculate the RMSD matrix of a MultipleAlignment, that is, entry (i,j) of the
	 * matrix contains the RMSD between structures i and j.
	 *
	 * @param msa Multiple Structure Alignment
	...
	/**
	 * Calculate a phylogenetic tree of the MultipleAlignment using Kimura distances
	...
	/**
	 * Calculate a phylogenetic tree of the MultipleAlignment using dissimilarity
	 * scores (DS), based in SDM Substitution Matrix (ideal for distantly related
	 * proteins, structure-derived) and the Neighbor Joining algorithm from
	 * forester.
	 *
	 * @param msta MultipleAlignment of protein structures
	 * @return Phylogeny phylogenetic tree
	...
	/**
	 * Calculate a phylogenetic tree of the MultipleAlignment using RMSD distances
	 * and the Neighbor Joining algorithm from forester.
	 *
	 * @param msta MultipleAlignment of protein structures
	 * @return Phylogeny phylogenetic tree
