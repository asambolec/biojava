 ...
 * </pre>
 * 
 * For more documentation see <a href=
 * "http://biojava.org/wiki/BioJava:CookBook#Protein_Structure">http://biojava.org/wiki/BioJava:CookBook#Protein_Structure</a>.
 *
...
public class SimpleMMcifParser implements MMcifParser {
	...
	/**
	 * The header appearing at the beginning of a mmCIF file. A "block code" can be
	 * added to it of no more than 32 chars. See
	...
	public void parse(BufferedReader buf) throws IOException {
		...
		while ((line = buf.readLine()) != null) {
			...
			if (!foundHeader) {
				// the first non-comment line is a data_PDBCODE line, test if this looks like a
				// mmcif file
			...
			logger.debug(new StringBuilder().append(inLoop).append(" ").append(line).toString());

			if (line.startsWith(MMCIF_TOP_HEADER)) {
				// either first line in file, or beginning of new section (data block in CIF
				// parlance)
				if (inLoop) {
			...
			if (inLoop) {
				...
				if (line.matches(new StringBuilder().append("\\s*").append(FIELD_LINE).append("\\w+.*").toString())) {
					...
					if (lineData.size() != loopFields.size()) {
						logger.warn("Expected {} data fields, but found {} in line: {}", loopFields.size(),
								lineData.size(), line);
		...
		while (true) {
			...
			if (line.startsWith(STRING_LIMIT)) {
				...
				if (inString) {
					...
					dat.forEach(lineData::add);
				}
			}

			// System.out.println("in process line : " + lineData.size() + " " +
			// fieldLength);

			if (lineData.size() > fieldLength) {

				logger.warn(new StringBuilder().append("wrong data length (").append(lineData.size())
						.append(") should be (").append(fieldLength).append(") at line ").append(line)
						.append(" got lineData: ").append(lineData).toString());
				return lineData;
			}
	...
	private void endLineChecks(String category, List<String> loopFields, List<String> lineData,
			Set<String> loopWarnings) throws IOException {
		...
		if (loopFields.size() != lineData.size()) {
			...
			throw new IOException(new StringBuilder().append("data length (").append(lineData.size())
					.append(") != fields length (").append(loopFields.size()).append(") category: ").append(category)
					.append(" fields: ").append(loopFields).append(" DATA: ").append(lineData).toString());
		}

		if ("_entity".equals(category)) {

			Entity e = (Entity) buildObject(Entity.class.getName(), loopFields, lineData, loopWarnings);
			triggerNewEntity(e);

		} else if ("_entity_poly".equals(category)) {
			EntityPoly ep = (EntityPoly) buildObject(EntityPoly.class.getName(), loopFields, lineData, loopWarnings);
			triggerNewEntityPoly(ep);

		} else if ("_struct".equals(category)) {

			struct = (Struct) buildObject(Struct.class.getName(), loopFields, lineData, loopWarnings);

		} else if ("_atom_site".equals(category)) {

			AtomSite a = (AtomSite) buildObject(AtomSite.class.getName(), loopFields, lineData, loopWarnings);
			triggerNewAtomSite(a);

		} else if ("_database_PDB_rev".equals(category)) {
			DatabasePDBrev dbrev = (DatabasePDBrev) buildObject(DatabasePDBrev.class.getName(), loopFields, lineData,
					loopWarnings);

			triggerNewDatabasePDBrev(dbrev);

		} else if ("_database_PDB_rev_record".equals(category)) {
			DatabasePdbrevRecord dbrev = (DatabasePdbrevRecord) buildObject(DatabasePdbrevRecord.class.getName(),
					loopFields, lineData, loopWarnings);

			triggerNewDatabasePDBrevRecord(dbrev);

			// MMCIF version 5 dates
		} else if ("_pdbx_audit_revision_history".equals(category)) {
			PdbxAuditRevisionHistory history = (PdbxAuditRevisionHistory) buildObject(
					PdbxAuditRevisionHistory.class.getName(), loopFields, lineData, loopWarnings);

			triggerNewPdbxAuditRevisionHistory(history);

			// MMCIF version 5 dates
		} else if ("_pdbx_database_status".equals(category)) {
			PdbxDatabaseStatus status = (PdbxDatabaseStatus) buildObject(PdbxDatabaseStatus.class.getName(), loopFields,
					lineData, loopWarnings);

			triggerNewPdbxDatabaseStatus(status);

		} else if ("_database_PDB_remark".equals(category)) {
			DatabasePDBremark remark = (DatabasePDBremark) buildObject(DatabasePDBremark.class.getName(), loopFields,
					lineData, loopWarnings);

			triggerNewDatabasePDBremark(remark);

		} else if ("_exptl".equals(category)) {
			Exptl exptl = (Exptl) buildObject(Exptl.class.getName(), loopFields, lineData, loopWarnings);

			triggerExptl(exptl);

		} else if ("_cell".equals(category)) {
			Cell cell = (Cell) buildObject(Cell.class.getName(), loopFields, lineData, loopWarnings);

			triggerNewCell(cell);

		} else if ("_symmetry".equals(category)) {
		...
		} else if ("_struct_ncs_oper".equals(category)) {

			StructNcsOper sNcsOper = (StructNcsOper) buildObject(StructNcsOper.class.getName(), loopFields, lineData,
					loopWarnings);
			triggerNewStructNcsOper(sNcsOper);
		} else if ("_atom_sites".equals(category)) {

			AtomSites atomSites = (AtomSites) buildObject(AtomSites.class.getName(), loopFields, lineData,
					loopWarnings);
			triggerNewAtomSites(atomSites);

		} else if ("_struct_ref".equals(category)) {
			StructRef sref = (StructRef) buildObject(StructRef.class.getName(), loopFields, lineData, loopWarnings);

			triggerNewStrucRef(sref);

		} else if ("_struct_ref_seq".equals(category)) {
			StructRefSeq sref = (StructRefSeq) buildObject(StructRefSeq.class.getName(), loopFields, lineData,
					loopWarnings);

			triggerNewStrucRefSeq(sref);
		} else if ("_struct_ref_seq_dif".equals(category)) {
			StructRefSeqDif sref = (StructRefSeqDif) buildObject(StructRefSeqDif.class.getName(), loopFields, lineData,
					loopWarnings);

			triggerNewStrucRefSeqDif(sref);
		} else if ("_struct_site_gen".equals(category)) {
			StructSiteGen sref = (StructSiteGen) buildObject(StructSiteGen.class.getName(), loopFields, lineData,
					loopWarnings);

			triggerNewStructSiteGen(sref);
		} else if ("_struct_site".equals(category)) {
		...
		} else if ("_entity_poly_seq".equals(category)) {
			EntityPolySeq exptl = (EntityPolySeq) buildObject(EntityPolySeq.class.getName(), loopFields, lineData,
					loopWarnings);

			triggerNewEntityPolySeq(exptl);
		} else if ("_entity_src_gen".equals(category)) {
			EntitySrcGen entitySrcGen = (EntitySrcGen) buildObject(EntitySrcGen.class.getName(), loopFields, lineData,
					loopWarnings);
			triggerNewEntitySrcGen(entitySrcGen);
		} else if ("_entity_src_nat".equals(category)) {
			EntitySrcNat entitySrcNat = (EntitySrcNat) buildObject(EntitySrcNat.class.getName(), loopFields, lineData,
					loopWarnings);
			triggerNewEntitySrcNat(entitySrcNat);
		} else if ("_pdbx_entity_src_syn".equals(category)) {
			EntitySrcSyn entitySrcSyn = (EntitySrcSyn) buildObject(EntitySrcSyn.class.getName(), loopFields, lineData,
					loopWarnings);
			triggerNewEntitySrcSyn(entitySrcSyn);
		} else if ("_struct_asym".equals(category)) {
		...
		} else if ("_pdbx_poly_seq_scheme".equals(category)) {
			PdbxPolySeqScheme ppss = (PdbxPolySeqScheme) buildObject(PdbxPolySeqScheme.class.getName(), loopFields,
					lineData, loopWarnings);

			triggerNewPdbxPolySeqScheme(ppss);

		} else if ("_pdbx_nonpoly_scheme".equals(category)) {
			PdbxNonPolyScheme ppss = (PdbxNonPolyScheme) buildObject(PdbxNonPolyScheme.class.getName(), loopFields,
					lineData, loopWarnings);

			triggerNewPdbxNonPolyScheme(ppss);

		} else if ("_pdbx_entity_nonpoly".equals(category)) {
			PdbxEntityNonPoly pen = (PdbxEntityNonPoly) buildObject(PdbxEntityNonPoly.class.getName(), loopFields,
					lineData, loopWarnings);
			triggerNewPdbxEntityNonPoly(pen);
		} else if ("_struct_keywords".equals(category)) {
			StructKeywords kw = (StructKeywords) buildObject(StructKeywords.class.getName(), loopFields, lineData,
					loopWarnings);
			triggerNewStructKeywords(kw);
		} else if ("_refine".equals(category)) {
			Refine r = (Refine) buildObject(Refine.class.getName(), loopFields, lineData, loopWarnings);
			triggerNewRefine(r);
		} else if ("_chem_comp".equals(category)) {
			ChemComp c = (ChemComp) buildObject(ChemComp.class.getName(), loopFields, lineData, loopWarnings);
			triggerNewChemComp(c);
		} else if ("_audit_author".equals(category)) {
		...
		} else if ("_pdbx_chem_comp_descriptor".equals(category)) {
			ChemCompDescriptor ccd = (ChemCompDescriptor) buildObject(ChemCompDescriptor.class.getName(), loopFields,
					lineData, loopWarnings);
			triggerNewChemCompDescriptor(ccd);
		} else if ("_pdbx_struct_oper_list".equals(category)) {

			PdbxStructOperList structOper = (PdbxStructOperList) buildObject(PdbxStructOperList.class.getName(),
					loopFields, lineData, loopWarnings);
			triggerNewPdbxStructOper(structOper);

		} else if ("_pdbx_struct_assembly".equals(category)) {
			PdbxStructAssembly sa = (PdbxStructAssembly) buildObject(PdbxStructAssembly.class.getName(), loopFields,
					lineData, loopWarnings);
			triggerNewPdbxStructAssembly(sa);

		} else if ("_pdbx_struct_assembly_gen".equals(category)) {
			PdbxStructAssemblyGen sa = (PdbxStructAssemblyGen) buildObject(PdbxStructAssemblyGen.class.getName(),
					loopFields, lineData, loopWarnings);
			triggerNewPdbxStructAssemblyGen(sa);
		} else if ("_chem_comp_atom".equals(category)) {
			ChemCompAtom atom = (ChemCompAtom) buildObject(ChemCompAtom.class.getName(), loopFields, lineData,
					loopWarnings);
			triggerNewChemCompAtom(atom);

		} else if ("_chem_comp_bond".equals(category)) {
			ChemCompBond bond = (ChemCompBond) buildObject(ChemCompBond.class.getName(), loopFields, lineData,
					loopWarnings);
			triggerNewChemCompBond(bond);
		} else if ("_pdbx_chem_comp_identifier".equals(category)) {
			PdbxChemCompIdentifier id = (PdbxChemCompIdentifier) buildObject(PdbxChemCompIdentifier.class.getName(),
					loopFields, lineData, loopWarnings);
			triggerNewPdbxChemCompIdentifier(id);
		} else if ("_pdbx_chem_comp_descriptor".equals(category)) {
			PdbxChemCompDescriptor id = (PdbxChemCompDescriptor) buildObject(PdbxChemCompDescriptor.class.getName(),
					loopFields, lineData, loopWarnings);
			triggerNewPdbxChemCompDescriptor(id);
		} else if ("_struct_conn".equals(category)) {
	...
	public void triggerNewPdbxStructOper(PdbxStructOperList structOper) {
		consumers.forEach(c -> c.newPdbxStructOperList(structOper));

	}

	public void triggerNewStructNcsOper(StructNcsOper sNcsOper) {
		consumers.forEach(c -> c.newStructNcsOper(sNcsOper));

	}

	public void triggerNewAtomSites(AtomSites atomSites) {
		consumers.forEach(c -> c.newAtomSites(atomSites));
	}

	/**
	 * Populates a bean object from the
	 * {@link org.biojava.nbio.structure.io.mmcif.model} package, from the data read
	 * from a CIF file. It uses reflection to lookup the field and setter method
	 * names given the category found in the CIF file.
	 * <p>
	 * Due to limitations in variable names in java, not all fields can have names
	 * exactly as defined in the CIF categories. In those cases the {@link CIFLabel}
	 * tag can be used in the field names to give the appropriate name that
	 * corresponds to the CIF category, which is the name that will be then looked
	 * up here. The {@link IgnoreField} tag can also be used to exclude fields from
	 * being looked up.
	 * 
	 * @param className
	 * @param loopFields
	...
	private Object buildObject(String className, List<String> loopFields, List<String> lineData, Set<String> warnings) {
		...
		// these methods get the fields but also looking at the IgnoreField and CIFLabel
		// annotations
		Field[] fields = MMCIFFileTools.getFields(c);
		String[] names = MMCIFFileTools.getFieldNames(fields);

		// let's build a map of all methods so that we can look up the setter methods
		// later
		...
		for (String key : loopFields) {
			...
			// we first start looking up the field which can be annotated with a CIFLabel if
			// they
			// need alternative names (e.g. for field _symmetry.space_group_name_H-M, since
			// hyphen is not allowed in var names in java)
			...
			// note that we can't use the field directly and then call Field.set() because
			// many setters
			// have more functionality than just setting the value (e.g. some setters in
			// ChemComp)

			// building up the setter method name: need to upper case the first letter,
			// leave the rest untouched
			String setterMethodName = new StringBuilder().append("set")
			...
			// now we populate the object with the values by invoking the corresponding
			// setter method,
			// note that all of the mmCif container classes have only one argument (they are
			// beans)
	...
	private void produceWarning(String key, String val, Class<?> c, Set<String> warnings) {
		...
		String warning = new StringBuilder().append("Trying to set field ").append(key).append(" in ")
				.append(c.getName())
				.append(" found in file, but no corresponding field could be found in model class (value:").append(val)
				.append(")").toString();
		String warnkey = new StringBuilder().append(key).append("-").append(c.getName()).toString();
		// Suppress duplicate warnings or attempts to store empty data
		if ("?".equals(val) || ".".equals(val) || (warnings != null && warnings.contains(warnkey))) {
