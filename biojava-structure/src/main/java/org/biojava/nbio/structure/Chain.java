...
/**
 * <p>
 * Defines the interface for a Chain. A Chain corresponds to a Chain in a PDB
 * file. A chain consists out of a list of {@link Group} objects. A Group can
 * either be an {@link AminoAcid}, {@link HetatomImpl Hetatom} or
 * {@link NucleotideImpl Nucleotide}.
 * </p>
 *
 * <p>
 * The BioJava API provides access to both the ATOM and SEQRES records in a PDB
 * file. During parsing of a PDB file it aligns the ATOM and SEQRES groups and
 * joins them. The SEQRES sequence can be accessed via
 * {@link #getSeqResGroups()} and the ATOM groups via {@link #getAtomGroups()}.
 * Groups that have been observed (i.e. they are in the ATOM records) can be
 * detected by {@link Group}.has3D()
 * </p>
 *
 * @author Andreas Prlic
 * @version %I% %G%
...
public interface Chain extends Serializable {
	...
	/**
	 * add a group to the list of ATOM record group of this chain. To add SEQRES
	 * records a more complex alignment between ATOM and SEQRES residues is
	 * required, please see SeqRes2AtomAligner for more details on that.
	 * 
	...
	/**
	 * Return the Group at given position, from within Groups with observed density
	 * in the chain, i.e. those with coordinates in ATOM and HETATMS (including
	 * waters) records.
	 * 
	...
	/**
	 * Return the Group at given position, from within groups in the SEQRES records
	 * of the chain, i.e. the aminoacids/nucleotides in the construct.
	 * 
	 * @param position an int
	 * @return a Group object
	 * @see #getSeqResLength()
	...
	/**
	 * Return all Groups with observed density in the chain, i.e. those with
	 * coordinates in ATOM and HETATMS (including waters) records.
	 *
	...
	/**
	 * Set all Groups with observed density in the chain, i.e. those with
	 * coordinates in ATOM and HETATMs (including waters) records.
	 * 
	 ...
	 * {@link GroupType#AMINOACID}, {@link GroupType#HETATM} or
	 * {@link GroupType#NUCLEOTIDE}. Note that if a standard aminoacid appears as a
	 * HETATM (because it is part of a ligand) then it is still considered as
	 * {@link GroupType#AMINOACID} and not as {@link GroupType#HETATM}.
	 * 
	...
	/**
	 * Get a group by its PDB residue numbering. If the PDB residue number is not
	 * known, throws a StructureException.
	 *
	 ...
	 *
	 * @param pdbresnumStart PDB residue number of start. If null, defaults to the
	 *                       chain start.
	 * @param pdbresnumEnd   PDB residue number of end. If null, defaults to the
	 *                       chain end.
	 * @return Groups in between. or throws a StructureException if either start or
	...
	/**
	 * Get all groups that are located between two PDB residue numbers. In contrast
	 * to getGroupsByPDB this method call ignores if the exact outer groups are not
	 * found. This is useful e.g. when requesting the range of groups as specified
	 * by the DBREF records - these frequently are rather inaccurate.
	 *
	 *
	 * @param pdbresnumStart PDB residue number of start. If null, defaults to the
	 *                       chain start.
	 ...
	 *
	 */
	Group[] getGroupsByPDB(ResidueNumber pdbresnumStart, ResidueNumber pdbresnumEnd, boolean ignoreMissing)
			throws StructureException;

	/**
	 * Returns the number of Groups with observed density in the chain, i.e. those
	 * with coordinates in ATOM and HETATMs (including waters) records
	 *
	 * @return the length
	 * @see #getAtomGroup(int)
	 * @see #getAtomGroups()
	...
	/**
	 * Returns the number of groups in the SEQRES records of the chain, i.e. the
	 * number of aminoacids/nucleotides in the construct
	 *
	 * @return the length
	 * @see #getSeqResGroup(int)
	 * @see #getSeqResGroups()
	...
	/**
	 * If available, returns the internal chain ID that is used in mmCIF files
	 * (asym_id), otherwise null
	 *
	...
	/**
	 * Returns the sequence of amino acids as it has been provided in the ATOM
	 * records. Non-standard residues will be present in the string only if the
	 * property
	 * {@value org.biojava.nbio.structure.io.PDBFileReader#LOAD_CHEM_COMP_PROPERTY}
	 * has been set.
	 * 
	...
	/**
	 * Returns the PDB SEQRES sequence as a one-letter sequence string. Non-standard
	 * residues are represented by an "X".
	 * 
	...
	/**
	 * Returns a list of all groups in SEQRES records of the chain, i.e. the
	 * aminoacids/nucleotides in the construct.
	 * 
	 ...
	 *
	 * @param seqResGroups a List of Group objects that from the SEQRES groups of
	 *                     this chain.
	 * @see #getSeqResGroups()
	 */
	 ...
	 * Will automatically fetch Chemical Component files from the PDB web site, even
	 * if {@link FileParsingParameters#setLoadChemCompInfo(boolean)} has not been
	 * set to true. Otherwise the Ligands could not correctly be identified.
	 * 
	 * @return list of Groups that are ligands
	 * @deprecated since biojava 5.0 this does not apply anymore. Chains contain
	 *             either polymeric groups or non-polymeric groups
	 */
	@Deprecated
	...
	/**
	 * Sets annotated sequence mismatches for this chain. This is based on the
	 * STRUCT_REF_SEQ_DIF mmCif category
	 *
	 * @param seqMisMatches
	 */
	...
	/**
	 * Gets annotated sequence mismatches for this chain. This is based on the
	 * STRUCT_REF_SEQ_DIF mmCif category
	 *
	...
	/**
	 * Returns true if the given chain is composed of non-polymeric (including
	 * water) groups only.
	 *
	 ...
	 * rules:
	 * <li>if the ratio of number of residues of a certain {@link GroupType} to
	 * total non-water residues is above the threshold
	 * {@value #org.biojava.nbio.structure.StructureTools.RATIO_RESIDUES_TO_TOTAL},
	 * then that {@link GroupType} is returned</li>
	 * <li>if there is no {@link GroupType} that is above the threshold then the
	 * {@link GroupType} with most members is chosen, logging it</li>
	 * <p>
	 * See also {@link ChemComp#getPolymerType()} and
	 * {@link ChemComp#getResidueType()} which follow the PDB chemical component
	 * dictionary and provide a much more accurate description of groups and their
	 * linking.
	 * </p>
	 *
	 * @return
