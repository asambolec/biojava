...
public class AfpChainWriter {
	...
	private static final Logger logger = LoggerFactory.getLogger(AfpChainWriter.class);
	 ...
	 *
	 * <p>
	 * Note that if a circular permutation has occured the residue numbers may be
	 * innaccurate.
	 *
	 * @param afpChain
	 * @param ca1
	 ...
	 * @return
	 */
	public static String toFatCatCore(AFPChain afpChain, Atom[] ca1, Atom[] ca2, boolean printLegend,
			boolean longHeader, boolean showHTML, boolean showAlignmentBlock) {
		...
		if (longHeader) {
			...
			txt.append(String.format("Align %s.pdb Length1: %d with %s.pdb Length2: %d", name1, ca1Length, name2,
					ca2Length));
		}
		txt.append(newline);
		...
		// txt.append(String.format("raw-score: %.2f norm.-score: %.2f ", alignScore,
		// normAlignScore));

		if (longHeader) {
			txt.append(String.format(
					"Twists %d ini-len %d ini-rmsd %.2f opt-equ %d opt-rmsd %.2f chain-rmsd %.2f Score %.2f align-len %d gaps %d (%.2f%%)",
					blockNum - 1, totalLenIni, totalRmsdIni, optLength, totalRmsdOpt, chainRmsd, alignScore, alnLength,
			...
			printScoresInLines(afpChain, blockNum, optLength, totalRmsdOpt, alignScore, alnLength, gapLen, identity,
					similarity, txt);
		}

		// txt.append(String.format("P-value %.2e Afp-num %d Identity %.2f%% Similarity
		// %.2f%% norm.-score: %.2f"+newline, probability, afpNum, identity * 100,
		// similarity * 100, normAlignScore));

		if (longHeader) {
			printScore(txt, algorithmName, probability, longHeader);

			txt.append(String.format("Afp-num %d Identity %.2f%% Similarity %.2f%%", afpNum, identity * 100,
					similarity * 100));
			txt.append(newline);
		}
		...
		if (longHeader) {
			...
			for (i = 0; i < blockNum; i++) {
				...
				txt.append(String.format("Block %2d afp %2d score %5.2f rmsd %5.2f gap %d (%.2f%%)", i, blockSize[i],
						blockScore[i], blockRmsd[i], blockGap[i], gap));
				txt.append(newline);
			}
		}
		...
		while ((alnLength - t) > 0) {
			...
			if (showHTML) {
				...
				// <span class=\"qg\">.</span> ... Structurally equivalent, but not similar
				// residues.
				...
				for (int pos = 0; pos < lseq1.length(); pos++) {
					...
					if (cl != ' ') {
						...
						if (showAlignmentBlock && block > -1) {
							a += new StringBuilder().append("<span class=\"alignmentBlock1").append(block).append("\">")
									.append(c1).append("</span>").toString();
							b += new StringBuilder().append("<span class=\"alignmentBlock2").append(block).append("\">")
									.append(c2).append("</span>").toString();
							c += new StringBuilder().append("<span class=\"m\">").append(cl).append("</span>")
									.toString();
						} else {
							a += new StringBuilder().append(getPrefix(c1, c2, 0, block, false).toString()).append(c1)
									.append("</span>").toString();
							b += new StringBuilder().append(getPrefix(c1, c2, 1, block, false).toString()).append(c2)
									.append("</span>").toString();
							c += new StringBuilder().append("<span class=\"m\">").append(cl).append("</span>")
									.toString();
						}
						...
						a += new StringBuilder().append("<span class=\"sm\">").append(c1).append("</span>").toString();
						b += new StringBuilder().append("<span class=\"sm\">").append(c2).append("</span>").toString();
						c += new StringBuilder().append("<span class=\"sm\">").append(cl).append("</span>").toString();

					} else {

						a += new StringBuilder().append("<span class=\"qg\">").append(c1).append("</span>").toString();
						b += new StringBuilder().append("<span class=\"qg\">").append(c2).append("</span>").toString();
						c += new StringBuilder().append("<span class=\"qg\">").append(cl).append("</span>").toString();
			...
			txt.append(newline);
			txt.append(String.format(new StringBuilder().append("Chain 1:%5s %s").append(newline).append("%14s%s")
					.append(newline).append("Chain 2:%5s %s").toString(), pdb1, a, " ", c, pdb2, b));
	...
	public static void printScoresInLines(AFPChain afpChain, int blockNum, int optLength, double totalRmsdOpt,
			double alignScore, int alnLength, int gapLen, double identity, double similarity, StringBuffer txt) {
		if (blockNum - 1 > 0) {
		...
		txt.append(newline);
		txt.append(String.format("Gaps: %d (%.2f%%)", gapLen, (100.0 * gapLen / alnLength)));
		txt.append(newline);
	...
	private static void printScore(StringBuffer txt, String algorithmName, double probability, boolean longHeader) {
		if (algorithmName.equalsIgnoreCase(CeMain.algorithmName)
	 ...
	 * @param ca1
	 * @param ca2
	 * @return a String representation as it is used on the RCSB PDB web site for
	 *         display.
	 */
	...
	public static String toWebSiteDisplay(AFPChain afpChain, Atom[] ca1, Atom[] ca2, boolean showAlignmentBlock) {
		...
		msg = new StringBuilder().append(msg).append(newline)
				.append("     <span class=\"m\">|</span> ... Structurally equivalent and identical residues ")
				.append(newline)
				.append("     <span class=\"sm\">:</span> ... Structurally equivalent and similar residues ")
				.append(newline)
				.append("     <span class=\"qg\">.</span> ... Structurally equivalent, but not similar residues. ")
	...
	public static String toPrettyAlignment(AFPChain afpChain, Atom[] ca1, Atom[] ca2, boolean showHTML,
		...
		txt.append(
				String.format("Align %s.pdb Length1: %d with %s.pdb Length2: %d", name1, ca1Length, name2, ca2Length));
			...
			for (j = 0; j < optLen[i]; j++) {
				...
				if (len == 0) {
					...
					for (k = 0; k < lmax; k++) {
						...
						formatGappedRegion(ca1, ca2, txt, p1, p2, k, p1b, p2b, alnseq1, alnseq2, alnsymb, header1,
								footer1, header2, footer2, block, len, showHTML);
						len++;
				...
				formatAlignedRegion(afpChain, ca1, ca2, p1, p2, alnseq1, alnseq2, alnsymb, header1, footer1, header2,
						footer2, block, len, aligPos, showHTML, showAlignmentBlock);
				// System.out.println(len + " >" + header1.toString() +"< ");
				// System.out.println(len + " >" + header2.toString() +"< ");
				// System.out.println(len + " >" + alnseq1.toString() +"< ");
				// System.out.println(len + " >" + alnsymb.toString() +"< ");
	...
	/**
	 * Prints the alignment in the simplest form: a list of aligned residues. Format
	 * is one line per residue pair, tab delimited:
	 * <ul>
	 * <li>1. PDB number. Includes insertion code</li>
	 * <li>1. Chain.</li>
	 ...
	 * <li>2. Amino Acid.</li>
	 * </ul>
	 * example: <code>152	A	ALA	161S	A	VAL</code>
	 * <p>
	...
	private static void formatGappedRegion(Atom[] ca1, Atom[] ca2, StringBuffer txt, int p1, int p2, int k, int p1b,
			int p2b, StringWriter alnseq1, StringWriter alnseq2, StringWriter alnsymb, StringWriter header1,
			StringWriter footer1, StringWriter header2, StringWriter footer2, StringWriter block, int len,
			boolean formatHTML) {
	...
	private static CharSequence getPrefix(char oneletter1, char oneletter2, int i, int blockNr,
			boolean showAlignmentBlock) {
		...
		if (showAlignmentBlock && blockNr > -1) {
			return new StringBuilder().append("<span class=\"alignmentBlock").append(i + 1).append(blockNr + 1)
					.append("\">").toString();
		}
	...
	private static void formatPosition(int pos1, Atom[] ca, int len, StringWriter header1, StringWriter header2) {
		...
		if ((pos1 % 10 == 0) && (!hasInsertionCode)) {
			...
			if (header1.getBuffer().length() - 1 > linePos) {
				ignoreH1 = true;
				// System.out.println("Ignore h1: " + len + " " + header1.getBuffer().length() +
				// " linePos: " + linePos +" >" + header1.toString() +"<");
			}
			// System.out.println(len + " p1:" + tmp + " = " + pos1 + " " + " " + display +
			// " " + ignoreH1);
			if (!ignoreH1) {
	...
	private static void formatAlignedRegion(AFPChain afpChain, Atom[] ca1, Atom[] ca2, int p1, int p2,
			StringWriter alnseq1, StringWriter alnseq2, StringWriter alnsymb, StringWriter header1,
			StringWriter footer1, StringWriter header2, StringWriter footer2, StringWriter block, int len, int aligPos,
	...
	private static void formatStartingText(int p1, int p2, StringWriter header1, StringWriter header2,
			StringWriter footer1, StringWriter footer2, Atom[] ca1, Atom[] ca2) {
	...
	private static boolean doLenCheck(int len, StringBuffer txt, StringWriter header1, StringWriter header2,
			StringWriter alnseq1, StringWriter alnsymb, StringWriter alnseq2, StringWriter footer1,
			StringWriter footer2, StringWriter block, boolean formatHTML) {
	...
	private static CharSequence getPDBPos(Atom atom) {
		...
		if (g != null) {
			Chain c = g.getChain();
			if (c != null) {
				return new StringBuilder().append(g.getResidueNumber().toString()).append(":").append(c.getName())
						.toString();
	...
	public static String toRotMat(AFPChain afpChain) {
		...
		for (int blockNr = 0; blockNr < blockNum; blockNr++) {
			...
			txt.append(
					String.format(
							new StringBuilder().append("     X").append(blockNr + 1).append(" = (%9.6f)*X")
									.append(origString).append(" + (%9.6f)*Y").append(origString).append(" + (%9.6f)*Z")
									.append(origString).append(" + (%12.6f)").toString(),
							m.get(0, 0), m.get(1, 0), m.get(2, 0), shift.getX()));
			txt.append(newline);
			txt.append(
					String.format(
							new StringBuilder().append("     Y").append(blockNr + 1).append(" = (%9.6f)*X")
									.append(origString).append(" + (%9.6f)*Y").append(origString).append(" + (%9.6f)*Z")
									.append(origString).append(" + (%12.6f)").toString(),
							m.get(0, 1), m.get(1, 1), m.get(2, 1), shift.getY()));
			txt.append(newline);
			txt.append(
					String.format(
							new StringBuilder().append("     Z").append(blockNr + 1).append(" = (%9.6f)*X")
									.append(origString).append(" + (%9.6f)*Y").append(origString).append(" + (%9.6f)*Z")
									.append(origString).append(" + (%12.6f)").toString(),
							m.get(0, 2), m.get(1, 2), m.get(2, 2), shift.getZ()));
			txt.append(newline);
		}
		return txt.toString();
	}
	...
	public static String toCE(AFPChain afpChain, Atom[] ca1, Atom[] ca2) {
		...
		txt.append(newline);
		txt.append(newline);
		txt.append(String.format("Alignment length = %d Rmsd = %.2fA Z-Score = %.1f", optLength, totalRmsdOpt,
				probability));
		txt.append(String.format(" Gaps = %d(%.1f%%) CPU = %d ms. Sequence identities = %.1f%%", gapLen,
				(gapLen * 100.0 / optLength), calculationTime, identity * 100));
		...
		while ((alnLength - t) > 0) {
			...
			/*
			 * txt.append(newline); txt.append(String.format("%14s", " "));
			 * 
			 * for(k = 10; k <= len; k += 10) txt.append("    .    :"); if(k <= len + 5)
			 * txt.append("    .");
			 */
			...
			txt.append(newline);
			txt.append(String.format(
					new StringBuilder().append("Chain 1:%5s %s").append(newline).append("Chain 2:%5s %s").toString(),
					(ap + 1), a, (bp + 1), b));
			txt.append(newline);
			for (k = 0; k < len; k++) {
				if (a.charAt(k) != '-') {
