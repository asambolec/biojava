...
public class CeCPMain extends CeMain {
	...
	/**
	 * version history: 1.5 - Added more parameters to the command line, including
	 * -maxOptRMSD 1.4 - Added DuplicationHint parameter & default to duplicating
	 * the shorter chain 1.3 - Short CPs are now discarded 1.2 - now supports check
	 * AlignmentTools.isSequentialAlignment. XML protocol 1.1 - skipped, (trying to
	 * avoid confusion with jfatcat in all vs. all comparisons) 1.0 - initial
	 * release
	 */
	...
	public static void main(String[] args) throws ConfigurationException {
		CeCPUserArgumentProcessor processor = new CeCPUserArgumentProcessor(); // Responsible for creating a CeCPMain
																				// instance
	 ...
	 * @throws StructureException
	 */
	private AFPChain alignRight(Atom[] ca1, Atom[] ca2, CECPParameters cpparams) throws StructureException {
		...
		// since the process of creating ca2m strips the name info away, set it
		// explicitely
		try {
	 ...
	 * @throws StructureException
	 */
	public static AFPChain postProcessAlignment(AFPChain afpChain, Atom[] ca1, Atom[] ca2m, CECalculator calculator)
			throws StructureException {
	 ...
	 */
	public static AFPChain postProcessAlignment(AFPChain afpChain, Atom[] ca1, Atom[] ca2m, CECalculator calculator,
			CECPParameters param) throws StructureException {
	...
	/**
	 * Takes as input an AFPChain where ca2 has been artificially duplicated. This
	 * raises the possibility that some residues of ca2 will appear in multiple
	 * AFPs. This method filters out duplicates and makes sure that all AFPs are
	 * numbered relative to the original ca2.
	 *
	 * <p>
	 * The current version chooses a CP site such that the length of the alignment
	 * is maximized.
	 *
	 * <p>
	 * This method does <i>not</i> update scores to reflect the filtered alignment.
	 * It <i>does</i> update the RMSD and superposition.
	 *
	 * @param afpChain The alignment between ca1 and ca2-ca2. Blindly assumes that
	 *                 ca2 has been duplicated.
	 * @return A new AFPChain consisting of ca1 to ca2, with each residue in at most
	 *         1 AFP.
	 * @throws StructureException
	 */
	public static AFPChain filterDuplicateAFPs(AFPChain afpChain, CECalculator ceCalc, Atom[] ca1, Atom[] ca2duplicated)
			throws StructureException {
		return filterDuplicateAFPs(afpChain, ceCalc, ca1, ca2duplicated, null);
	}

	public static AFPChain filterDuplicateAFPs(AFPChain afpChain, CECalculator ceCalc, Atom[] ca1, Atom[] ca2duplicated,
			CECPParameters params) throws StructureException {
		...
		if (nStart >= ca2len || cEnd < ca2len) { // no circular permutation
			...
			if (overlapLength <= 0) {
				...
				CutPoint cp = calculateCutPoint(optAln[0][1], nStart, cEnd, overlapLength, alignLen, minCPlength,
						ca2len, firstRes);
		...
		if (alignLen > 0) {
			...
			// superimpose
			Matrix4d trans = SuperPositions.superpose(Calc.atomsToPoints(atoms1), Calc.atomsToPoints(atoms2));
	...
	/**
	 * Finds the alignment index of the residues minCPlength before and after the
	 * duplication.
	 *
	 * @param block       The permuted block being considered, generally
	 *                    optAln[0][1]
	 * @param blockLen    The length of the block (in case extra memory was
	 *                    allocated in block)
	 ...
	 *         <dl>
	 *         <dt>n</dt>
	 *         <dd>Index into <code>block</code> of the residue such that
	 *         <code>minCPlength</code> residues remain to the end of
	 *         <code>ca2len</code>, or -1 if no residue fits that criterium.</dd>
	 *         <dt>mid</dt>
	 *         <dd>Index of the first residue higher than <code>ca2len</code>.</dd>
	 *         <dt>c</dt>
	 *         <dd>Index of <code>minCPlength</code>-th residue after ca2len, or
	 *         ca2len*2 if no residue fits that criterium.</dd>
	 *         </dl>
	 */
	...
	private static CutPoint calculateCutPoint(int[] block, int nStart, int cEnd, int overlapLength, int alignLen,
			int minCPlength, int ca2len, int firstRes) {
		...
		int[] nTermResCount = countNtermResidues(block, nStart, overlapLength);

		// Determine the position with the largest sum of lengths
		int[] cTermResCount = countCtermResidues(block, alignLen, cEnd, overlapLength);
		...
		if (debug) {
			System.out.format(
					"Found a CP at residue %d. Trimming %d aligned residues from %d-%d of block 0 and %d-%d of block 1.\n",
	...
	private static void displayAlignment(AFPChain afpChain, Atom[] ca1, Atom[] ca2) throws ClassNotFoundException,
			NoSuchMethodException, InvocationTargetException, IllegalAccessException, StructureException {
