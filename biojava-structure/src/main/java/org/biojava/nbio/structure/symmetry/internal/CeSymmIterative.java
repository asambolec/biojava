...
public class CeSymmIterative {
	...
	/**
	 * For the iterative algorithm to work properly the refinement and optimization
	 * options should be turned on, because the alignment has to be consistent at
	 * every recursive step.
	 *
	 * @param param CeSymm parameters, make sure they are cloned
	 */
	...
	/**
	 * This method uses iteratively CeSymm to calculate all symmetries in the input
	 * array of atoms and organize them in a multiple alignment of the repeats.
	 *
	 * @param atoms atoms
	 * @return CeSymmResult
	 *
	...
	/**
	 * After all the analysis iterations have finished, the final Result object is
	 * reconstructed using the cumulative alignment graph.
	 *
	 * @param atoms the original structure atoms
	...
	private CeSymmResult reconstructSymmResult(Atom[] atoms) throws StructureException {
		...
		msa.getEnsemble().setStructureIdentifiers(new ArrayList<>());
		...
		ConnectivityInspector<Integer, DefaultEdge> inspector = new ConnectivityInspector<>(alignGraph);
		List<Set<Integer>> comps = inspector.connectedSets();
		List<ResidueGroup> groups = new ArrayList<>(comps.size());
		comps.forEach(comp -> groups.add(new ResidueGroup(comp)));
	 ...
	 * iterations have finished, using the stored MultipleAlignment at each symmetry
	 * level.
	 * 
	 * @return SymmetryAxes
	 */
	private SymmetryAxes recoverAxes(CeSymmResult result) {
		...
		levels.forEach(level -> {
			...
			MultipleAlignment align = level.getMultipleAlignment();
			Matrix4d axis = align.getBlockSet(0).getTransformations().get(1);
			SymmetryType type = level.getAxes().getElementaryAxis(0).getSymmType();
	 ...
	 *
	 * @param atoms Atom array of the repeat found
