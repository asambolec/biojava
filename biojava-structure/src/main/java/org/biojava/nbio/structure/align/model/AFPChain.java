...
/**
 * A bean to contain the core of a structure alignment. The FatCat aligner class
 * is working on the AFPChain class.
 *
...
public class AFPChain implements Serializable, Cloneable {
	...
	private String[][][] pdbAln; // only needed temp. during XML serialization, since we don;t have coordinates
									// loaded at that time and can map from PDB res numbers to atom positions.
	private int[][][] optAln;
	private int[] optLen;
	 ...
	 * analysis methods (scores, display, etc) behave differently if the alignment
	 * is flexible (created with FatCat).
	 *
	 * @param algorithmName
	 */
	...
	public AFPChain(AFPChain o) {
		...
		this.conn = Double.valueOf(o.conn);
		this.dvar = Double.valueOf(o.dvar);
	...
	/**
	 * used temporarily during XML serialization to track the PDB positions of the
	 * alignmnet
	 *
	 * @return String array
	 */
	...
	/**
	 * Get the set of AFPs for this alignment. An AFP is a local ungapped alignment
	 * between the two peptides.
	 *
	 * AFPs are set before the final optimization step. To get the final alignment,
	 * look at the aligned pairs from {@link #getOptAln()}.
	 *
	...
	/**
	 * Set the set of AFPs for this alignment. An AFP is a local ungapped alignment
	 * between the two peptides.
	 *
	 * AFPs are set before the final optimization step. To get the final alignment,
	 * look at the aligned pairs from {@link #getOptAln()}.
	...
	/**
	 * Tracks the Atom positions in the optimal alignment. Note: only considers the
	 * equivalent positions, gaps are ignored... first dimension is the block nr
	 * second dimension is 0 or 1 (the alignment chain index) third is the position
	 * 
	 * @return int array
	 */
	...
	/**
	 * The length of the optimal alignment. Set by AFPOptimizer.optimizeAln(). This
	 * should be the sum of the elements in optLen
	 * 
	 * @param optLength
	 */
	...
	/**
	 * Caution has to be made when changing the algorithmName of an AFPChain, since
	 * downstream analysis methods (scores, display, etc) behave differently if the
	...
	/**
	 * Get whether this alignment has the normal topology, ie the residues aligned
	 * in each block increase sequentially over the original protein.
	 *
	...
	/**
	 * Set whether this alignment has the normal topology, ie the residues aligned
	 * in each block increase sequentially over the original protein.
	 *
	 * This will be false if a circular permutation was detected.
	 */
	...
	/**
	 * A matrix with <i>ca1length</i> rows and <i>ca2length</i> columns. For CE this
	 * is the distance matrix, but the exact interpretation is left up to the
	 * alignment algorithm.
	 *
	 * <p>
	 * Note: A {@link org.biojava.nbio.structure.gui.JMatrixPanel}, which is used in
	...
	public void setDistanceMatrix(Matrix distanceMatrix) {
		...
		// System.out.println("Setting distMatrix "+(distanceMatrix==null?"null":"not
		// null"));
	}
	...
	/**
	 * Returns the tmScore of the alignment. If the score has not been calcualted
	 * yet, returns -1. To calculate it call AFPChainScorer.getTMScore(afpChain,
	 * ca1, ca2);
	 *
	 ...
	 *
	 * Checks if the optAlign is the same, and if the objects being compared seem to
	 * be the same (same names, lengths). Does not check properties of the alignment
	 * such as scores or superposition matrices.
	 * 
