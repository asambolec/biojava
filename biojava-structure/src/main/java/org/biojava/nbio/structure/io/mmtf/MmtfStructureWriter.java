...
public class MmtfStructureWriter {
	...
	/**
	 * Pass data from Biojava structure to another generic output type. Loops
	 * through the data structure and calls all the set functions.
	 * 
	 * @param structure             the input {@link Structure} to write
	 * @param dataTransferInterface the generic interface that implements all the
	 *                              set methods.
	 */
	public MmtfStructureWriter(Structure structure, StructureAdapterInterface dataTransferInterface) {
		...
		// Reset structure to consider altloc groups with the same residue number but
		// different group names as seperate groups
		...
		mmtfDecoderInterface.initStructure(numBonds, allAtoms.size(), MmtfUtils.getNumGroups(structure),
				allChains.size(), structure.nrModels(), structure.getPDBCode());
		...
		mmtfDecoderInterface.setHeaderInfo(pdbHeader.getRfree(), pdbHeader.getRwork(), pdbHeader.getResolution(),
				pdbHeader.getTitle(), MmtfUtils.dateToIsoString(pdbHeader.getDepDate()),
				MmtfUtils.dateToIsoString(pdbHeader.getRelDate()),
				MmtfUtils.techniquesToStringArray(pdbHeader.getExperimentalTechniques()));
		mmtfDecoderInterface.setXtalInfo(MmtfUtils.getSpaceGroupAsString(xtalInfo.getSpaceGroup()),
				MmtfUtils.getUnitCellAsArray(xtalInfo), MmtfUtils.getNcsAsArray(xtalInfo.getNcsOperators()));
		...
		for (int modelIndex = 0; modelIndex < structure.nrModels(); modelIndex++) {
			...
			modelChains.forEach(chain -> {
				...
				groups.forEach(group -> {
					...
					mmtfDecoderInterface.setGroupInfo(group.getPDBName(), group.getResidueNumber().getSeqNum(),
							insCode.charValue(), chemComp.getType().toUpperCase(), atomsInGroup.size(),
							MmtfUtils.getNumBondsInGroup(atomsInGroup), singleLetterCode, sequenceGroups.indexOf(group),
							MmtfUtils.getSecStructType(group));
					atomsInGroup.forEach(atom -> {
						...
						mmtfDecoderInterface.setAtomInfo(atom.getName(), atom.getPDBserial(), altLoc,
								(float) atom.getX(), (float) atom.getY(), (float) atom.getZ(), atom.getOccupancy(),
	 ...
	 */
	private void storeBioassemblyInformation(Map<String, Integer> chainIdToIndexMap,
			Map<Integer, BioAssemblyInfo> inputBioAss) {
		...
		for (Entry<Integer, BioAssemblyInfo> entry : inputBioAss.entrySet()) {
			...
			for (Entry<double[], int[]> transformEntry : transformMap.entrySet()) {
				mmtfDecoderInterface.setBioAssemblyTrans(bioAssemblyIndex, transformEntry.getValue(),
						transformEntry.getKey(), entry.getKey().toString());
			}
			bioAssemblyIndex++;
		}
	}
