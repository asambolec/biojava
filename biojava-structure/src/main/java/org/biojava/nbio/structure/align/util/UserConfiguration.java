...
public class UserConfiguration {
	 ...
	 * <ol>
	 * <li>{@value #PDB_DIR} system property (for instance,
	 * -D{@value #PDB_DIR}=/tmp)</li>
	 * <li>{@value #PDB_DIR} environment variable</li>
	 * <li>System temp directory (java.io.tmpdir property)</li>
	 * </ol>
	 * if the provided path is not a directory then the system's temp directory is
	 * used. A non-writable path is allowed, only a warning will be logged.</li>
	 * <li>default cache location. This is the first specified of:
	 * <ol>
	 * <li>{@value #PDB_CACHE_DIR} system property (for instance,
	 * -D{@value #PDB_CACHE_DIR}=/tmp)</li>
	 * <li>{@value #PDB_CACHE_DIR} environment variable</li>
	 * <li>the value set for {@value #PDB_DIR}</li>
	 * </ol>
	 * if the provided path is not a directory or is not writable then the system's
	 * temp directory is used.</li>
	 * </ul>
	 */
	public UserConfiguration() {
		...
		// note that in initCacheFilePath, we set to the provided one (if readable) or
		// to the same as pdbFilePath
		...
		if (userProvidedDir != null && !userProvidedDir.trim().isEmpty()) {
			...
			if (!f.isDirectory()) {
				...
				logger.warn(
						...
						path, propertyName);
				// we don't require the PDB_DIR to be writable, so that it can be used with a
				// pre-rsynced dir
	...
	private String initCacheFilePath() {
		...
		if (userProvidedDir != null) {
			...
			if (env.containsKey(propertyName)) {
				...
				// as PDB_DIR is not checked for being writable, we have to do that check here
				// in case
				if (new File(pdbFilePath).canWrite()) {
					...
					path = System.getProperty(TMP_DIR);
					logger.warn(
							"Could not read cache dir from system property {} or environment variable {}, "
	...
	/**
	 * convert Configuration to an XML file so it can be serialized add to an
	 * already existing xml file.
	 *
