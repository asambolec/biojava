...
public class StructureAlignmentOptimizer {
	...
	/**
	 * optimize the structural alignment by update the equivalent residues and then
	 * run dynamic programming input: len1 the length of structure 1; c1: the
	 * structure information of 1 len2 the length of structure 2; c2: the structure
	 * information of 2 iniLen and iniSet is the length and list of initial
	 * equivalent residues
	 */

	public StructureAlignmentOptimizer(int b1, int end1, Atom[] c1, int b2, int end2, Atom[] c2, int iniLen,
			int[][] iniSet) throws StructureException {

		// System.err.println("optimizing range:" + b1 + "-" + end1 + "("+ (end1-b1) +
		// ") b2: " + b2 + "-" + end2+ "("+ (end2-b2) + ") iniLen " + iniLen);
		...
		for (int i = 0; i < iniLen; i++) {
			...
			if (iniSet[0][i] > len1 || iniSet[1][i] > len2) {
				throw new RuntimeException(
						String.format("StructureAlignmentOptimizer: focus exceeds the protein 1 or 2 length!"));
			}
		}
	...
	public void runOptimization(int maxi) throws StructureException {
		superimposeBySet();
		if (debug) {
			logger.error("   initial rmsd " + rmsd);
		}

		// if (showAlig)
		// showCurrentAlignment(equLen, equSet, "after initial superimposeBySet Len:"
		// +equLen + " rmsd:" +rmsd);
	 ...
	 * optimization continues until either i)alignment length is less than 95% of
	 * alignment length before optimization ii)rmsd is less than 110% of rmsd at the
	 * cycle when condition i) was first satisfied
	 */
	...
	private void superimposeBySet() throws StructureException {
		...
		for (i = 0; i < equLen; i++) {
			...
			/*
			 * try { System.out.println("before superimpos: " + equSet[0][i]+"-"+
			 * equSet[1][i]+ " dist:" + Calc.getDistance(tmp1[i], cod2[equSet[1][i]])); }
		...
		// transform structure 2 according to the superimposition of the equivalent
		// residues
		Calc.transform(cod2, trans);

		// for(i = 0; i < equLen; i ++) {
		// try {
		// System.err.println("after superimpos: " + equSet[0][i]+"-"+ equSet[1][i]+ "
		// dist:" + Calc.getDistance(tmp1[i], cod2[equSet[1][i]]));
		// } catch (Exception e){
		// e.printStackTrace();
		// }
		// }
	...
	private void optimize(int maxi) throws StructureException {
		...
		if (debug) {
			logger.info(
					new StringBuilder().append("Optimizing up to ").append(maxi).append(" iterations.. ").toString());
		}
		boolean ifstop = true;
		int i;
		int alnLen;
		...
		for (i = 0; i < maxi; i++) {
			...
			if (alnLen < 3) {
				ifstop = true; // very rare, mark by Y.Y on 5/1/03
			} else {
				ifstop = defineEquPos(alnLen, alnList);
			}
			...
			// if (showAlig)
			// if ( i == 0 )
			// showCurrentAlignment(alnLen, alnList, "optimizing alignment - after " + i + "
			// iterations alnLen:" + alnLen + " rmsd " + rmsd);
		}
		...
		if (i < maxi) {
			logger.info(String.format("   optimize converged at %d iterations%n", i));
		} else {
			logger.info("   optimize stop without convergence\n");
		}
		logger.info(new StringBuilder().append("optimization time: ").append(System.currentTimeMillis() - optStart)
				.append(" ms.").toString());
	...
	/**
	 * the equivalent residues: residues where Dij &lt;= Dc and i,j is an aligned
	 * pair use the previous superimposing
	 */
	private boolean defineEquPos(int alnLen, int[][] alnList) throws StructureException {
		...
		equLen = 0;
		// if (debug)
		// System.out.println(String.format(" OPT: Dc %f, equLenOld %d, rmsdOld %f,
		// alnLen %d", Dc, equLenOld, rmsdOld, alnLen));
		for (i = 0; i < alnLen; i++) {
			r1 = alnList[0][i];
			r2 = alnList[1][i];
		...
		// if (debug)
		// System.out.println(String.format(" OPT: new equLen %d rmsd %f", equLen,
		// rmsd));

		boolean ifstop = false;

		// if (debug) {
		// System.out.print(" OPT: rmsd diff: " + Math.abs(rmsd - rmsdOld) + " equLens:
		// " + equLenOld + ":"+ equLen);
