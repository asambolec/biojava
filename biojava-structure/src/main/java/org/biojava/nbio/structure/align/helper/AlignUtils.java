...
public class AlignUtils {
	 ...
	 *
	 * @param caall
	 * @param idx   an array where each element is a position of all the Atoms to
	 *              return
	...
	/**
	 * Get the centroid for the set of atoms starting from position pos, length
	 * fragmentLenght
	 *
	 * @param ca
	 * @param pos
	...
	public static Atom getCenter(Atom[] ca, int pos, int fragmentLength) {
		...
		if (pos + fragmentLength > ca.length) {
			logger.info(new StringBuilder().append("pos (").append(pos).append("), fragL (").append(fragmentLength)
					.append(") > ca.length").append(ca.length).toString());
			return center;
		}
	...
	/**
	 * Given distance matrix diagonals dk1, dk2, get the rmsd of a fpair. i,j is the
	 * fpair start in mol1 and mol2, l is the length of the fragment
	 *
	...
	/**
	 * Matrix of all distances between two sets of Atoms. Does not superimpose or
	 * modify the Atoms.
	 *
	 * @param ca1
	 * @param ca2
