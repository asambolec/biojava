...
public class QuatSymmetryDetector {
	...
	/**
	 * Maximal distance between Calpha atoms of residues of different subunits to
	 * establish a residue contact.
	 */
	private static final double CONTACT_GRAPH_DISTANCE_CUTOFF = 8;
	/**
	 * The minimal number of residue contacts between subunits to consider them
	 * connected and add an edge to the contact graph.
	 */
	...
	/**
	 * Calculate GLOBAL symmetry results. This means that all {@link Subunit} are
	 * included in the symmetry.
	 *
	 * @param structure     protein chains will be extracted as {@link Subunit}
	 * @param symmParams    quaternary symmetry parameters
	 * @param clusterParams subunit clustering parameters
	 * @return GLOBAL quaternary structure symmetry results
	 */
	public static QuatSymmetryResults calcGlobalSymmetry(Structure structure, QuatSymmetryParameters symmParams,
	 ...
	 * included in the symmetry.
	 *
	 * @param subunits      list of {@link Subunit}
	 * @param symmParams    quaternary symmetry parameters
	 * @param clusterParams subunit clustering parameters
	 * @return GLOBAL quaternary structure symmetry results
	 */
	public static QuatSymmetryResults calcGlobalSymmetry(List<Subunit> subunits, QuatSymmetryParameters symmParams,
			SubunitClustererParameters clusterParams) {
	 ...
	 * included in the symmetry.
	 *
	 * @param composition {@link Stoichiometry} object that contains clustering
	 *                    results
	 * @param symmParams  quaternary symmetry parameters
	 * @return GLOBAL quaternary structure symmetry results
	 */
	 ...
	 * {@link SubunitCluster} is left out of the symmetry calculation. Each element
	 * of the List is one possible LOCAL symmetry result.
	 * <p>
	 ...
	 * @param symmParams    quaternary symmetry parameters
	 * @param clusterParams subunit clustering parameters
	 * @return List of LOCAL quaternary structure symmetry results. Empty if none.
	 */
	public static List<QuatSymmetryResults> calcLocalSymmetries(Structure structure, QuatSymmetryParameters symmParams,
			SubunitClustererParameters clusterParams) {
	 ...
	 * @return List of LOCAL quaternary structure symmetry results. Empty if none.
	 */
	public static List<QuatSymmetryResults> calcLocalSymmetries(List<Subunit> subunits,
			QuatSymmetryParameters symmParams, SubunitClustererParameters clusterParams) {
	 ...
	 * subunits (heteromers with just 2 chains cannot have local symmetry)
	 *
	 * @param globalComposition {@link Stoichiometry} object that contains global
	 *                          clustering results
	 * @param symmParams        quaternary symmetry parameters
	 * @return List of LOCAL quaternary structure symmetry results. Empty if none.
	 */

	public static List<QuatSymmetryResults> calcLocalSymmetries(Stoichiometry globalComposition,
		...
		List<SubunitCluster> nontrivialClusters = clusters.stream().filter(cluster -> (cluster.size() > 1))
		...
		// since clusters are rearranged and trimmed, we need a reference to the
		// original data
		// to maintain consistent IDs of clusters and subunits across all solutions
		Map<Integer, List<Integer>> clusterIdToSubunitIds = allSubunitIds.stream()
				.collect(Collectors.groupingBy(allSubunitClusterIds::get, Collectors.toList()));

		List<QuatSymmetryResults> redundantSymmetries = new ArrayList<>();
		// first, find symmetries for single clusters and their groups
		// grouping is done based on symmetries found (i.e., no exhaustive permutation
		// search is performed)
		if (clusters.size() > 1) {

			List<QuatSymmetryResults> clusterSymmetries = calcLocalSymmetriesCluster(nontrivialComposition,
					clusterIdToSubunitIds, symmParams, knownCombinations);
		...
		List<QuatSymmetryResults> graphSymmetries = calcLocalSymmetriesGraph(nontrivialComposition,
				allSubunitClusterIds, clusterIdToSubunitIds, symmParams, knownCombinations, graph);
		...
		List<QuatSymmetryResults> outputSymmetries = redundantSymmetries.stream()
				.filter(a -> redundantSymmetries.stream().noneMatch(b -> a != b && a.isSupersededBy(b)))
				.collect(Collectors.toList());
	...
	private static Graph<Integer, DefaultEdge> initContactGraph(List<SubunitCluster> clusters) {
		...
		List<Point3d[]> clusterSubunitCoords = clusters.stream().flatMap(c -> c.getSubunits().stream())
				.map(r -> Calc.atomsToPoints(r.getRepresentativeAtoms())).collect(Collectors.toList());
	...
	private static List<QuatSymmetryResults> calcLocalSymmetriesCluster(Stoichiometry nontrivialComposition,
			Map<Integer, List<Integer>> clusterIdToSubunitIds, QuatSymmetryParameters symmParams,
		...
		for (int i = 0; i < nontrivialComposition.numberOfComponents(); i++) {
			QuatSymmetryResults localResult = calcQuatSymmetry(nontrivialComposition.getComponent(i), symmParams);
		...
		// group clusters by symmetries found, in case they all share axes and have the
		// same number of subunits
		Map<String, Map<Integer, List<QuatSymmetryResults>>> groupedSymmetries = clusterSymmetries.stream()
				.collect(Collectors.groupingBy(QuatSymmetryResults::getSymmetry,
						Collectors.groupingBy(QuatSymmetryResults::getSubunitCount, Collectors.toList())));
		...
		for (Map<Integer, List<QuatSymmetryResults>> symmetriesByGroup : groupedSymmetries.values()) {
			for (List<QuatSymmetryResults> symmetriesBySubunits : symmetriesByGroup.values()) {
				...
				Stoichiometry groupComposition = symmetriesBySubunits.stream()
						.map(QuatSymmetryResults::getStoichiometry).reduce(Stoichiometry::combineWith).get();
				...
				if (localResult != null && !"C1".equals(localResult.getSymmetry())) {
					...
					groupComposition.getClusters().stream()
							.mapToInt(cluster -> nontrivialComposition.getClusters().indexOf(cluster))
							.forEach(i -> knownResult.addAll(clusterIdToSubunitIds.get(i)));
					// since symmetry is found,
					// do not try graph decomposition of this set of subunits later
					knownCombinations.add(knownResult);
				}
	...
	private static List<QuatSymmetryResults> calcLocalSymmetriesGraph(final Stoichiometry globalComposition,
			final List<Integer> allSubunitClusterIds, final Map<Integer, List<Integer>> clusterIdToSubunitIds,
			QuatSymmetryParameters symmParams, Set<Set<Integer>> knownCombinations, Graph<Integer, DefaultEdge> graph) {
		...
		CliqueMinimalSeparatorDecomposition<Integer, DefaultEdge> cmsd = new CliqueMinimalSeparatorDecomposition<>(
				graph);

		// only consider components with more than 1 vertex (subunit)
		Set<Set<Integer>> graphComponents = cmsd.getAtoms().stream().filter(component -> component.size() > 1)
		...
		for (Set<Integer> graphComponent : graphComponents) {
			...
			Stoichiometry localStoichiometry = trimSubunitClusters(globalComposition, allSubunitClusterIds,
					clusterIdToSubunitIds, usedSubunitIds);
			...
			for (Integer removeSubunitId : usedSubunitIds) {
				...
				List<QuatSymmetryResults> localSubSymmetries = calcLocalSymmetriesGraph(globalComposition,
						allSubunitClusterIds, clusterIdToSubunitIds, symmParams, knownCombinations, subGraph);
	...
	private static Stoichiometry trimSubunitClusters(Stoichiometry globalComposition,
			List<Integer> allSubunitClusterIds, Map<Integer, List<Integer>> clusterIdToSubunitIds,
		...
		Set<Integer> usedClusterIds = usedSubunitIds.stream().map(allSubunitClusterIds::get).distinct()
				.collect(Collectors.toSet());

		// for each used cluster, remove unused subunits
		usedClusterIds.forEach(usedClusterId -> {
			...
			List<Integer> subunitsToRetain = usedSubunitIdsInCluster.stream().map(i -> i - minSUValue)
					.collect(Collectors.toList());
			...
			if (subunitsToRetain.size() > 1) {
				...
				// if the cluster ends up having only 1 subunit, remove it from further
				// processing
		...
		if (symmetry.startsWith("C")) {
			...
			if (helixLayers.size() > 0) {
				...
				if ("C1".equals(symmetry)
						|| (!"C1".equals(symmetry) && deltaRmsd <= parameters.getHelixRmsdThreshold())) {
