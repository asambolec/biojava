...
public abstract class LocalPDBDirectory implements StructureIOFile {
	 ...
	 * Note that we don't support file stamp retrieving for ftp protocol, thus some
	 * of the fetch modes will not work properly with ftp protocol
	 */
	...
	/**
	 * The server name, prefixed by the protocol string (http:// or ftp://). Note
	 * that we don't support file stamp retrieving for ftp protocol, thus some of
	 * the fetch modes will not work properly with ftp protocol
	 */
	...
	private String splitDirURL; // path on the server, starting with a slash and ending before the 2-char split
								// directories
	...
	/**
	 * Subclasses should provide default and single-string constructors. They should
	 * use {@link #addExtension(String)} to add one or more extensions.
	 *
	 * <p>
	...
	/**
	 * define supported file extensions compressed extensions .Z,.gz do not need to
	 * be specified they are dealt with automatically.
	 */
	@Override
	...
	/**
	 * Set the behavior for fetching files from the server. This replaces the
	 * {@link #setAutoFetch(boolean)} method with a more extensive set of options.
	 * 
	 * @param fetchBehavior
	 */
	...
	/**
	 * Load or download the specified structure and return it as an InputStream for
	 * direct parsing.
	 * 
	 * @param pdbId
	 * @return
	...
	protected InputStream getInputStream(String pdbId) throws IOException {
		...
		if (!file.exists()) {
			throw new IOException(new StringBuilder().append("Structure ").append(pdbId)
					.append(" not found and unable to download.").toString());
		}
	...
	protected File downloadStructure(String pdbId) throws IOException {
		...
		case LOCAL_ONLY:
			if (existing == null) {
				throw new IOException(String.format("Structure %s not found in %s " + "and configured not to download.",
						pdbId, getPath()));
			} else {
				return existing;
			}
		case FETCH_FILES:
			...
			// existing is null, downloadStructure(String,String,boolean,File) will download
			// it
			break;
		case FETCH_IF_OUTDATED:
			...
			// download it
			// existing != null : downloadStructure(String,String,boolean,File) will check
			// its date and download if older
			break;
		case FETCH_REMEDIATED:
			...
			if (existing != null) {
				...
				if (lastModified < LAST_REMEDIATION_DATE) {
					...
					logger.warn("Replacing file {} with latest remediated (remediation of {}) file from PDB.", existing,
							LAST_REMEDIATION_DATE_STRING);
					existing = null;
					break;
				} else {
					return existing;
	 ...
	 * @param obsolete     Whether or not file should be saved to the obsolete
	 *                     location locally
	 * @param existingFile if not null and checkServerFileDate is true, the last
	 *                     modified date of the server file and this file will be
	 *                     compared to decide whether to download or not
	 * @return
	 * @throws IOException
	 */
	private File downloadStructure(String pdbId, String pathOnServer, boolean obsolete, File existingFile)
		...
		if (getFilename(pdbId).endsWith(".mmtf.gz")) {
			...
			ftp = String.format("%s%s/%s/%s", serverName, pathOnServer, pdbId.substring(1, 3).toLowerCase(),
					getFilename(pdbId));
		}
		...
		if (existingFile != null) {
			...
			if (serverFileDate != null) {
				if (existingFile.lastModified() >= serverFileDate.getTime()) {
					...
					logger.warn("File {} is outdated, will download new one from PDB (updated on {})", existingFile,
							serverFileDate.toString());
				}
			} else {
				logger.warn(
						"Could not determine if file {} is outdated (could not get timestamp from server). Will force redownload",
						existingFile);
			}
		}
		...
		// Commented out following code used for setting the modified date to the
		// downloaded file - JD 2015-01-15
		// The only reason to have it was in order to get an rsync-like behavior,
		// respecting the timestamps
		// but the issue is that it would make the FETCH_REMEDIATED mode redownload
		// files with timestamps before
		// the remediation.
		// if (serverFileDate==null)
		// serverFileDate = getLastModifiedTime(url);
		//
		// if (serverFileDate!=null) {
		// logger.debug("Setting modified time of downloaded file {} to
		// {}",realFile,serverFileDate.toString());
		// realFile.setLastModified(serverFileDate.getTime());
		// } else {
		// logger.warn("Unknown modified time of file {}, will set its modified time to
		// now.", ftp);
		// }
	...
	/**
	 * Get the last modified time of the file in given url by retrieveing the
	 * "Last-Modified" header. Note that this only works for http URLs
	 * 
	 * @param url
	 * @return the last modified date or null if it couldn't be retrieved (in that
	 *         case a warning will be logged)
	 */
	private Date getLastModifiedTime(URL url) {

		// see
		// http://stackoverflow.com/questions/2416872/how-do-you-obtain-modified-date-from-a-remote-file-java
		Date date = null;
		try {
			...
			if (lastModified != null) {
				...
				try {
					...
					logger.warn(
							"Could not parse last modified time from string '{}', no last modified time available for file {}",
			...
			for (String prefix : prefixes) {
				for (String ex : getExtensions()) {
					File f = new File(searchdir,
							new StringBuilder().append(prefix).append(pdbId.toLowerCase()).append(ex).toString());
					if (f.exists()) {
						return f;
					}
				}
	 ...
	 * String (http:// or ftp://). The server name will be by default the value
	 * {@value #DEFAULT_PDB_FILE_SERVER} or the one read from system property
	 * {@value #PDB_FILE_SERVER_PROPERTY}
	 *
	...
	public static String getServerName() {
		...
		if (name == null || name.trim().isEmpty()) {
			...
			if (!name.startsWith("http://") && !name.startsWith("ftp://") && !name.startsWith("https://")) {
				logger.warn(
						"Server name {} read from system property {} does not have a leading protocol string. Adding http:// to it",
						name, PDB_FILE_SERVER_PROPERTY);
				name = "http://" + name;
			}
	...
	/**
	 * Should be called whenever any of the path variables change. Thus, if
	 * {@link getSplitDirPath()} or {@link getObsoleteDirPath()} depend on anything,
	 * they should call this function when that thing changes (possibly including at
	...
	protected void initPaths() {
		...
		for (String aSplit : split) {
			splitURL.append("/");
			splitURL.append(aSplit);
		}
		StringBuilder obsoleteURL = new StringBuilder("/pub/pdb");
		for (String anObsolete : obsolete) {
			obsoleteURL.append("/");
			obsoleteURL.append(anObsolete);
		}
		...
		splitDirPath = path;
		for (String aSplit : split) {
			splitDirPath = new File(splitDirPath, aSplit);
		}
		obsoleteDirPath = path;
		for (String anObsolete : obsolete) {
			obsoleteDirPath = new File(obsoleteDirPath, anObsolete);
		}
	}
	...
	/**
	 * Location of split files within the directory, as an array of paths. These
	 * will be joined with either slashes (for the URL) or the file separator (for
	 * directories). The returned results should be constant, to allow for caching.
	 * 
	 * @return A list of directories, relative to the /pub/pdb directory on the
	 *         server
	 */
	protected abstract String[] getSplitDirPath();

	/**
	 * Location of obsolete files within the directory, as an array of paths. These
	 * will be joined with either slashes (for the URL) or the file separator (for
	 * directories). The returned results should be constant, to allow for caching.
	 * 
	 * @return A list of directories, relative to the /pub/pdb directory on the
	...
	public static enum FetchBehavior {
		...
		FETCH_FILES,
		/**
		 * Fetch missing files from the server, also fetch if file present but older
		 * than the server file. This requires always querying the server for the last
		 * modified time of the file, thus it adds an overhead to getting files from
		 * cache.
		 */
		FETCH_IF_OUTDATED,
		/**
		 * Fetch missing files from the server. Also force the download of files older
		 * than {@value #LAST_REMEDIATION_DATE_STRING}.
		 */
		FETCH_REMEDIATED,
