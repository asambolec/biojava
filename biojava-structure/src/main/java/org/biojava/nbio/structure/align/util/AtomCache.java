...
/**
 * A utility class that provides easy access to Structure objects. If you are
 * running a script that is frequently re-using the same PDB structures, the
 * AtomCache keeps an in-memory cache of the files for quicker access. The cache
 * is a soft-cache, this means it won't cause out of memory exceptions, but
 * garbage collects the data if the Java virtual machine needs to free up space.
 * The AtomCache is thread-safe.
 *
...
public class AtomCache {
	...
	/**
	 * The default output bioassembly style: if true the bioassemblies are
	 * multimodel, if false the bioassemblies are flat with renamed chains for
	 * symmetry-partners.
	 */
	 ...
	 *
	 * Usually stores files in a temp directory, but this can be overriden by
	 * setting the PDB_DIR variable at runtime.
	 *
	...
	/**
	 * Creates an instance of an AtomCache that is pointed to the a particular path
	 * in the file system. It will use the same value for pdbFilePath and cachePath.
	 *
	 * @param pdbFilePath a directory in the file system to use as a location to
	 *                    cache files.
	 */
	...
	/**
	 * Returns the CA atoms for the provided name. See {@link #getStructure(String)}
	 * for supported naming conventions.
	 * <p>
	 * This method only works with protein chains. Use
	 * {@link #getRepresentativeAtoms(String)} for a more general solution.
	 * 
	 * @param name
	...
	/**
	 * Returns the representative atoms for the provided name. See
	 * {@link #getStructure(String)} for supported naming conventions.
	 *
	 * @param name
	 * @return an array of Atoms.
	 * @throws IOException
	...
	/**
	 * Returns the biological assembly for a given PDB ID and bioAssemblyId, by
	 * building the assembly from the biounit annotations found in
	 * {@link Structure#getPDBHeader()}
	 * <p>
	 * Note, the number of available biological unit files varies. Many entries
	 * don't have a biological assembly specified (e.g. NMR structures), many
	 * entries have only one biological assembly (bioAssemblyId=1), and some
	 * structures have multiple biological assemblies.
	 *
	 * @param pdbId         the PDB ID
	 * @param bioAssemblyId the 1-based index of the biological assembly (0 gets the
	 *                      asymmetric unit)
	 * @param multiModel    if true the output Structure will be a multi-model one
	 *                      with one transformId per model, if false the
	 *                      outputStructure will be as the original with added
	 *                      chains with renamed asymIds (in the form
	 *                      originalAsymId_transformId and
	 *                      originalAuthId_transformId).
	 * @return a structure object
	 * @throws IOException
	 * @throws StructureException if biassemblyId < 0 or other problems while
	 *                            loading structure
	 * @author Peter Rose
	 * @since 3.2
	 */
	public Structure getBiologicalAssembly(String pdbId, int bioAssemblyId, boolean multiModel)
		...
		if (bioAssemblyId < 0) {
			throw new StructureException(new StringBuilder().append("bioAssemblyID must be nonnegative: ").append(pdbId)
					.append(" bioAssemblyId ").append(bioAssemblyId).toString());
		}
		...
		if (asymUnit.getPDBHeader() == null || asymUnit.getPDBHeader().getBioAssemblies() == null) {
			logger.info("No bioassembly information found for {}, returning asymmetric unit as biological assembly",
					pdbId);
			return asymUnit;
		}
		...
		// 0 ... asym unit
		if (bioAssemblyId == 0) {
			logger.info(new StringBuilder().append("Requested biological assembly 0 for PDB id ").append(pdbId)
					.append(", returning asymmetric unit").toString());
			return asymUnit;
		}
		// does it exist?
		if (!asymUnit.getPDBHeader().getBioAssemblies().containsKey(bioAssemblyId)) {
			throw new StructureException(
					new StringBuilder().append("No biological assembly available for biological assembly id ")
							.append(bioAssemblyId).append(" of ").append(pdbId).toString());
		}

		List<BiologicalAssemblyTransformation> transformations = asymUnit.getPDBHeader().getBioAssemblies()
				.get(bioAssemblyId).getTransforms();

		if (transformations == null || transformations.size() == 0) {

			throw new StructureException(
					new StringBuilder().append("Could not load transformations to recreate biological assembly id ")
							.append(bioAssemblyId).append(" of ").append(pdbId).toString());
	...
	/**
	 * Returns the default biological unit (bioassemblyId=1, known in PDB as
	 * pdb1.gz). If it is not available, the asymmetric unit will be returned, e.g.
	 * for NMR structures.
	 *
	 * <p>
	 ...
	 * 
	 * @param pdbId      the PDB id
	 * @param multiModel if true the output Structure will be a multi-model one with
	 *                   one transformId per model, if false the outputStructure
	 *                   will be as the original with added chains with renamed
	 *                   asymIds (in the form originalAsymId_transformId and
	 *                   originalAuthId_transformId).
	 * @return a structure object
	 * @throws IOException
	 * @throws StructureException
	 ...
	 * @since 5.0
	 */
	public List<Structure> getBiologicalAssemblies(String pdbId, boolean multiModel)
			throws StructureException, IOException {
		...
		if (asymUnit.getPDBHeader() == null || asymUnit.getPDBHeader().getBioAssemblies() == null) {
			logger.info(
					"No bioassembly information found for {}, returning asymmetric unit as the only biological assembly",
					pdbId);
		...
		for (int bioAssemblyId : asymUnit.getPDBHeader().getBioAssemblies().keySet()) {
			...
			if (transformations == null || transformations.size() == 0) {
				...
				logger.info(
						new StringBuilder().append("Could not load transformations to recreate biological assembly id ")
								.append(bioAssemblyId).append(" of ").append(pdbId)
								.append(". Assembly id will be missing in biological assemblies.").toString());
				continue;
			}
	...
	/**
	 * Returns the path that contains the caching file for utility data, such as
	 * domain definitions.
	 *
	 * @return
	 */
	 ...
	 *
	 * <ul>
	 * <li>If only a PDB code is provided, the whole structure will be return
	 * including ligands, but the first model only (for NMR).
	 * <li>Chain IDs are case sensitive, PDB ids are not. To specify a particular
	 * chain write as: 4hhb.A or 4HHB.A</li>
	 * <li>To specify a SCOP domain write a scopId e.g. d2bq6a1. Some flexibility
	 * can be allowed in SCOP domain names, see {@link #setStrictSCOP(boolean)}</li>
	 ...
	 * 
	 * @param name
	 * @return a Structure object, or null if name appears improperly formated (eg
	 *         too short, etc)
	 * @throws IOException        The PDB file cannot be cached due to IO errors
	 * @throws StructureException The name appeared valid but did not correspond to
	 *                            a structure. Also thrown by some submethods upon
	...
	/**
	 * Returns the representation of a {@link ScopDomain} as a BioJava
	 * {@link Structure} object.
	 *
	 ...
	 * @param domain               a SCOP domain
	 * @param scopDatabase         A {@link ScopDatabase} to use
	 * @param strictLigandHandling If set to false, hetero-atoms are included if and
	 *                             only if they belong to a chain to which the SCOP
	 *                             domain belongs; if set to true, hetero-atoms are
	 *                             included if and only if they are strictly within
	 *                             the definition (residue numbers) of the SCOP
	 *                             domain
	 * @return a Structure object
	 * @throws IOException
	 * @throws StructureException
	...
	public Structure getStructureForDomain(ScopDomain domain, ScopDatabase scopDatabase, boolean strictLigandHandling)
		...
		// because ligands sometimes occur after TER records in PDB files, we may need
		// to add some ligands back in
	...
	/**
	 * Send a signal to the cache that the system is shutting down. Notifies
	 * underlying SerializableCache instances to flush themselves...
	 */
	...
	/**
	 * <b>[Optional]</b> This method changes the behavior when obsolete entries are
	 * requested. Current behaviors are:
	 * <ul>
	 * <li>{@link ObsoleteBehavior#THROW_EXCEPTION THROW_EXCEPTION} Throw a
	 * {@link StructureException} (the default)
	 * <li>{@link ObsoleteBehavior#FETCH_OBSOLETE FETCH_OBSOLETE} Load the requested
	 * ID from the PDB's obsolete repository
	 * <li>{@link ObsoleteBehavior#FETCH_CURRENT FETCH_CURRENT} Load the most recent
	 * version of the requested structure
	 *
	 * <p>
	 * This setting may be silently ignored by implementations which do not have
	 * access to the server to determine whether an entry is obsolete, such as if
	 * {@link #isAutoFetch()} is false. Note that an obsolete entry may still be
	...
	/**
	 * Returns how this instance deals with obsolete entries. Note that this setting
	 * may be ignored by some implementations or in some situations, such as when
	 * {@link #isAutoFetch()} is false.
	 *
	 * <p>
	...
	/**
	 * Returns a {@link Structure} corresponding to the CATH identifier supplied in
	 * {@code structureName}, using the the {@link CathDatabase} at
	 * {@link CathFactory#getCathDatabase()}.
	 */
	...
	/**
	 * Returns a {@link Structure} corresponding to the CATH identifier supplied in
	 * {@code structureName}, using the specified {@link CathDatabase}.
	 */
	public Structure getStructureForCathDomain(StructureName structureName, CathDatabase cathInstall)
		...
		origChains.stream().map(Chain::getAtomGroups).flatMap(List::stream)
				.filter(g -> !newChain.getAtomGroups().contains(g)).forEach(newChain::addGroup);
