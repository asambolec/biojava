 ...
 * Service Interface</a>. A factory for {@link RCSBDescription RCSBDescriptions}
 * from {@code describeMol} XML files. The factory methods will return null if
 * the data was not found (rather than throwing an exception); client code
 * should test for this. This is for consistency: if the factory could not read
 * some part (corresponding to a field in a class in {@code rcsb.descriptions})
 * of the XML file, either because it was blank or contained an error that could
...
public class RCSBDescriptionFactory {
	...
	/**
	 * @return An {@link RCSBDescription} from the XML file loaded as
	 *         {@code stream}. Prefer calling {@link #get(String)} if you want data
	...
	public static RCSBDescription get(InputStream stream) {
		...
		for (int i = 0; i < data.getLength(); i++) {
			...
			if ("polymer".equals(polymerE.getNodeName())) {
	...
	private static RCSBMacromolecule makeMolecule(Element moleculeE) {
		...
		for (int i = 0; i < data.getLength(); i++) {
			...
			if ("accession".equals(element.getNodeName())) {
	...
	private static RCSBPolymer makePolymer(Element polymerE) {
		...
		for (int i = 0; i < data.getLength(); i++) {
			...
			} else if ("Taxonomy".equals(element.getNodeName())) {
			...
			} else if ("macroMolecule".equals(element.getNodeName())) {
				RCSBMacromolecule molecule = makeMolecule(element);
				polymer.setMolecule(molecule);
			} else if ("polymerDescription".equals(element.getNodeName())) {
				polymer.setDescription(element.getAttribute("description"));
			} else if ("enzClass".equals(element.getNodeName())) {
				polymer.setEnzClass(element.getAttribute("ec"));
			} else if ("synonym".equals(element.getNodeName())) {
