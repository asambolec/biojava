...
public class ClusterDomains {
	...
	public static List<Domain> cluster(List<Domain> domains, PDPDistanceMatrix pdpDistMatrix) {
		...
		do {
			for (int i = 0; i < ndom - 1; i++) {
				for (int j = i + 1; j < ndom; j++) {
					...
					logger.info(new StringBuilder().append(" pos: d1:").append(i).append(" vs d2:").append(j)
							.append(" d1:").append(d1.getSegmentAtPos(0).getFrom()).append("-")
							.append(d1.getSegmentAtPos(0).getTo()).append(" ").append(d2.getSegmentAtPos(0).getFrom())
							.append("-").append(d2.getSegmentAtPos(0).getTo()).append(" ").append(total_contacts)
							.toString());
					...
					minDomSize = Math.min(Math.pow(minDomSize, 1.6 / 3) + PDPParameters.RG1,
							Math.pow(minDomSize, 1.4 / 3) + Math.pow(PDPParameters.TD1, 1.6 / 3) + PDPParameters.RG1);
					maxDomSize = Math.min(Math.pow(maxDomSize, 1.6 / 3) + PDPParameters.RG1,
							Math.pow(maxDomSize, 1.4 / 3) + Math.pow(PDPParameters.TD1, 1.6 / 3) + PDPParameters.RG1);

					/*
					 * total_max_contacts = 10.+ (long)(
					 * (10.*(size1)/(size1+10.)+(size1)*pow((double)(size1),(double)(2./3.))/(size1+
					 * 10.))*(10.*(size2)/(size2+10.)+(size2)*pow((double)(size2),(double)(2./3.))/(
					 * size2+10.))); total_max_contacts = (min(200,size1))*(min(200,size2))/4;
					 */
					/*
					...
					if (size1 > 130) {
						total_max_contacts = Double.valueOf(minDomSize * maxDomSize * 9).longValue();
						/*
					...
					if (verbose) {
						logger.info(String.format(
								" size1=%d size2=%d minDomSize=%5.2f maxDomSize=%5.2f total_contacts = %d ", size1,
								size2, minDomSize, maxDomSize, total_contacts));
					}
					if (verbose) {
						logger.info(String.format(" total_contacts = %d total_max_contacts = %d", total_contacts,
								total_max_contacts));
					}
					if (verbose) {
						logger.info(String.format(" maximum_value = %f S_value = %f%n", maximum_value, S_value));
					}
			...
			if (verbose) {
				logger.info(new StringBuilder().append("Check for combining: ").append(maximum_value).append(" 1 :")
						.append(PDPParameters.CUT_OFF_VALUE1).toString());
				logger.info(new StringBuilder().append("                     ").append(maximum_valuem).append(" 1M:")
						.append(PDPParameters.CUT_OFF_VALUE1M).toString());
				logger.info(new StringBuilder().append("                     ").append(maximum_values).append(" 1S:")
						.append(PDPParameters.CUT_OFF_VALUE1S).toString());
			}
			...
			if (maximum_value > PDPParameters.CUT_OFF_VALUE1) {
				 ...
				 */
				if (verbose) {
					logger.info(" Criteria 1 matched");
				}
				if (verbose) {
					logger.info(String.format(" maximum_value = %f", maximum_value));
				}
				if (verbose) {
					logger.info(String.format(" Si = %d Sj = %d ", Si, Sj));
				}
				 ...
				 */
				if (verbose) {
					logger.info(String.format(" Listing the domains after combining..."));
				}
				if (verbose) {
					listdomains(domains);
				}
				 ...
				 */
				if (verbose) {
					logger.info(" Criteria 2 matched");
				}
				if (verbose) {
					logger.info(String.format(" maximum_values = %f", maximum_valuem));
				}
				if (verbose) {
					logger.info(String.format(" Sim = %d Sjm = %d", Sim, Sjm));
				}
				 ...
				 */
				if (verbose) {
					logger.info(" Criteria 3 matched");
				}
				if (verbose) {
					logger.info(String.format(" maximum_values = %f", maximum_values));
				}
				if (verbose) {
					logger.info(String.format(" Sis = %d Sjs = %d", Sis, Sjs));
				}
				...
				if (verbose) {
					logger.info(
							String.format(new StringBuilder().append(" Maximum value is less than cut off value. (max:")
									.append(maximum_value).append(")").toString()));
				}
				maximum_value = -1.0;
				maximum_values = -1.0;
				maximum_valuem = -1.0;
		...
		if (verbose) {
			logger.info(String.format(" The domains are:"));
		}
		if (verbose) {
			listdomains(domains);
		}
	...
	private static long getTotalContacts(List<Domain> domains, PDPDistanceMatrix pdpDistMatrix, Domain i, Domain j) {
		...
		for (int k = 0; k < i.nseg; k++) {
			for (int l = 0; l < j.nseg; l++) {
				long contacts = calc_S(j.getSegmentAtPos(l).getFrom(), j.getSegmentAtPos(l).getTo(),
						i.getSegmentAtPos(k).getFrom(), i.getSegmentAtPos(k).getTo(), pdpDistMatrix);
	...
	private static List<Domain> combine(List<Domain> domains, int Si, int Sj, double maximum_value) {
		...
		if (verbose) {
			logger.info(new StringBuilder().append("  +++  combining domains ").append(Si).append(" ").append(Sj)
					.toString());
		}
	...
	private static final void listdomains(List<Domain> domains) {
		...
		for (Domain dom : domains) {
			i++;
			logger.info(new StringBuilder().append("DOMAIN:").append(i).append(" size:").append(dom.size).append(" ")
					.append(dom.score).toString());
			List<Segment> segments = dom.getSegments();

			segments.forEach(s -> logger.info("   Segment: " + s));
		}
	}
}
