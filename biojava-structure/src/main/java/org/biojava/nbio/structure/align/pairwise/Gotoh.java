			...
			for (int colCounter = 1; colCounter < colDim; colCounter++) {
				...
				if (currentGap.getValue() > currentCell.getValue()) {
					if (currentGap.getIndex() >= rowDim) {
						logger.error(new StringBuilder().append("col gap at").append(rowCounter).append(" ")
								.append(colCounter).append(" to ").append(currentGap.getIndex()).toString());
					}
				...
				if (currentGap.getValue() > currentCell.getValue()) {
					if (currentGap.getIndex() >= colDim) {
						logger.error(new StringBuilder().append("row gap at").append(rowCounter).append(" ")
								.append(colCounter).append(" to ").append(currentGap.getIndex()).toString());
					}
					currentCell.setValue(currentGap.getValue());
		...
		while ((backId[n - 1].getRow() >= 1) && (backId[n - 1].getCol() >= 1)) {
			...
			try {
				...
				try {
					...
					for (int f = 0; f < n; f++) {
						logger.info(String.valueOf(backId[f]));
					}
				...
				if (el == null) {
					logger.info(
							new StringBuilder().append("el = null! x:").append(x).append(" y ").append(y).toString());
				}
				backId[n] = el;
			} catch (Exception e) {
				logger.error(e.getMessage(), e);
				logger.info("x " + x);
				logger.info("y " + y);
				logger.info(String.valueOf(backId[n - 2]));
				System.exit(0);
			}
