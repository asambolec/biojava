...
public class ProteinComplexSignature {
	...
	public String getComplexSignature() {
		...
		chainSignatures.forEach(s -> builder.append(s.toString()));
		return builder.toString();
	}

	public String getCompositionId(String chainId) {
		return chainSignatures.stream().filter(s -> s.getChainIds().contains(chainId)).findFirst()
				.map(ChainSignature::getCompositionId).orElse("");
	}

	public String getComplexStoichiometry() {
		StringBuilder s = new StringBuilder();
		chainSignatures.forEach(c -> {
	...
	private List<ChainSignature> getChainSignatures() {
		...
		mapCounts.entrySet().forEach(entry -> {
