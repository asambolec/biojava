...
/**
 * A grid to be used for calculating atom contacts through a spatial hashing
 * algorithm.
 * <p>
 * The grid is composed of cells of size of the cutoff so that the distances
 * that need to be calculated are reduced to those within each cell and to the
 * neighbouring cells.
 * <p>
...
public class Grid {
	...
	/**
	 * The scale: we use units of hundredths of Angstroms (thus cutoffs can be
	 * specified with a maximum precision of 0.01A)
	 */
	...
	/**
	 * Creates a <code>Grid</code>, the cutoff is in the same units as the
	 * coordinates (Angstroms if they are atom coordinates) and can be specified to
	 * a precision of 0.01.
	 * 
	 * @param cutoff
	 */
	 ...
	 * Subsequent call to {@link #getIndicesContacts()} or
	 * {@link #getAtomContacts()} will produce the interatomic contacts.
	 * 
	 * @param iAtoms
	 * @param jAtoms
	 */
	...
	/**
	 * Adds the i and j atoms and fills the grid, passing their bounds (array of
	 * size 6 with x,y,z minima and x,y,z maxima) This way the bounds don't need to
	 * be recomputed. Subsequent call to {@link #getIndicesContacts()} or
	 * {@link #getAtomContacts()} will produce the interatomic contacts.
	 * 
	 * @param iAtoms
	 * @param icoordbounds
	...
	/**
	 * Adds a set of atoms, subsequent call to {@link #getIndicesContacts()} or
	 * {@link #getAtomContacts()} will produce the interatomic contacts. The
	 * bounding box of the atoms will be computed based on input array
	 * 
	 * @param atoms
	 */
	...
	/**
	 * Adds a set of atoms, subsequent call to {@link #getIndicesContacts()} or
	 * {@link #getAtomContacts()} will produce the interatomic contacts. The bounds
	 * calculated elsewhere can be passed, or if null they are computed.
	 * 
	 * @param atoms
	 * @param bounds
	 */
	...
	/**
	 * Adds the i and j coordinates and fills the grid. Their bounds will be
	 * computed. Subsequent call to {@link #getIndicesContacts()} will produce the
	 * contacts, i.e. the set of points within distance cutoff.
	 * 
	 * Subsequent calls to method {@link #getAtomContacts()} will produce a
	 * NullPointerException since this only adds coordinates and no atom
	 * information.
	 * 
	 * @param iAtoms
	 * @param jAtoms
	...
	/**
	 * Adds the i and j coordinates and fills the grid, passing their bounds (array
	 * of size 6 with x,y,z minima and x,y,z maxima) This way the bounds don't need
	 * to be recomputed. Subsequent call to {@link #getIndicesContacts()} will
	 * produce the contacts, i.e. the set of points within distance cutoff.
	 * 
	 * Subsequent calls to method {@link #getAtomContacts()} will produce a
	 * NullPointerException since this only adds coordinates and no atom
	 * information.
	...
	/**
	 * Adds a set of coordinates, subsequent call to {@link #getIndicesContacts()}
	 * will produce the contacts, i.e. the set of points within distance cutoff. The
	 * bounding box of the atoms will be computed based on input array. Subsequent
	 * calls to method {@link #getAtomContacts()} will produce a
	 * NullPointerException since this only adds coordinates and no atom
	 * information.
	 * 
	 * @param atoms
	 ...
	 * Adds a set of coordinates, subsequent call to {@link #getIndicesContacts()}
	 * will produce the contacts, i.e. the set of points within distance cutoff. The
	 * bounds calculated elsewhere can be passed, or if null they are computed.
	 * Subsequent calls to method {@link #getAtomContacts()} will produce a
	 * NullPointerException since this only adds coordinates and no atom
	 * information.
	 * 
	...
	/**
	 * Creates the grid based on the boundaries defined by all atoms given (iAtoms
	 * and jAtoms) and places the atoms in their corresponding grid cells. Checks
	 * also if the i and j grid overlap, i.e. the enclosing bounds of the 2 grids (i
	 * and j) are no more than one cell size apart. If they don't overlap then they
	 * are too far apart so there's nothing to calculate, we set the noOverlap flag
	 * and then {@link #getIndicesContacts()} will do no calculation at all.
	 */
	private void fillGrid() {
		...
		cells = new GridCell[1 + (bounds[3] - bounds[0]) / cellSize][1 + (bounds[4] - bounds[1]) / cellSize][1
				+ (bounds[5] - bounds[2]) / cellSize];
	...
	/**
	 * Calculates an int array of size 6 into member variable bounds: - elements
	 * 0,1,2: minimum x,y,z of the iAtoms and jAtoms - elements 3,4,5: maximum x,y,z
	 * of the iAtoms and jAtoms
	 */
	...
	/**
	 * Returns an int array of size 6 : - elements 0,1,2: minimum x,y,z (in grid int
	 * coordinates) of the given atoms - elements 3,4,5: maximum x,y,z (in grid int
	 * coordinates) of the given atoms
	 * 
	 * @return
	 */
	...
	/**
	 * Returns all contacts, i.e. all atoms that are within the cutoff distance. If
	 * both iAtoms and jAtoms are defined then contacts are between iAtoms and
	 * jAtoms, if jAtoms is null, then contacts are within the iAtoms.
	 * 
	 * @return
	 */
	public AtomContactSet getAtomContacts() {
		...
		if (jAtomObjects == null) {
			list.forEach(cont -> contacts.add(new AtomContact(
					new Pair<Atom>(iAtomObjects[cont.getI()], iAtomObjects[cont.getJ()]), cont.getDistance())));

		} else {
			list.forEach(cont -> contacts.add(new AtomContact(
					new Pair<Atom>(iAtomObjects[cont.getI()], jAtomObjects[cont.getJ()]), cont.getDistance())));
		}
	...
	/**
	 * Returns all contacts, i.e. all atoms that are within the cutoff distance, as
	 * simple Contact objects containing the atom indices pairs and the distance. If
	 * both iAtoms and jAtoms are defined then contacts are between iAtoms and
	 * jAtoms, if jAtoms is null, then contacts are within the iAtoms.
	 * 
	 * @return
	...
	public List<Contact> getIndicesContacts() {
		...
		// if the 2 sets of atoms are not overlapping they are too far away and no need
		// to calculate anything
		// this won't apply if there's only one set of atoms (iAtoms), where we would
		// want all-to-all contacts
		if (noOverlap) {
			return list;
		}
		...
		for (int xind = 0; xind < cells.length; xind++) {
			for (int yind = 0; yind < cells[xind].length; yind++) {
				for (int zind = 0; zind < cells[xind][yind].length; zind++) {
					...
					// distances of points from this box to all neighbouring boxes: 26 iterations
					// (26 neighbouring boxes)
					for (int x = xind - 1; x <= xind + 1; x++) {
						for (int y = yind - 1; y <= yind + 1; y++) {
							for (int z = zind - 1; z <= zind + 1; z++) {
								...
								if (x >= 0 && x < cells.length && y >= 0 && y < cells[x].length && z >= 0
										&& z < cells[x][y].length) {
									if (cells[x][y][z] == null) {
										continue;
									}
	...
	/**
	 * Fast determination of whether any atoms from a given set fall within the
	 * cutoff of iAtoms. If {@link #addAtoms(Atom[], Atom[])} was called with two
	 * sets of atoms, contacts to either set are considered.
	 * 
	 * @param atoms
	 * @return
	 */
	...
	/**
	 * Tells whether (after having added atoms to grid) the i and j grids are not
	 * overlapping. Overlap is defined as enclosing bounds of the 2 grids being no
	 * more than one cell size apart.
	 * 
