...
/**
 * Implementation of a PDB Structure. This class provides the data contained in
 * a PDB file. to get structure objects from different sources see io package.
 *
 * @author Andreas Prlic
...
public class StructureImpl implements Structure {
	...
	public Structure clone() {
		...
		n.setName(getName());
		// TODO the header data is not being deep-copied, that's a minor issue since it
		// is just some static metadata, but we should recheck this if needed - JD
		// 2014-12-11
		...
		// deep-copying of entityInfofos is tricky: there's cross references also in the
		// Chains
		// beware: if we copy the entityInfos we would also need to reset the references
		// to entityInfos in the individual chains
		...
		for (EntityInfo entityInfo : this.entityInfos) {
			...
			for (String asymId : entityInfo.getChainIds()) {
				...
				for (int modelNr = 0; modelNr < n.nrModels(); modelNr++) {
					...
					if (newChain == null) {
						// this actually happens for structure 1msh, which has no chain B for model 29
						// (clearly a deposition error)
						logger.warn(new StringBuilder().append("Could not find chain asymId ").append(asymId)
								.append(" of model ").append(modelNr).append(" while cloning entityInfo ")
								.append(entityInfo.getMolId()).append(". Something is wrong!").toString());
						continue;
					}
		...
		// TODO ssbonds are complicated to clone: there are deep references inside Atom
		// objects, how would we do it? - JD 2016-03-03
	...
	public Group findGroup(String chainName, String pdbResnum, int modelnr) throws StructureException {
		...
		if (modelnr > models.size()) {
			throw new StructureException(new StringBuilder().append(" no model nr ").append(modelnr)
					.append(" in this structure. (contains ").append(models.size()).append(")").toString());
		}

		// first we need to gather all groups with the author id chainName: polymers,
		// non-polymers and waters
		...
		if (polyChain != null) {
			...
			getNonPolyChainsByPDB(chainName, modelnr).forEach(chain -> groups.addAll(chain.getAtomGroups()));
		...
		throw new StructureException(new StringBuilder().append("could not find group ").append(pdbResnum)
				.append(" in chain ").append(chainName).toString());
	}
	...
	public void addModel(List<Chain> modelChains) {
		modelChains.forEach(c -> c.setStructure(this));
		Model model = new Model();
		model.setChains(modelChains);
		models.add(model);
	}
	...
	public String toString() {
		...
		for (int i = 0; i < nrModels(); i++) {
			...
			for (int j = 0; j < size(i); j++) {
				...
				str.append("chain ").append(j).append(": asymId:").append(cha.getId()).append(" authId:")
		...
		dbrefs.forEach(dbref -> str.append(dbref.toPDB()).append(newline));
		str.append("Molecules: ").append(newline);
		entityInfos.forEach(mol -> str.append(mol).append(newline));
	...
	/**
	 * Whether this Structure is a crystallographic structure or not. It will first
	 * check the experimental technique and if not present it will try to guess from
	 * the presence of a space group and sensible cell parameters
	 *
	...
	/**
	 * Whether this Structure is a NMR structure or not. It will first check the
	 * experimental technique and if not present it will try to guess from the
	 * presence of more than 1 model and from b-factors being 0 in first chain of
	 * first model
	 * 
	...
	public void setChains(int modelnr, List<Chain> chains) {
		chains.forEach(c -> c.setStructure(this));
	...
	public Chain getChainByPDB(String authId, int modelnr) throws StructureException {
		...
		if (c == null) {
			throw new StructureException(
					new StringBuilder().append("Could not find chain with authId \"").append(authId).append("\"")
							.append(" for PDB id ").append(pdbId).append(", model ").append(modelnr).toString());
		}
	...
	public Chain getChain(String asymId, int modelnr) {
		...
		return chains.stream().filter(c -> c.getId().equals(asymId)).findFirst().orElse(null);
	...
	public Chain getPolyChain(String asymId, int modelIdx) {
		...
		return polyChains.stream().filter(c -> c.getId().equals(asymId)).findFirst().orElse(null);
	}
	...
	public Chain getNonPolyChain(String asymId, int modelIdx) {
		...
		return nonpolyChains.stream().filter(c -> c.getId().equals(asymId)).findFirst().orElse(null);
	}
	...
	public Chain getPolyChainByPDB(String authId, int modelIdx) {
		...
		List<Chain> polyChains = model.getPolyChains();
		return polyChains.stream().filter(c -> c.getName().equals(authId)).findFirst().orElse(null);
	}
	...
	public List<Chain> getNonPolyChainsByPDB(String authId, int modelIdx) {
		...
		List<Chain> nonpolyChains = model.getNonPolyChains();
		chains.addAll(nonpolyChains.stream().filter(c -> c.getName().equals(authId)).collect(Collectors.toList()));
	...
	public Chain getWaterChain(String asymId, int modelIdx) {
		...
		return waterChains.stream().filter(c -> c.getId().equals(asymId)).findFirst().orElse(null);
	}
	...
	public Chain getWaterChainByPDB(String authId, int modelIdx) {
		...
		List<Chain> waterChains = model.getWaterChains();
		return waterChains.stream().filter(c -> c.getName().equals(authId)).findFirst().orElse(null);
	}
	...
	public boolean hasChain(String authId) {
		...
		List<Chain> chains = getChains(modelnr);
		// we check here with equals because we might want to distinguish between upper
		// and lower case chains!
		return chains.stream().anyMatch(c -> c.getId().equals(authId));
	}
	...
	public boolean hasNonPolyChain(String asymId) {
		...
		// we check here with equals because we might want to distinguish between upper
		// and lower case chains!
		return chains.stream().anyMatch(c -> c.getId().equals(asymId));
	}
	...
	public boolean hasPdbChain(String authId) {
		...
		// we check here with equals because we might want to distinguish between upper
		// and lower case chains!
		return chains.stream().anyMatch(c -> c.getName().equals(authId));
	}
	...
	public EntityInfo getEntityById(int entityId) {
		return this.entityInfos.stream().filter(mol -> mol.getMolId() == entityId).findFirst().orElse(null);
	}
	...
	public void setDBRefs(List<DBRef> dbrefs) {
		...
		dbrefs.forEach(ref -> ref.setParent(this));
		this.dbrefs = dbrefs;
	}
	 ...
	 * publication. The JRNL section is not mandatory and thus may not be present.
	 * 
	...
	/**
	 * Caution: we should probably remove this to avoid confusion. Currently this is
	 * always an empty list!
	 *
	 * @return a list of Groups listed in the HET records - this will not include
	 *         any waters.
	 */
	 ...
	 *
	 * Only the first and last residues of each chain are considered, so chains with
	 * gaps
	 * 
	 * @return A {@link SubstructureIdentifier} with residue ranges constructed from
	 *         each chain
	 */
