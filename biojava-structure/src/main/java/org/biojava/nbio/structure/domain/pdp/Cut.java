...
public class Cut {
	...
	public int cut(Atom[] ca, Domain dom, CutValues val, int[][] dist, PDPDistanceMatrix pdpMatrix) {
		...
		if (verbose) {
			logger.info(new StringBuilder().append("  ---  Cut.cut ").append(dom).append(" ").toString());
		}
		average_density = 0.0;
		size0 = 0;
		for (iseg = 0; iseg < dom.nseg; iseg++) {
			...
			for (k = from; k < to; k++) {
				...
				/*
				 */
				x = Math.min(Math.pow(x, 1.3 / 3) + PDPParameters.RG,
						Math.pow(x, 1.1 / 3) + Math.pow(PDPParameters.TD, 1.3 / 3) + PDPParameters.RG);
				y = Math.min(Math.pow(y, 1.3 / 3) + PDPParameters.RG,
						Math.pow(y, 1.1 / 3) + Math.pow(PDPParameters.TD, 1.3 / 3) + PDPParameters.RG);
				 ...
				 */
				/*
				 * if(first_cut) if(k==277 && !first_cut)
				 * printf("k=%d s1=%d s2=%d x=%f y=%f mc=%d c=%d cd=%f\n",k,size1,size2,x,y,
				 * max_contacts[k],contacts[k],contact_density[k]);
				 */
				// if(verbose) System.out.println(String.format("%d %d %d %f %f %f %d
				// %f",k,size1,size2,x,y,max_contacts[k],contacts[k],contact_density[k]));
				/*
				 */
				if (from == 0) {
		...
		if (verbose) {
			logger.info(String.format(
					"  --- Trying to cut domain of size %d having %d segments and  average cont_density %f%n", dom.size,
					dom.nseg, average_density));
		}

		if (verbose) {
			for (kseg = 0; kseg < dom.nseg; kseg++) {
				logger.info(new StringBuilder()
						.append(String.format("	--- segment %d from %d to %d", kseg,
								dom.getSegmentAtPos(kseg).getFrom(), dom.getSegmentAtPos(kseg).getTo()))
						.append(" av density: ").append(average_density).toString());
			}
		}
		...
		if (val.first_cut) {
			 ...
			 * printf("%d	%s	%d	%d	%d	%d	%f\n",k,protein.res[k].type,k-from,to-k,
			 * contacts[k],max_contacts[k],contact_density[k]);
			 */
			val.AD = average_density;
			if (verbose) {
				logger.info(String.format("  --- AD=%f", average_density));
				/*
				 */
			}
		}
		...
		if (verbose) {
			logger.info(String.format("  --- after single cut: s_min = %f site_min = %d", val.s_min, site_min));
		}
		...
		for (l = 0; l < nclose; l++) {
			...
			// System.out.println(String.format("[0,iseg[ - ]iseg,jseg[ :
			// %d\n",contacts[nc]-no));
			...
			if (iseg == jseg) {
				// System.out.println(" CONTACT: " + from + " " + iclose[l] + " " + iseg + " " +
				// jseg);
				...
				for (int i = jclose[l]; i < to1; i++) {
					for (kseg = iseg + 1; kseg < jseg; kseg++) {
						for (int j = dom.getSegmentAtPos(kseg).getFrom(); j < dom.getSegmentAtPos(kseg).getTo(); j++) {
							/*
							 * if(iclose[l]==33&&jclose[l]==69&&dist[i][j])
							 * printf("%d %s %d %s %d\n",i,protein.res[i].type,j,protein.res[j].type,dist[i]
							 * [j]);
							 */
							contacts[nc] += (dist[j][i]);
						}
			 ...
			 */
			// if ( verbose)
			// System.out.println("double cut" + l + " " +
			// ca[iclose[l]].getGroup().getType() + " " + iclose[l] + " " + jclose[l] + " "
			// + contacts[nc]+
			// " " + max_contacts[nc] + " " + x+ " " + y + " " + size11 + " " + size22 + " "
			// + contact_density[nc] + " " + contact_density[nc]/val.AD );
			// if(verbose) System.out.println(String.format(" double cut: %d %s %d %d c=%d
			// mc=%d x=%f y=%f s1=%d s2=%d cd=%f
		...
		val.first_cut = false;
		if (verbose) {
			logger.info(String.format("  --- E ... at the end of cut: s_min %f CUTOFF %f site_min %d *site2 %d",
