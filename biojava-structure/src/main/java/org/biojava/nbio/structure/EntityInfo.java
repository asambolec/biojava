...
/**
 * An object to contain the info from the PDB header for a Molecule. In mmCIF
 * dictionary, it is called an Entity. In the case of polymers it is defined as
 * each group of sequence identical NCS-related chains
 *
...
public class EntityInfo implements Serializable {
	...
	// TODO We could drop a lot of the stuff here that is PDB-file related (actually
	// many PDB files don't contain many of these fields) - JD 2016-03-25
	// The only really essential part of a EntityInfo is the member chains and the
	// entity_id/mol_id
	...
	/**
	 * A map to cache residue number mapping, between ResidueNumbers and index
	 * (1-based) in aligned sequences (SEQRES). Initialised lazily upon call to
	 * {@link #getAlignedResIndex(Group, Chain)} Keys are asym_ids of chains, values
	 * maps of residue numbers to indices.
	 */
	...
	/**
	 * Constructs a new EntityInfo copying all data from the given one but not
	 * setting the Chains
	 * 
	 * @param c
	 */
	...
	public String toString() {
		...
		buf.append(description == null ? "(no name)"
				: new StringBuilder().append("(").append(description).append(")").toString());
		buf.append(" asymIds: ");
		if (chains != null) {
	...
	/**
	 * Get the representative Chain for this EntityInfo. We choose the Chain with
	 * the first asym_id chain identifier after lexicographical sorting (case
	 * insensitive), e.g. chain A if EntityInfo is composed of chains A,B,C,D,E
	 * 
	 * @return
	 */
	public Chain getRepresentative() {
		...
		chains.forEach(chain -> chainIds.add(chain.getId()));
	...
	/**
	 * Return the list of member chain ids (asym ids) that are described by this
	 * EntityInfo, only unique chain IDs are contained in the list. Note that in the
	 * case of multimodel structures this will return just the unique chain
	 * identifiers whilst {@link #getChains()} will return a corresponding chain per
	 * model.
	 * 
	...
	/**
	 * Given a Group g of Chain c (member of this EnityInfo) return the
	 * corresponding position in the alignment of all member sequences (1-based
	 * numbering), i.e. the index (1-based) in the SEQRES sequence. This allows for
	 * comparisons of residues belonging to different chains of the same EnityInfo
	 * (entity).
	 * <p>
	 * If {@link FileParsingParameters#setAlignSeqRes(boolean)} is not used or
	 * SEQRES not present, a mapping will not be available and this method will
	 * return {@link ResidueNumber#getSeqNum()} for all residues, which in some
	 * cases will be correctly aligned indices (when no insertion codes are used and
	 * when all chains within the entity are numbered in the same way), but in
	 * general they will be neither unique (because of insertion codes) nor aligned.
	 * </p>
	 * 
	 * @param g
	 * @param c
	 * @return the aligned residue index (1 to n), if no SEQRES groups are available
	 *         at all then {@link ResidueNumber#getSeqNum()} is returned as a
	 *         fall-back, if the group is not found in the SEQRES groups then -1 is
	 *         returned for the given group and chain
	 * @throws IllegalArgumentException if the given Chain is not a member of this
	 *                                  EnityInfo
	 * @see {@link Chain#getSeqResGroup(int)}
	 */
	public int getAlignedResIndex(Group g, Chain c) {

		boolean contained = getChains().stream().anyMatch(member -> c.getId().equals(member.getId()));
		if (!contained) {
			throw new IllegalArgumentException(new StringBuilder().append("Given chain with asym_id ").append(c.getId())
					.append(" is not a member of this entity: ").append(getChainIds().toString()).toString());
		}
		...
		if (map != null) {
			...
			// still it can happen that a group is in ATOM in one chain but not in other of
			// the same entity.
			// This is what we try to find out here (analogously to what we do in
			// initResSerialsMap() ):
			...
			// no seqres groups available we resort to using the pdb residue numbers are
			// given
	...
	private void initResSerialsMap(Chain c) {
		if (c.getSeqResGroups() == null || c.getSeqResGroups().isEmpty()) {
			logger.warn(
					"No SEQRES groups found in chain with asym_id {}, will use residue numbers as given (no insertion codes, not necessarily aligned). "
							+ "Make sure your structure has SEQRES records and that you use FileParsingParameters.setAlignSeqRes(true)",
					c.getId());
			// we add a explicit null to the map so that we flag it as unavailable for this
			// chain
		...
		for (int i = 0; i < c.getSeqResGroups().size(); i++) {
			...
			// The seqres group will have a null residue number whenever its corresponding
			// atom group doesn't exist
			// because it is missing in the electron density.
			// However, it can be observed in the density in other chains of the same
			// entity,
			// to be complete we go and look for the residue number in other chains, so that
			// we have a
			// seqres to atom mapping as complete as possible (with all known atom groups of
			// any chain of this entity)
			...
			// Thus the mapping will not be possible for residues that are only in SEQRES
			// groups
	...
	private ResidueNumber findResNumInOtherChains(int i, Chain chain) {
		...
		// note that getChains contains all chains from all models, we'll just use first
		// model found and skip the others
		for (Chain c : getFirstModelChains()) {
			...
			if (seqResGroup == null) {
				logger.warn(
						"The SEQRES group is null for index {} in chain with asym_id {}, whilst it wasn't null in chain with asym_id {}",
	...
	/**
	 * Get the list of chains that are part of this EntityInfo. Note that for
	 * multi-model structures chains from all models are returned.
	 *
	...
	private List<Chain> getFirstModelChains() {
		...
		chains.stream().filter(chain -> lookupChainIds.contains(chain.getId()))
				.forEach(chain -> firstModelChains.putIfAbsent(chain.getId(), chain));
	...
	/**
	 * Get the type of entity this EntityInfo describes. Options are polymer,
	 * non-polymer or water.
	 * 
	 * @return a string describing the type of entity. (polymer, non-polymer or
	 *         water).
	 */
	...
	/**
	 * Set the type of entity this EntityInfo describes. Options are polymer,
	 * non-polymer or water.
	 * 
	 * @param type a string describing the type of entity. (polymer, non-polymer or
	 *             water).
	 */
