...
/**
 * A simple bean to store disulfide bridge information, the SSBOND records in
 * the PDB files. The two residues specified here are CYS residues that form a
 * Disulfide bridge.
 *
...
public class SSBondImpl implements PDBRecord, Cloneable {
	...
	public void toPDB(StringBuffer buf) {
		...
		/*
		 * 12 - 14 LString(3) "CYS" Residue name. 16 Character chainID1 Chain
		 * identifier. 18 - 21 Integer seqNum1 Residue sequence number. 22 AChar icode1
		 * Insertion code. 26 - 28 LString(3) "CYS" Residue name. 30 Character chainID2
		 * Chain identifier. 32 - 35 Integer seqNum2 Residue sequence number. 36 AChar
		 * icode2 Insertion code. 60 - 65 SymOP sym1 Symmetry oper for 1st resid 67 - 72
		 * SymOP sym2 Symmetry oper for 2nd resid
		 */
	...
	public String toString() {
		...
		s += "Atom 1:\n";
		s += new StringBuilder().append("\tChain: ").append(chainID1).append("\n").toString();
		s += new StringBuilder().append("\tResidue #: ").append(resnum1).append("\n").toString();
		s += new StringBuilder().append("\tIns. Code: ").append(insCode1).append("\n").toString();

		s += "Atom 2:\n";
		s += new StringBuilder().append("\tChain: ").append(chainID2).append("\n").toString();
		s += new StringBuilder().append("\tResidue #: ").append(resnum2).append("\n").toString();
		s += new StringBuilder().append("\tIns. Code: ").append(insCode2).append("\n").toString();
	...
	public static SSBondImpl toSsBond(Bond bond) {
		...
		if (!"CYS".equals(bond.getAtomA().getGroup().getPDBName())
			...
			throw new IllegalArgumentException(
					"Trying to create a SSBond from a Bond between 2 groups that are not CYS");
		}
