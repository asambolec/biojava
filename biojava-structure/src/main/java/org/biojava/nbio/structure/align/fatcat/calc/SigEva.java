...
public class SigEva {
	...
	public void getPara(int set, int len) {
		if (set == 1) {
			...
			beta_a = 0.1284;
			beta_b = 1.3756;
		} // fitting, based on the old benchmark (the fold.list is redundant), old
			// parameters
		else if (set == 2) {
			mu_a = 1.1137;
			mu_b = 6.5574;
			...
			beta_a = 0.3525;
			beta_b = 18.6652;
		} // score * sqrt(optLen / (rmsd * (twist + 1))), the new best parameters for
			// flexible FATCAT, "FNS8"
		else if (set == 4) {
			mu_a = 0.4708;
			mu_b = 38.9863;
			beta_a = 0.2511;
			beta_b = 12.9228;
		} // score * sqrt(optLen / RMSD), the new best parameters for rigid FATCAT,
			// sparse-sampling=2, modified scoring
		else if (set == 5) {
			mu_a = 1.3794;
			mu_b = -14.4778;
			beta_a = 0.7465;
			beta_b = -22.9452;
		} // score * sqrt(optLen / RMSD), the new best parameters for flexible FATCAT,
			// sparse-sampling=2, modified scoring 3
		else if (set == 6) {
			mu_a = 0.6036;
			mu_b = 35.4783;
			beta_a = 0.3136;
			...
			// sparse-sampling=3, modified scoring
		else if (set == 9) {
			mu_a = 0.6672;
			mu_b = 26.5767;
			beta_a = 0.4373;
			...
			// sparse-sampling=3, modified scoring 2
		else {
	...
	private double calSigAll(int twist, int sparse, int len1, int len2, double score, double rmsd, int optLen, int r) {
		...
		if (sparse == 2) { // use sparse sampling = 2
			if (twist == 0) {
				getPara(4, len); // rigid-FATCAT
			} else {
				getPara(5, len); // flexible-FATCAT
			}
		} else if (sparse == 3) { // sparse sampling = 3
			if (twist == 0) {
				getPara(8, len); // rigid-FATCAT
			} else {
				getPara(8, len); // flexible-FATCAT
			}
		} else if (sparse == 1) { // sparse sampling = 1
			if (twist == 0) {
				getPara(6, len); // rigid-FATCAT
			} else {
				getPara(7, len); // flexible-FATCAT
			}
		} else { // no sparse sampling or the corresponding sparse parameters are not fitted
			if (twist == 0) {
				getPara(2, len); // rigid-FATCAT
			} else {
				getPara(3, len); // flexible-FATCAT
			}
		}
