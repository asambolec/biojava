...
/**
 * A simple interface for an Atom. The coordinates can be accessed via the
 * {@link #getCoords()}, or the {@link #getX()}, {@link #getY()},
 * {@link #getZ()} methods. There are a few additional methods here to provide
 * some PDB specific information.
 *
 * 
 * @author Andreas Prlic
...
public interface Atom extends Cloneable, PDBRecord {
	...
	/**
	 * Get atom name, e.g. "CA". Beware that some PDB atom names are ambiguous (e.g.
	 * CA, which means C-alpha or Calcium), the ambiguity can simply be resolved by
	 * also checking the element with {@link #getElement()}
	 * 
	 ...
	 * Get the coordinates.
	 * <p>
	 * Internally the coordinates are represented as Point3d so this is recommended
	 * over {@link #getCoords()}
	 * 
	...
	/**
	 * Return the parent Group of the Atom. returns null if the referenced object is
	 * not Group
	 * 
