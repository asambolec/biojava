...
/**
 * A crystallographic space group. We store the standard numeric identifier, the
 * international short symbol and the transformations corresponding to each
 * space group (as Matrix4ds and in algebraic notation). The information for all
 * (protein crystallography) space groups can be parsed from the XML file in the
 * resource directory.
 *
...
public class SpaceGroup implements Serializable {
	...
	private static final Pattern coordPat = Pattern.compile("(?:([+-])?([XYZ]))+?"); // the last +? is for ungreedy
																						// matching
	...
	private Vector3d[] cellTranslations; // in space groups I, C, F or H there are pure cell translations corresponding
											// to recenterings
	...
	public SpaceGroup(int id, int multiplicity, int primitiveMultiplicity, String shortSymbol, String altShortSymbol,
	...
	/**
	 * Get the space group for the given international short name, using the PDB
	 * format, e.g. 'P 21 21 21' or 'C 1 c 1'
	 * 
	 * @param shortName
	...
	public static Matrix4d getMatrixFromAlgebraic(String transfAlgebraic) {
		...
		mat.setIdentity();
		mat.setRotation(
				new Matrix3d(xCoef[0], xCoef[1], xCoef[2], yCoef[0], yCoef[1], yCoef[2], zCoef[0], zCoef[1], zCoef[2]));
	...
	/**
	 * Gets the standard numeric identifier for the space group. See for example
	 * http://en.wikipedia.org/wiki/Space_group or the IUCr crystallographic tables
	 * 
	 * @return
	 */
	public int getId() {
	...
	/**
	 * Gets the international short name (as used in PDB), e.g. "P 21 21 21" or "C 1
	 * c 1"
	 * 
	 * @return
	 */
	...
	private void calcRotAxesAndAngles() {
		...
		for (int i = 1; i < this.transformations.size(); i++) {
			Matrix3d r = new Matrix3d(transformations.get(i).m00, transformations.get(i).m01,
					transformations.get(i).m02, transformations.get(i).m10, transformations.get(i).m11,
					transformations.get(i).m12, transformations.get(i).m20, transformations.get(i).m21,
					transformations.get(i).m22);
	...
	/**
	 * Calculates the axis fold type (1, 2, 3, 4, 5, 6 for rotations or -1, -2, -3,
	 * -4, -6 improper rotations) from the trace of the rotation matrix, see for
	 * instance
	...
	/**
	 * Returns true if both given transform ids belong to the same crystallographic
	 * axis (a, b or c) For two non-rotation transformations (i.e. identity
	 * operators) it returns true
	 * 
	 * @param tId1
	 * @param tId2
	 * @return
	...
	public boolean areInSameAxis(int tId1, int tId2) {
		...
		// we can't deal yet with improper rotations: we return false whenever either of
		// them is improper
		...
		// TODO revise: we might need to consider that the 2 are in same direction but
		// opposite senses
	...
	/**
	 * Given a transformId returns the type of axis of rotation: 1 (no rotation), 2,
	 * 3, 4 or 6 -fold and for improper rotations: -1, -2, -3, -4 and -6
	 *
	 * @param transformId
	 * @return
	 */
	...
	/**
	 * Gets a transformation by index expressed in crystal axes basis. Index 0
	 * corresponds always to the identity transformation. Beware the returned
	 * Matrix4d is not a copy but it stays linked to the one stored in this
	 * SpaceGroup object
	 * 
	 * @param i
	 * @return
	...
	/**
	 * Gets a transformation algebraic string given its index. Index 0 corresponds
	 * always to the identity transformation.
	 * 
	 * @param i
	 * @return
	 */
	...
	/**
	 * Gets the number of symmetry operators corresponding to this SpaceGroup
	 * (counting the identity operator)
	 * 
	 * @return
	 */
	...
	public static String getAlgebraicFromMatrix(Matrix4d m) {
		...
		String alg = new StringBuilder().append(x).append(",").append(y).append(",").append(z).toString();
		return alg;
	}
	...
	private static String formatAlg(double xcoef, double ycoef, double zcoef, double trans) {
		...
		return new StringBuilder().append(x).append(y).append(z).append(t).toString();
	...
	/**
	 * Given a rotation matrix calculates the rotation axis and angle for it. The
	 * angle is calculated from the trace, the axis from the eigenvalue
	 * decomposition. If given matrix is improper rotation or identity matrix then
	 * axis (0,0,0) and angle 0 are returned.
	 * 
	 * @param m
	 * @return
	 * @throws IllegalArgumentException if given matrix is not a rotation matrix
	 *                                  (determinant not 1 or -1)
	 */
	public static AxisAngle4d getRotAxisAndAngle(Matrix3d m) {
		...
		if (deltaComp(eval.get(0, 0), 1.0, DELTA) && deltaComp(eval.get(1, 1), 1.0, DELTA)
				&& deltaComp(eval.get(2, 2), 1.0, DELTA)) {
	...
	/**
	 * Given a transformation matrix containing a rotation returns the type of
	 * rotation: 1 for identity, 2 for 2-fold rotation, 3 for 3-fold rotation, 4 for
	 * 4-fold rotation, 6 for 6-fold rotation, -1 for inversions, -2 for mirror
	 * planes, -3 for 3-fold improper rotation, -4 for 4-fold improper rotation and
	 * -6 for 6-fold improper rotation
	 * 
	 * @param m
	 * @return
	 */
	...
	// @XmlElementWrapper(name="transfAlgebraicList",
	// namespace="http://www.biojava.org")
	// @XmlElement
	...
	public void setTransfAlgebraic(List<String> transfAlgebraic) {
		...
		transfAlgebraic.forEach(this::addTransformation);
	}
