...
public class MultipleAlignmentWriter {
	...
	public static String toFASTA(MultipleAlignment alignment) {
		...
		for (int st = 0; st < alignment.size(); st++) {
			...
			fasta += new StringBuilder().append(">")
					.append(alignment.getEnsemble().getStructureIdentifiers().get(st).getIdentifier()).append("\n")
					.append(alnSequences.get(st)).append("\n").toString();
		}
		return fasta;
	}

	/**
	 * Converts the {@link MultipleAlignment} into a FatCat String format. Includes
	 * summary information about the alignment in the top and a multiple sequence
	...
	public static String toFatCat(MultipleAlignment alignment) {
		...
		for (int pos = 0; pos < alnSequences.get(0).length(); pos++) {
			...
			if (blockNr != -1) {
				blockNumbers = blockNumbers + String.valueOf(blockNr + 1);
			} else {
		...
		for (int str = 0; str < alignment.size(); str++) {
			if (str < 9) {
				fatcat.append(new StringBuilder().append("Chain 0").append(str + 1).append(": ")
						.append(alnSequences.get(str)).append("\n").toString());
			} else {
				fatcat.append(new StringBuilder().append("Chain ").append(str + 1).append(": ")
						.append(alnSequences.get(str)).append("\n").toString());
			}
			if (str != alignment.size() - 1) {
				fatcat.append(new StringBuilder().append("          ").append(blockNumbers).append("\n").toString());
			}
		}
	...
	public static String toAlignedResidues(MultipleAlignment multAln) {
		...
		for (int str = 0; str < multAln.size(); str++) {
			residueGroup.append(new StringBuilder().append("#Struct").append(str + 1).append(":\t").toString());
			residueGroup.append(multAln.getEnsemble().getStructureIdentifiers().get(str).getIdentifier());
			residueGroup.append("\n");
		}
		// Whrite header for columns
		for (int str = 0; str < multAln.size(); str++) {
			residueGroup.append(new StringBuilder().append("#Num").append(str + 1).append("\tChain").append(str + 1)
					.append("\tAA").append(str + 1).append("\t").toString());
		}
		residueGroup.append("\n");

		// Write optimally aligned pairs
		multAln.getBlocks().forEach(b -> {
		...
		for (int bs = 0; bs < alignment.getBlockSets().size(); bs++) {
			...
			for (int str = 0; str < alignment.size(); str++) {
				...
				txt.append(String.format(
						new StringBuilder().append("     X").append(str + 1).append(" = (%9.6f)*X").append(origString)
								.append(" + (%9.6f)*Y").append(origString).append(" + (%9.6f)*Z").append(origString)
								.append(" + (%12.6f)").toString(),
						btransforms.get(str).getElement(0, 0), btransforms.get(str).getElement(0, 1),
						btransforms.get(str).getElement(0, 2), btransforms.get(str).getElement(0, 3)));
				txt.append("\n");
				txt.append(String.format(
						new StringBuilder().append("     Y").append(str + 1).append(" = (%9.6f)*X").append(origString)
								.append(" + (%9.6f)*Y").append(origString).append(" + (%9.6f)*Z").append(origString)
								.append(" + (%12.6f)").toString(),
						btransforms.get(str).getElement(1, 0), btransforms.get(str).getElement(1, 1),
						btransforms.get(str).getElement(1, 2), btransforms.get(str).getElement(1, 3)));
				txt.append("\n");
				txt.append(String.format(
						new StringBuilder().append("     Z").append(str + 1).append(" = (%9.6f)*X").append(origString)
								.append(" + (%9.6f)*Y").append(origString).append(" + (%9.6f)*Z").append(origString)
								.append(" + (%12.6f)").toString(),
						btransforms.get(str).getElement(2, 0), btransforms.get(str).getElement(2, 1),
						btransforms.get(str).getElement(2, 2), btransforms.get(str).getElement(2, 3)));
				txt.append("\n\n");
			}
		}
		return txt.toString();
	 ...
	 * String format. Cached variables, like transformation matrices and scores, are
	 * also converted.
	 *
	 ...
	 *
	 * <p>
	 * The format is closely related to a standard PDB file, but contains only CA
	 * atoms and adds two columns for specifying the alignment:
	 *
	 * <pre>
	 ...
	 *
	 * <p>
	 * Note that the output is a pairwise alignment. Only the first and second rows
	 * in the MultipleAlignment will be used, others ignored.
	 *
	 * <p>
	 * This method supports topology-independent alignments. The output will have
	 * sequence order matching the query, but include atoms from the template.
	 *
	 ...
	 * @throws StructureException If an error occurs parsing the alignment's
	 *                            structure names
	 */
	public static String to3DFormat(MultipleAlignment alignment, int queryIndex, int templateIndex)
		...
		str.append(String.format("REMARK Template name:%s:%s%n", tPdbId, tChain));
