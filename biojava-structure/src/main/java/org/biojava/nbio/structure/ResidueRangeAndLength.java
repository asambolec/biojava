 ...
 *
 * Also stores a length. Because of insertion codes, this length is not
 * necessarily {@code end − start}.
 */
public class ResidueRangeAndLength extends ResidueRange {
	 ...
	 * {@code rrs}.
	 *
	 * Assumes no overlap. If two or more ranges cover the same residues, will
	 * over-count the union of the residues.
	 *
	 * @param rrs A list of ResidueRanges
	 * @return The combined length
	 * @throws IllegalArgumentException If the {@link #getLength() length} of one or
	 *                                  more ResidueRange is null
	 * @see #getLength()
	 */
	 ...
	 * information, such as for whole chains ('A:'). Supports the special chain name
	 * '_' for single-chain structures.
	 *
	 * If residues are specified outside of the range given in the map, attempts to
	 * decrease the input range to valid values. In extreme cases where this process
	 * fails fails to find any valid indices, returns null.
	 *
	 * For a function which more conservatively represents the input range, without
	 * chain inference and error fixes, use {@link ResidueRange#parse(String)}.
	 * 
	 * @param s A string of the form chain_start-end. For example:
	 *          <code>A.5-100</code>.
	...
	public static ResidueRangeAndLength parse(String s, AtomPositionMap map) {
		...
		if (chain == null || "_".equals(chain)) {
			...
			// Quick check for additional chains. Not guaranteed if the atoms are out of
			// order.
		...
		// if it's the whole chain, choose the first and last residue numbers in the
		// chain
		if (start == null) {
	...
	public static List<ResidueRangeAndLength> parseMultiple(List<String> ranges, AtomPositionMap map) {
		...
		ranges.stream().map(range -> ResidueRangeAndLength.parse(range, map)).forEach(rr -> {
			if (rr != null) {
				rrs.add(rr);
			}
		});
