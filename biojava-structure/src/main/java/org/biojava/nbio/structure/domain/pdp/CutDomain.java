...
public class CutDomain {
	...
	public void cutDomain(Domain dom, CutSites cut_sites, PDPDistanceMatrix pdpMatrix) {
		...
		if (verbose) {
			logger.info(new StringBuilder().append("  B ... beginning of cutDomain ").append(dom).append(" cutsites: ")
					.append(cut_sites).toString());
		}
		...
		if (verbose) {
			logger.info(new StringBuilder().append("  S ... site ").append(dom).append(" : site: ").append(site)
					.append(" val : ").append(val).toString());
		}
		...
		if (verbose) {
			logger.info(String.format("   C ... Cutting at position(s): %d %d %f%n", site, val.site2, dom.score));
		}
		...
		if (val.site2 == 0) { /* single cut */
			...
			for (i = 0; i < dom.nseg; i++) {
				if (site > dom.getSegmentAtPos(i).getTo() || val.site2 < dom.getSegmentAtPos(i).getFrom()) {
					...
					dom1.getSegmentAtPos(dom1.nseg).setFrom(dom.getSegmentAtPos(i).getFrom());
					dom1.size += (dom1.getSegmentAtPos(dom1.nseg).getTo() - dom1.getSegmentAtPos(dom1.nseg).getFrom()
							+ 1);
					dom1.nseg++;
					dom2.getSegmentAtPos(dom2.nseg).setFrom(site + 1);
					if (val.site2 > dom.getSegmentAtPos(i).getFrom() && val.site2 < dom.getSegmentAtPos(i).getTo()) {
						dom2.getSegmentAtPos(dom2.nseg).setTo(val.site2 - 1);
						dom2.size += (dom2.getSegmentAtPos(dom2.nseg).getTo()
								- dom2.getSegmentAtPos(dom2.nseg).getFrom() + 1);
						dom2.nseg++;
						dom1.getSegmentAtPos(dom1.nseg).setFrom(val.site2);
						dom1.getSegmentAtPos(dom1.nseg).setTo(dom.getSegmentAtPos(i).getTo());
						dom1.size += (dom1.getSegmentAtPos(dom1.nseg).getTo()
								- dom1.getSegmentAtPos(dom1.nseg).getFrom() + 1);
						dom1.nseg++;
					} else {
						dom2.getSegmentAtPos(dom2.nseg).setTo(dom.getSegmentAtPos(i).getTo());
						dom2.size += (dom2.getSegmentAtPos(dom2.nseg).getTo()
				...
				} else if (val.site2 > dom.getSegmentAtPos(i).getFrom() && val.site2 < dom.getSegmentAtPos(i).getTo()) {
					dom2.getSegmentAtPos(dom2.nseg).setTo(val.site2 - 1);
					dom2.getSegmentAtPos(dom2.nseg).setFrom(dom.getSegmentAtPos(i).getFrom());
					dom2.size += (dom2.getSegmentAtPos(dom2.nseg).getTo() - dom2.getSegmentAtPos(dom2.nseg).getFrom()
							+ 1);
					dom2.nseg++;
					dom1.getSegmentAtPos(dom1.nseg).setFrom(val.site2);
					dom1.getSegmentAtPos(dom1.nseg).setTo(dom.getSegmentAtPos(i).getTo());
		...
		if (verbose) {
			for (i = 0; i < dom1.nseg; i++) {
				logger.info(String.format("	F ... from %d to %d", dom1.getSegmentAtPos(i).getFrom(),
		...
		if (verbose) {
			for (i = 0; i < dom2.nseg; i++) {
				logger.info(String.format("	F ... from %d to %d", dom2.getSegmentAtPos(i).getFrom(),
