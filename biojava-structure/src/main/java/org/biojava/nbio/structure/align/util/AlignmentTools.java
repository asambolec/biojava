...
/**
 * Methods for analyzing and manipulating AFPChains and for other pairwise
 * alignment utilities.
 * <p>
 * Current methods: replace optimal alignment, create new AFPChain, format
 * conversion, update superposition, etc.
 *
...
public class AlignmentTools {
	...
	/**
	 * Checks that the alignment given by afpChain is sequential. This means that
	 * the residue indices of both proteins increase monotonically as a function of
	 * the alignment position (ie both proteins are sorted).
	 *
	 * This will return false for circularly permuted alignments or other
	 * non-topological alignments. It will also return false for cases where the
	 * alignment itself is sequential but it is not stored in the afpChain in a
	 * sorted manner.
	 *
	 * Since algorithms which create non-sequential alignments split the alignment
	 * into multiple blocks, some computational time can be saved by only checking
	 * block boundaries for sequentiality. Setting <tt>checkWithinBlocks</tt> to
	 * <tt>true</tt> makes this function slower, but detects AFPChains with
	 ...
	 * relies on the StructureAlignment algorithm correctly setting this parameter,
	 ...
	 *
	 * @param afpChain An alignment
	 * @return A mapping from aligned residues of protein 1 to their partners in
	 *         protein 2.
	...
	/**
	 * Applies an alignment k times. Eg if alignmentMap defines function f(x), this
	 * returns a function f^k(x)=f(f(...f(x)...)).
	 *
	 * @param              <T>
	 * @param alignmentMap The input function, as a map (see
	 *                     {@link AlignmentTools#alignmentAsMap(AFPChain)})
	 ...
	 * numbering schemes, the identity function could calculate the offset between
	 * residue numbers, eg I(x) = x-offset.
	 *
	 ...
	 * @param alignmentMap The input function, as a map (see
	 *                     {@link AlignmentTools#alignmentAsMap(AFPChain)})
	 * @param identity     An identity-like function providing the isomorphism
	 *                     between the codomain of alignmentMap (of type <T>) and
	 *                     the domain (type <S>).
	 * @param k            The number of times to apply the alignment
	 * @return A new alignment. If the input function is not automorphic
	 *         (one-to-one), then some inputs may map to null, indicating that the
	...
	public static <S, T> Map<S, T> applyAlignment(Map<S, T> alignmentMap, Map<T, S> identity, int k) {
		...
		/*
		 * for(int i=0;i<preimage.size();i++) { S pre = preimage.get(i); T intermediate
		 * = (pre==null?null: alignmentMap.get(pre)); S post = (intermediate==null?null:
		 * identity.get(intermediate)); imageMap.put(post, null); }
		 */
	 ...
	 *
	 * <p>
	 * This method should only be used in cases where the two proteins aligned have
	 * identical numbering, as for self-alignments. See
	 * {@link #getSymmetryOrder(AFPChain, int, float)} for a way to guess the
	 * sequential correspondence between two proteins.
	 *
	 * @param alignment
	 * @param maxSymmetry
	 ...
	 * @return
	 */
	public static int getSymmetryOrder(Map<Integer, Integer> alignment, final int maxSymmetry,
	 ...
	 *
	 * <p>
	 * Conceptually, an alignment is a function f:A->B between two sets of integers.
	 * The function may have simple topology (meaning that if two elements of A are
	 * close, then their images in B will also be close), or may have more complex
	 * topology (such as a circular permutation). This function checks
	 * <i>alignment</i> against a reference function <i>identity</i>, which should
	 * have simple topology. It then tries to determine the symmetry order of
	 * <i>alignment</i> relative to <i>identity</i>, up to a maximum order of
	 * <i>maxSymmetry</i>.
	 *
	 *
	 * <p>
	 * <strong>Details</strong><br/>
	 * Considers the offset (in number of residues) which a residue moves after
	 * undergoing <i>n</i> alternating transforms by alignment and identity. If
	 * <i>n</i> corresponds to the intrinsic order of the alignment, this will be
	 * small. This algorithm tries increasing values of <i>n</i> and looks for
	 * abrupt decreases in the root mean squared offset. If none are found at
	 * <i>n</i><=maxSymmetry, the alignment is reported as non-symmetric.
	 *
	 * @param alignment           The alignment to test for symmetry
	 * @param identity            An alignment with simple topology which
	 *                            approximates the sequential relationship between
	 *                            the two proteins. Should map in the reverse
	 ...
	 * @param minimumMetricChange Percent decrease in root mean squared offsets in
	 *                            order to declare symmetry. 0.4f seems to work well
	 *                            for CeSymm.
	 * @return The order of symmetry of alignment, or 1 if no order <= maxSymmetry
	 *         is found.
	 *
	 ...
	 *
	 * <p>
	 * Uses {@link #getSymmetryOrder(Map alignment, Map identity, int, float)} to
	 * determine the the symmetry order. For the identity alignment, sorts the
	 * aligned residues of each protein sequentially, then defines the ith residues
	 * of each protein to be equivalent.
	 *
	 * <p>
	 * Note that the selection of the identity alignment here is <i>very</i> naive,
	 * and only works for proteins with very good coverage. Wherever possible, it is
	 * better to construct an identity function explicitly from a sequence alignment
	 * (or use an {@link IdentityMap} for internally symmetric proteins) and use
	 * {@link #getSymmetryOrder(Map, Map, int, float)}.
	 */
	...
	/**
	 * Takes a potentially non-sequential alignment and guesses a sequential version
	 * of it. Residues from each structure are sorted sequentially and then compared
	 * directly.
	 *
	 * <p>
	 * The results of this method are consistent with what one might expect from an
	 * identity function, and are therefore useful with
	 * {@link #getSymmetryOrder(Map, Map identity, int, float)}.
	 * <ul>
	 * <li>Perfect self-alignments will have the same pre-image and image, so will
	 * map X->X</li>
	 * <li>Gaps and alignment errors will cause errors in the resulting map, but
	 * only locally. Errors do not propagate through the whole alignment.</li>
	 * </ul>
	 *
	 * <h4>Example:</h4> A non sequential alignment, represented schematically as
	 * 
	 * <pre>
	 * 12456789
	 * 78912345
	 ...
	 * @param inverseAlignment If false, map from structure1 to structure2. If true,
	 *                         generate the inverse of that map.
	 * @return A mapping from sequential residues of one protein to those of the
	 *         other
		...
		for (Entry<Integer, Integer> pair : alignment.entrySet()) {
			...
			if (!aligned2.add(pair.getValue())) {
				throw new IllegalArgumentException(
						new StringBuilder().append("Alignment is not one-to-one for residue ").append(pair.getValue())
								.append(" of the second structure.").toString());
			}
		}
	...
	/**
	 * Retrieves the optimum alignment from an AFPChain and returns it as a java
	 * collection. The result is indexed in the same way as
	 ...
	 * protein. This method converts two lists of ResidueNumbers into an AFPChain.
	 *
	 * <p>
	 ...
	 * @param aligned2 A list of aligned residues from the second protein. Must be
	 *                 the same length as aligned1.
	 * @return An AFPChain representing the alignment. Many properties may be null
	 *         or another default.
	...
	public static AFPChain createAFPChain(Atom[] ca1, Atom[] ca2, ResidueNumber[] aligned1, ResidueNumber[] aligned2)
		...
		for (int i = 0; i < alnLen; i++) {
			pdbAln[0][0][i] = new StringBuilder().append(aligned1[i].getChainName()).append(":").append(aligned1[i])
					.toString();
			pdbAln[0][1][i] = new StringBuilder().append(aligned2[i].getChainName()).append(":").append(aligned2[i])
					.toString();
		}
	...
	/**
	 * It replaces an optimal alignment of an AFPChain and calculates all the new
	 * alignment scores and variables.
	 */
	public static AFPChain replaceOptAln(int[][][] newAlgn, AFPChain afpChain, Atom[] ca1, Atom[] ca2)
		...
		// Create a copy of the original AFPChain and set everything needed for the
		// structure update
	...
	/**
	 * Takes an AFPChain and replaces the optimal alignment based on an alignment
	 * map
	 *
	 * <p>
	 * Parameters are filled with defaults (often null) or sometimes calculated.
	 ...
	 * @throws StructureException if an error occurred during superposition
	 * @see AlignmentTools#createAFPChain(Atom[], Atom[], ResidueNumber[],
	 *      ResidueNumber[])
	 */
	public static AFPChain replaceOptAln(AFPChain afpChain, Atom[] ca1, Atom[] ca2, Map<Integer, Integer> alignment)
			throws StructureException {
		...
		for (int i = 1; i < res1.length; i++) {
			...
			assert (currRes != null);// could be converted to if statement if assertion doesn't hold; just modify
										// below as well.
			if (lastRes < currRes) {
				blkLen++;
			} else {
	 ...
	 * @throws StructureException if an error occured during superposition
	 */
	public static AFPChain replaceOptAln(AFPChain afpChain, Atom[] ca1, Atom[] ca2, int blockNum, int[] optLens,
			int[][][] optAln) throws StructureException {
		int optLength = 0;
		for (int blk = 0; blk < blockNum; blk++) {
	...
	/**
	 * After the alignment changes (optAln, optLen, blockNum, at a minimum), many
	 * other properties which depend on the superposition will be invalid.
	 *
	 * This method re-runs a rigid superposition over the whole alignment and
	 * repopulates the required properties, including RMSD (TotalRMSD) and TM-Score.
	 * 
	 * @param afpChain
	 * @param ca1
	 * @param ca2      Second set of ca atoms. Will be modified based on the
	 *                 superposition
	 * @throws StructureException
	 * @see {@link CECalculator#calc_rmsd(Atom[], Atom[], int, boolean)} contains
	 *      much of the same code, but stores results in a CECalculator instance
	 *      rather than an AFPChain
	 */
		...
		for (int k = 0; k < afpChain.getBlockNum(); k++) {
			...
			if (position != afpChain.getOptLen()[k]) {
				logger.warn(new StringBuilder().append(
						...
						.append(position).append(" but should be ").append(afpChain.getOptLen()[k]).toString());
	...
	/**
	 * Print an alignment map in a concise representation. Edges are given as two
	 * numbers separated by '>'. They are chained together where possible, or
	 * separated by spaces where disjoint or branched.
	 *
	 * <p>
	 ...
	 * <li>1>2>3>2 4>3</li>
	 *
	 * @param alignment The input function, as a map (see
	 *                  {@link AlignmentTools#alignmentAsMap(AFPChain)})
	...
	/**
	 * Method that calculates the number of gaps in each subunit block of an optimal
	 * AFP alignment.
	 *
	 * INPUT: an optimal alignment in the format int[][][]. OUTPUT: an int[] array
	 * of <order> length containing the gaps in each block as int[block].
	 */
		...
		for (int i = 0; i < optAln.length; i++) {
			...
			for (int j = 0; j < optAln[i][0].length; j++) {
				...
				if (j == 0) {
					...
					// If one of the positions or both are not contiguous increment the number of
					// gaps
	 ...
	 * 1. backbone edges, which connect adjacent residues in the aligned protein 2.
	 * alignment edges, which connect aligned residues
	 *
	 ...
	 * @param alignmentInteraction Two-letter string used to identify alignment
	 *                             edges
	 * @throws IOException
	 */
	public static void alignmentToSIF(Writer out, AFPChain afpChain, Atom[] ca1, Atom[] ca2, String backboneInteraction,
		...
		for (int b = 0; b < nblocks; b++) {
			for (int r = 0; r < blockLen[b]; r++) {
				...
				String node1 = new StringBuilder().append(name1).append(rn1.getChainName()).append(rn1.toString())
						.toString();
				String node2 = new StringBuilder().append(name2).append(rn2.getChainName()).append(rn2.toString())
						.toString();

				out.write(String.format("%s\t%s\t%s%n", node1, alignmentInteraction, node2));
			}
		}

		// Print first backbone edges
		ResidueNumber rn = ca1[0].getGroup().getResidueNumber();
		String last = new StringBuilder().append(name1).append(rn.getChainName()).append(rn.toString()).toString();
		for (int i = 1; i < ca1.length; i++) {
			rn = ca1[i].getGroup().getResidueNumber();
			String curr = new StringBuilder().append(name1).append(rn.getChainName()).append(rn.toString()).toString();
			out.write(String.format("%s\t%s\t%s%n", last, backboneInteraction, curr));
			last = curr;
		}
		...
		if (!(!name1.equals(name2) || ca1.length != ca2.length
		...
		last = new StringBuilder().append(name2).append(rn.getChainName()).append(rn.toString()).toString();
		for (int i = 1; i < ca2.length; i++) {
			rn = ca2[i].getGroup().getResidueNumber();
			String curr = new StringBuilder().append(name2).append(rn.getChainName()).append(rn.toString()).toString();
			out.write(String.format("%s\t%s\t%s%n", last, backboneInteraction, curr));
			last = curr;
		}
	}
	...
	/**
	 * get an artificial List of chains containing the Atoms and groups. Does NOT
	 * rotate anything.
	 * 
	 * @param ca
	...
	public static final List<Chain> getAlignedModel(Atom[] ca) {
		...
		for (Atom a : ca) {
			...
			Chain newChain = model.stream().filter(c -> c.getId().equals(parentC.getId())).findFirst().orElse(null);
			if (newChain == null) {
	...
	public static final Structure getAlignedStructure(Atom[] ca1, Atom[] ca2) throws StructureException {
		 ...
		 * 
		 * 
		 * List<Chain>model1 = getAlignedModel(ca1); List<Chain>model2 =
		 * getAlignedModel(ca2); s.addModel(model1); s.addModel(model2);
		 * 
		 * return s;
		 */
	...
	public static Group[] prepareGroupsForDisplay(AFPChain afpChain, Atom[] ca1, Atom[] ca2) throws StructureException {
		...
		if ((afpChain.getAlgorithmName().equals(FatCatRigid.algorithmName))
				|| (afpChain.getAlgorithmName().equals(FatCatFlexible.algorithmName))) {
			...
			// } else if (( blockNum == 1 ) ||
			// (afpChain.getAlgorithmName().equals(CeCPMain.algorithmName))) {
		} else {
	 ...
	 * @param ca2aligned
	 */
	public static void fillAlignedAtomArrays(AFPChain afpChain, Atom[] ca1, Atom[] ca2, Atom[] ca1aligned,
			Atom[] ca2aligned) {
		...
		logger.warn(new StringBuilder()
				...
				.append(pos).append(" but should be ").append(afpChain.getOptLength()).toString());
		// we need to resize the array, because we allocated too many atoms earlier on.
	 ...
	 * equivalent atomic positions of the arrays and remove it from the alignment.
	 * 
	 * @param afpChain original alignment, will be modified
	 * @param ca1      atom array, will not be modified
	 * @param ca2      atom array, will not be modified
	 * @return the original alignment, with the alignment position at the highest
	 *         distance removed
	 * @throws StructureException
	 */
	public static AFPChain deleteHighestDistanceColumn(AFPChain afpChain, Atom[] ca1, Atom[] ca2)
			throws StructureException {
	 ...
	 * @param ca1      atom array, will not be modified
	 * @param ca2      atom array, will not be modified
	 * @param block    block of the alignment position
	 * @param pos      position index in the block
	 * @return the original alignment, with the alignment position removed
	 * @throws StructureException
	 */
	public static AFPChain deleteColumn(AFPChain afpChain, Atom[] ca1, Atom[] ca2, int block, int pos)
			throws StructureException {

		// Check validity of the inputs
		if (afpChain.getBlockNum() <= block) {
			throw new IndexOutOfBoundsException(
					String.format("Block index requested (%d) is higher than the total number of AFPChain blocks (%d).",
	...
	/**
	 * A Map<K,V> can be viewed as a function from K to V. This class represents the
	 * identity function. Getting a value results in the value itself.
	 *
	 * <p>
	 * The class is a bit inconsistent when representing its contents. On the one
	 * hand, containsKey(key) is true for all objects. However, attempting to
	 * iterate through the values returns an empty set.
	 *
	 * @author Spencer Bliven
	 *
	 * @param <K>
