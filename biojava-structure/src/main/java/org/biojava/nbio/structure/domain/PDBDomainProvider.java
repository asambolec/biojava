...
public class PDBDomainProvider implements DomainProvider {
	...
	public SortedSet<String> getDomainNames(String name) {
		...
		String url = String.format("%srepresentativeDomains?cluster=%s&structureId=%s", base, cutoff, name);
	...
	public SortedSet<String> getRepresentativeDomains() {
		String url = new StringBuilder().append(base).append("representativeDomains?cluster=").append(cutoff)
				.toString();
		return requestRepresentativeDomains(url);
	}
		...
		try {
			...
			DefaultHandler handler = new DefaultHandler() {
				...
				public void startElement(String uri, String localName, String qName, Attributes attributes)
					...
					if (!"representative".equalsIgnoreCase(qName)) {
						return;
					}
	...
	/**
	 * Handles fetching and processing REST requests. The actual XML parsing is
	 * handled by the handler, which is also in charge of storing interesting data.
	 * 
	 ...
	 */
	private static void handleRestRequest(String url, DefaultHandler handler)
	...
	public static void main(String[] args) {
		...
		logger.info(new StringBuilder().append("Domains for ").append(name).append(":").toString());
		domains.forEach(logger::info);
