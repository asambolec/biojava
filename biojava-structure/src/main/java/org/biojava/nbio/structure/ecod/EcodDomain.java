...
public class EcodDomain implements Serializable, Cloneable, StructureIdentifier {
	 ...
	 * [X-group].[H-group].[T-group].[F-group] Column 5: PDB identifier Column 6:
	 * Chain identifier (note: case-sensitive) Column 7: PDB residue number range
	 * Column 8: seq_id number range (based on internal PDB indices) Column 9:
	 * Architecture name Column 10: X-group name Column 11: H-group name Column 12:
	 * T-group name Column 13: F-group name (F_UNCLASSIFIED denotes that domain has
	 * not been assigned to an F-group) Column 14: Domain assembly status (if domain
	 * is member of assembly, partners' ecod domain ids listed) Column 15:
	 * Comma-separated value list of non-polymer entities within 4 A of at least one
	 * residue of domain
	 * 
	 * 
	 * 001502751 e4s1gA1 1.1.1 4s1g A A:68-251 beta barrels cradle loop barrel
	 * RIFT-related acid protease F_UNCLASSIFIED NOT_DOMAIN_ASSEMBLY NO_LIGANDS_4A
	 * 
	 */
	...
	public EcodDomain(Long uid, String domainId, Boolean manual, Integer xGroup, Integer hGroup, Integer tGroup,
			Integer fGroup, String pdbId, String chainId, String range, String architectureName, String xGroupName,
			String hGroupName, String tGroupName, String fGroupName, Long assemblyId, Set<String> ligands) {
		this(uid, domainId, manual, xGroup, hGroup, tGroup, fGroup, pdbId, chainId, range, null, architectureName,
				xGroupName, hGroupName, tGroupName, fGroupName, assemblyId, ligands);
	}

	public EcodDomain(Long uid, String domainId, Boolean manual, Integer xGroup, Integer hGroup, Integer tGroup,
			Integer fGroup, String pdbId, String chainId, String range, String seqId, String architectureName,
			String xGroupName, String hGroupName, String tGroupName, String fGroupName, Long assemblyId,
			Set<String> ligands) {
		this.uid = uid;
	...
	/**
	 * @return The assembly ID, or the DomainId if not in an assembly, or null if
	 *         unknown.
	 */
	...
	public String toString() {
		return new StringBuilder().append("EcodDomain [uid=").append(uid).append(", domainId=").append(domainId)
				.append(", manual=").append(manual).append(", xGroup=").append(xGroup).append(", hGroup=")
				.append(hGroup).append(", tGroup=").append(tGroup).append(", fGroup=").append(fGroup).append(", pdbId=")
				.append(pdbId).append(", chainName=").append(chainId).append(", range=").append(range)
				.append(", architectureName=").append(architectureName).append(", xGroupName=").append(xGroupName)
				.append(", hGroupName=").append(hGroupName).append(", tGroupName=").append(tGroupName)
				.append(", fGroupName=").append(fGroupName).append(", assemblyId=").append(assemblyId)
				.append(", ligands=").append(ligands).append("]").toString();
	}
