...
public class CECalculator {
	...
	private void checkBestTraces(AFPChain afpChain, Atom[] ca1, Atom[] ca2) throws StructureException {
		...
		// System.out.println("zStrAlign: " + winSize + " strLen " + strLen + " s/w " +
		// (strLen/winSize) + " " + bestTraceScore + " " + nGaps);
		...
		// for(int k=0; k<nBestTrace; k++)
		// System.out.println(
		// String.format("(%d,%d,%d) ", bestTrace1[k]+1, bestTrace2[k]+1,
		// bestTraceLen[k]));
		//
		//
		//
	...
	/**
	 * On input, mat[i][j] should give the score for aligning positions i and j. On
	 * output, mat[i][j] gives the maximum score possible for aligning 1..i of
	 * protein 1 with 1..j of protein 2.
	 *
	...
	private double dpAlign(int nSeq1, int nSeq2, double gapI, double gapE, boolean isGlobal1, boolean isGlobal2) {
		 ...
		 * for(i=0; i<nSeq1; i++) { printf("\n"); for(j=0; j<nSeq2; j++) { printf("%4d",
		 * (int)(*(mat[i]+j)*10)); } } printf("\n\n\n"); for(i=0; i<nSeq1; i++) {
		 * printf("\n"); for(j=0; j<nSeq2; j++) { printf("%4d", (int)(*(brk_flg[i]+j)));
		 * } } // exit(0);
		 */
			...
			if (!ge) {
				...
				for (k = i + 1; k < nSeq1; k++) {
					if (mat[k][j] - gapI - gapE * (k - i) > sum) {
						// System.out.println("gap1 " + k + " " + j + " " + sum + "<"
						// +(mat[k][j]-gapI-gapE*(k-i)));
						iMax = k;
						jMax = j;
				...
				for (k = j + 1; k < nSeq2; k++) {
					if (mat[i][k] - gapI - gapE * (k - j) > sum) {
						// System.out.println("gap2 " + i + " " + k + " " + sum + "<"+
						// (mat[i][k]-gapI-gapE*(k-j)));
						iMax = i;
						jMax = k;
