...
/**
 * This contains basic categories for Group types. It reflects the
 * categorization used in old PDB file (e.g. for storing whether a residue is
 * composed of ATOM or HETATM records. It is less specific than the
 * mmCIF/PDBx-defined ResidueType enum, which may be more suitable for future
 * applications.
 *
...
public enum GroupType {
	 ...
	 *
	 * <p>
	 * Because mmCIF and PDB handle modified residues differently, some Groups may
	 * have a well-defined ResidueType yet still be HETATMs.
	 * 
	...
	/**
	 * Get ResidueTypes whose polymerType is contained in a certain set. This is
	 * used for defining the AMINOACID and NUCLEOTIDE sets.
	 * 
	 * @param allowedTypes
	 * @return
	 */
	...
	private static Set<ResidueType> getHetatmTypes() {
		...
		for (ResidueType restype : ResidueType.values()) {
			if (!AMINOACID.getResidueTypes().contains(restype) && !NUCLEOTIDE.getResidueTypes().contains(restype)) {
