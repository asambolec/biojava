...
/**
 * Aligns the SEQRES residues to the ATOM residues. The AminoAcids that can be
 * matched between the two of them will be set in the SEQRES chains
 *
 *
...
public class SeqRes2AtomAligner {
	 ...
	 * @param seqRes
	 * @param atomList
	 * @param useChainId if true chainId (Chain.getId) is used for matching, if
	 *                   false chainName (Chain.getName) is used
	 * @return
	 */
	public static Chain getMatchingAtomRes(Chain seqRes, List<Chain> atomList, boolean useChainId) {
		...
		logger.info(new StringBuilder().append("Could not match SEQRES chainID asymId:").append(seqRes.getId())
				.append(" authId:").append(seqRes.getName()).append("  to ATOM chains!, size of atom chain: ")
				.append(atomList.size()).toString());
		return null;
	}
	...
	/**
	 * Map the seqRes groups to the atomRes chain. Updates the atomRes chain object
	 * with the mapped data The seqRes chain should not be needed after this and
	 * atomRes should be further used.
	 *
	 * @param atomRes the chain containing ATOM groups (in atomGroups slot). This
	 *                chain is modified to contain in its seqresGroups slot the
	 *                mapped atom groups
	 * @param seqRes  the chain containing SEQRES groups (in atomGroups slot). This
	...
	public void mapSeqresRecords(Chain atomRes, Chain seqRes) {
		...
		logger.debug("Comparing ATOM {} ({} groups) to SEQRES {} ({} groups) ", atomRes.getId(), atmResGroups.size(),
				seqRes.getId(), seqResGroups.size());
		...
		if (numAminosSeqres < 1) {
			...
			if (numNucleotidesSeqres > 1) {
				...
				logger.debug("SEQRES chain {} is a nucleotide chain ({} nucleotides), aligning nucleotides...",
						seqRes.getId(), numNucleotidesSeqres);

				alignNucleotideChains(seqRes, atomRes);
				return;
			} else {

				logger.debug("SEQRES chain {} contains {} amino acids and {} nucleotides, ignoring...", seqRes.getId(),
						numAminosSeqres, numNucleotidesSeqres);
	...
	/**
	 * A simple matching approach that tries to do a 1:1 mapping between SEQRES and
	 * ATOM records
	 *
	...
	private List<Group> trySimpleMatch(List<Group> seqResGroups, List<Group> atmResGroups) {
		...
		for (Group atomResGroup : atmResGroups) {
			...
			if (startAt1 && seqResPos >= seqResGroups.size()) {
				...
				if (atomResGroup instanceof AminoAcid) {
					logger.debug(
							new StringBuilder().append(" ATOM residue nr: ").append(seqResPos).append(" > seqres! ")
									.append(seqResGroups.size()).append(" ").append(atomResGroup).toString());
					return null;
				} else if (atomResGroup instanceof NucleotideImpl) {
					logger.debug(new StringBuilder().append(" NUCLEOTIDE residue nr: ").append(seqResPos)
							.append(" > seqres! ").append(seqResGroups.size()).append(" ").append(atomResGroup)
							.toString());
					return null;
				} else {
			...
			// if ( seqResPos < 0){
			//
			// System.err.println("What is going on??? " + atomRes.getChainID() + " " +
			// atomResGroup);
			// }
			...
			if (!seqResGroup.getPDBName().trim().equals(atomResGroup.getPDBName().trim())) {
				...
				logger.debug(new StringBuilder().append("Mismatch of SEQRES pos ").append(seqResPos)
						.append(" and ATOM record: ").append(atomResGroup).append(" | ").append(seqResGroup)
						.toString());
				return null;
			}
			...
			logger.debug("Merging index {}: replaced seqres group {} ({}) with atom group {} ({})", seqResPos,
					replacedGroup.getResidueNumber(), replacedGroup.getPDBName(), atomResGroup.getResidueNumber(),
					atomResGroup.getPDBName());
	...
	/**
	 * Returns the full sequence of the Atom records of a parent with X instead of
	 * HETATMSs. The advantage of this is that it allows us to also align HETATM
	 * groups back to the SEQRES.
	 * 
	 * @param groups            the list of groups in a parent
	 * @param positionIndex     a Map to keep track of which group is at which
	 *                          sequence position
	 * @param isNucleotideChain whether the atom groups are predominantly
	 *                          nucleotides (the groups represent a nucleotide
	 *                          chain), if true non-standard nucleotides will be
	 *                          represented with ambiguous letter 'N' instead of
	 *                          'X', if false all non-standard residues will be 'X'
	...
	public static String getFullAtomSequence(List<Group> groups, Map<Integer, Integer> positionIndex,
		...
		for (int i = 0; i < groups.size(); i++) {
			...
			if (g instanceof AminoAcid) {
				...
				if (ResidueType.lPeptideLinking == cc.getResidueType()
					...
					if ("?".equals(c)) {
						if (isNucleotideChain && PolymerType.POLYNUCLEOTIDE_ONLY.contains(cc.getPolymerType())) {
							// the way to represent unknown nucleotides is with 'N', see
					...
					// For some unusual cases the het residue can map to 2 or more standard aas and
					// thus give an
					...
					logger.debug("Group {} is not lPeptideLinked, nor PROTEIN_ONLY, nor POLYNUCLEOTIDE_ONLY",
							g.toString());
					continue;
				}
	...
	private boolean alignNucleotideGroups(List<Group> seqRes, List<Group> atomRes) {
		...
		logger.debug(
				new StringBuilder().append("align seq1 (").append(seq1.length()).append(") ").append(seq1).toString());
		logger.debug(
				new StringBuilder().append("align seq2 (").append(seq2.length()).append(") ").append(seq2).toString());
		...
		PairwiseSequenceAligner<Sequence<NucleotideCompound>, NucleotideCompound> smithWaterman = Alignments
				.getPairwiseAligner(s1, s2, PairwiseSequenceAlignerType.LOCAL, penalty, matrix);
		...
		try {
			...
			try {
				...
				try {
					...
					logger.warn(
							"Could not determine compound set (neither DNA, RNA nor hybrid) for nucleotide sequence "
									+ seq);
					return null;
				}
	...
	/**
	 * Aligns two chains of groups, where the first parent is representing the list
	 * of amino acids as obtained from the SEQRES records, and the second parent
	 * represents the groups obtained from the ATOM records (and containing the
	 * actual ATOM information). This does the actual alignment and if a group can
	 * be mapped to a position in the SEQRES then the corresponding position is
	 * replaced with the group that contains the atoms.
	 *
	 * @param seqRes
	 * @param atomRes
	...
	private boolean alignProteinChains(List<Group> seqRes, List<Group> atomRes) {
		...
		logger.debug(new StringBuilder().append("Protein seq1 to align (length ").append(seq1.length()).append("): ")
				.append(seq1).toString());
		logger.debug(new StringBuilder().append("Protein seq2 to align (length ").append(seq2.length()).append("): ")
		...
		try {
			...
			logger.warn(
					"Could not create protein sequences ({}) to align ATOM and SEQRES groups, they will remain unaligned.",
					e.getMessage());
			return true;
		}
		...
		// sequences that are only X (e.g. 1jnv chain A) produced empty alignments,
		// because nothing aligns to nothing and thus the local alignment is empty
	...
	private boolean mapChains(List<Group> seqResGroups, List<Group> atomRes,
			SequencePair<ProteinSequence, AminoAcidCompound> pair, Map<Integer, Integer> seqresIndexPosition,
			Map<Integer, Integer> atomIndexPosition) {

		// at the present stage the future seqRes are still stored as Atom groups in the
		// seqRes parent...
		...
		mainLoop: for (int i = 1; i <= aligLength; i++) {
			...
			if (s.equals(a)) {
				...
				if (s1 == null || a1 == null) {
					...
					logger.warn(new StringBuilder().append("can't map ").append(i).append(":").append(s).append(" ")
							.append(posSeq).append(" ").append(s1).append(" atom: ").append(posAtom).append(" ")
							.append(a1).toString());
					continue mainLoop;
				}
				...
				if (pdbNameS == null || pdbNameA == null) {
					logger.warn("null value for group.getPDBName found at {} when trying to align {} and {} {}", posSeq,
							s1, a1, posAtom);
				...
				if (!pdbNameA.trim().equals(pdbNameS.trim())) {
					...
					String msg = new StringBuilder().append("'").append(s1).append("' (position ").append(posSeq)
							.append(") does not align with '").append(a1).append("' (position ").append(posAtom)
							.append("), should be: ").append(s).append(" : ").append(a).toString();

					if (s1.getType() == HetatomImpl.type && a1.getType() == HetatomImpl.type) {
	...
	private boolean mapDNAChains(List<Group> seqResGroups, List<Group> atomRes,
			...
			Map<Integer, Integer> seqresIndexPosition, Map<Integer, Integer> atomIndexPosition) {
		...
		mainLoop: for (int i = 1; i <= aligLength; i++) {
			...
			if (s.equals(a)) {
				...
				String pdbNameA = a1.getPDBName();
				if (pdbNameS == null || pdbNameA == null) {
					logger.warn(new StringBuilder().append("null value found for group.getPDBName() at ").append(posSeq)
							.append(" when trying to align ").append(s1).append(" and ").append(a1).append(" ")
							.append(posAtom).toString());
					logger.warn("ATOM and SEQRES sequences will not be aligned.");
				}
				if (!pdbNameA.equals(pdbNameS)) {
					if (!pdbNameA.trim().equals(pdbNameS.trim())) {
						logger.info(new StringBuilder().append(s1).append(" ").append(posSeq)
								.append(" does not align with ").append(a1).append(" ").append(posAtom)
								.append(" should be: ").append(s).append(" : ").append(a).toString());
						if (s1.getType() == HetatomImpl.type && a1.getType() == HetatomImpl.type) {
							...
							logger.warn(new StringBuilder().append("Could not match residues ").append(s1).append(" ")
									.append(a1).toString());
						}
		...
		if (headerOnly) {
			...
			for (int i = 0; i < structure.nrModels(); i++) {
				...
				seqResChains.forEach(seqRes -> {
					...
					// TODO no idea if new parameter useChainId should be false or true here, used
					// true as a guess - JD 2016-05-09
