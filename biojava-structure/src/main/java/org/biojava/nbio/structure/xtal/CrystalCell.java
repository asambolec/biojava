...
public class CrystalCell implements Serializable {
	...
	// LoggerFactory.getLogger(CrystalCell.class);
	...
	/**
	 * Returns the volume of this unit cell. See
	...
	public double getVolume() {
		...
		volume = a * b * c
				* Math.sqrt(1 - Math.cos(alphaRad) * Math.cos(alphaRad) - Math.cos(betaRad) * Math.cos(betaRad)
	 ...
	 * Points in the unit cell one unit further along the `a` axis will return
	 * (1,0,0), etc.
	 * 
	...
	/**
	 * Converts the coordinates in pt so that they occur within the (0,0,0) unit
	 * cell
	 * 
	 * @param pt
	 */
	 ...
	 *
	 * This is useful to transform a whole chain at once, allowing some of the atoms
	 * to be outside the unit cell, but forcing the centroid to be within it.
	 *
	 * @param points    A set of points to transform (in orthonormal coordinates)
	 * @param reference The reference point, which is unmodified but which would be
	 *                  in the unit cell were it to have been transformed. It is
	 *                  safe to use a member of the points array here.
	 */
	 ...
	 * @param reference Reference point, which should be in the unit cell after each
	 *                  operation (also in orthonormal coordinates)
	 ...
	 * @param reference Reference point, which should be in the unit cell after each
	 *                  operation (also in crystal coordinates)
	 * @return A set of crystal operators with equivalent rotation to the inputs,
	 *         but with translation such that the reference point would fall within
	 *         the unit cell
	 */
	...
	/**
	 * Transform given Matrix4d in crystal basis to the orthonormal basis using the
	 * PDB axes convention (NCODE=1)
	 * 
	 * @param m
	 * @return
	 */
	 ...
	 * e.g. transfToOrthonormal(new Point3d(1,1,1)) returns the orthonormal
	 * coordinates of the vertex of the unit cell. See Giacovazzo section 2.E, eq.
	 * 2.E.1 (or any linear algebra manual)
	 * 
	 * @param v
	 */
	...
	/**
	 * Transform given Matrix4d in orthonormal basis to the crystal basis using the
	 * PDB axes convention (NCODE=1)
	 * 
	 * @param m
	 * @return
	...
	/**
	 * Returns the change of basis (crystal to orthonormal) transform matrix, that
	 * is M inverse in the notation of Giacovazzo. Using the PDB axes convention
	 * (CCP4 uses NCODE identifiers to distinguish the different conventions, the
	 * PDB one is called NCODE=1) The matrix is only calculated upon first call of
	 * this method, thereafter it is cached. See "Fundamentals of Crystallography"
	 * C. Giacovazzo, section 2.5 (eq 2.30)
	 *
	 * The non-standard orthogonalisation codes (NCODE for ccp4) are flagged in
	 * REMARK 285 after 2011's remediation with text: "THE ENTRY COORDINATES ARE NOT
	 * PRESENTED IN THE STANDARD CRYSTAL FRAME". There were only 148 PDB entries
	 * with non-standard code in 2011. See:
	 * http://www.wwpdb.org/documentation/2011remediation_overview-061711.pdf The
	 * SCALE1,2,3 records contain the correct transformation matrix (what Giacovazzo
	 * calls M matrix). In those cases if we calculate the M matrix following
	 * Giacovazzo's equations here, we get an entirely wrong one. Examples of PDB
	 * with non-standard orthogonalisation are 1bab and 1bbb.
	 * 
	 * @return
	 */
	private Matrix3d getMInv() {
		...
		minv = new Matrix3d(this.a, 0, 0, this.b * Math.cos(gammaRad), this.b * Math.sin(gammaRad), 0,
				this.c * Math.cos(betaRad), -this.c * Math.sin(betaRad) * getCosAlphaStar(), 1.0 / getCstar());
		return minv;
	}

	// another axes convention (from Giacovazzo) eq. 2.31b, not sure what would be
	// the NCODE of this
	// private Matrix3d getMInv() {
	// if (Minv!=null) {
	// return Minv;
	// }
	// Minv = new Matrix3d( 1/getAstar(),
	// -(getCosGammaStar()/getSinGammaStar())/getAstar(), this.a*Math.cos(betaRad),
	// 0, 1/(getBstar()*getSinGammaStar()), this.b*Math.sin(alphaRad),
	// 0, 0, this.c);
	// return Minv;
	// }

	// and yet another axes convention (from Giacovazzo) eq. 2.31c, not sure what
	// would be the NCODE of this
	// private Matrix3d getMInv() {
	// if (Minv!=null) {
	// return Minv;
	// }
	// Minv = new Matrix3d( alphaRad*Math.sin(gammaRad)*getSinBetaStar(),
	// this.a*Math.cos(gammaRad), this.a*Math.sin(gammaRad)*getCosBetaStar(),
	// 0, this.b, 0,
	...
	private double getCosBetaStar() {
		return (Math.cos(alphaRad) * Math.cos(gammaRad) - Math.cos(betaRad))
	...
	/**
	 * Returns the change of basis (orthonormal to crystal) transform matrix, that
	 * is M in the notation of Giacovazzo. Using the PDB convention (NCODE=1). The
	 * matrix is only calculated upon first call of this method, thereafter it is
	 * cached. See "Fundamentals of Crystallography" C. Giacovazzo, section 2.5
	 * 
	 * @return
	 */
	...
	/**
	 * Given a scale matrix parsed from the PDB entry (SCALE1,2,3 records), checks
	 * that the matrix is a consistent scale matrix by comparing the cell volume to
	 * the inverse of the scale matrix determinant (tolerance of 1/100). If they
	 * don't match false is returned. See the PDB documentation for the SCALE
	 * record. See also last equation of section 2.5 of "Fundamentals of
	 * Crystallography" C. Giacovazzo
	 * 
	 * @param scaleMatrix
	 * @return
	 */
	public boolean checkScaleMatrixConsistency(Matrix4d scaleMatrix) {
		...
		// note we need to have a relaxed tolerance here as the PDB scale matrix is
		// given with not such high precision
		...
		if ((Math.abs(vol - 1.0 / m.determinant()) > tolerance)) {
			// System.err.println("Warning! SCALE matrix from PDB does not match
			// 1/determinat == cell volume: "+
		...
		// System.err.println("Our calculated SCALE matrix does not match 1/det=cell
		// volume");
		// }
	...
	/**
	 * Given a scale matrix parsed from a PDB entry (SCALE1,2,3 records), compares
	 * it to our calculated Mtranspose matrix to see if they coincide and returns
	 * true if they do. If they don't that means that the PDB entry is not in the
	 * standard orthogonalisation (NCODE=1 in ccp4). In 2011's remediation only 148
	 * PDB entries were found not to be in a non-standard orthogonalisation. See:
	 * http://www.wwpdb.org/documentation/2011remediation_overview-061711.pdf For
	 * normal cases the scale matrix is diagonal without a translation component.
		...
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				if (!deltaComp(mtranspose.getElement(i, j), scaleMatrix.getElement(i, j))) {
					...
					// System.out.println("Their value ("+i+","+j+"):
					// "+scaleMatrix.getElement(i,j));
					return false;
				}
			}
		}
	 ...
	 * crystallography: if all 3 dimensions are below {@value #MIN_VALID_CELL_SIZE}
	 * the cell is considered unrealistic and false returned
	 * 
	 * @return
	 */
	public boolean isCellReasonable() {
		// this check is necessary mostly when reading PDB files that can contain the
		// default 1 1 1 crystal cell
		// if we use that further things can go wrong, for instance for interface
		// calculation
		// For instance programs like coot produce by default a 1 1 1 cell

		if (this.getA() < MIN_VALID_CELL_SIZE && this.getB() < MIN_VALID_CELL_SIZE
