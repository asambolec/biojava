...
public class AFPChainer {
	...
	/**
	 * // Key function: chain (assembly) the AFPs // a AFP (k) is defined as (i, j,
	 * k), with i and j are staring points // AFP extension (eg. AFP(k-1) -> AFP(k)
	 * ) requiring // AFP(k-1) < AFP(k)(refer AFP.h definition), // ie i(k-1) < i(k)
	 * and j(k-1) < j(k) // in the figure, only (2) AFP can extend to AFP(k) // Key
	 * features: a coordination transformation is allowed in the AFP extension //
	 * gap penalties are also considered // // protein1 //
	 * --------------------------- // | \ | // | \(1) | // | \ \ | // | \(2) \ | //
	 * p | \ | // r | \ | // o | \(3) \(i,j, k) | // t | \ \ | // e | \ | // i | |
	 * // n | | // 2 --------------------------- // schematic of AFP chaining
	 */
	public static void doChainAfp(FatCatParameters params, AFPChain afpChain, Atom[] ca1, Atom[] ca2) {
		...
		int currafp = maxafp;
		if (debug) {
			logger.info(String.format("maximum score %f, %d%n", maxsco, twi[currafp]));
		}
	...
	/*
	 * 
	 * derive the compabitle AFP lists for AFP-chaining this is important for
	 * speeding up the process for a given AFP(i1,j1), there are three regions that
	 * could be the starting point for the compabitle AFPs of AFP(i1,j1) // a1 a2 a3
	 * // i1-G i1-f-c i1-f+1 i1 // | | | | // ---------------------------- // | B |
	 * | //b1 j1-G -| ---------------| | // | | | | | // | | C | 3 | | // | | | | |
	 * //b2 j1-f-c -| |--------------| | // | | 2 | 1 | | //b3 j1-f+1
	 * -|------------------ | // | A | // j1 -|---------------------\ // | \
	 * (AFP(i1,j1)) // ----------------------------- // f: the length of AFPs (we
	 * use segments of same length) G: g + f, where g is the maximum allowed gaps c:
	 * the maximum allowed cross-over in AFP-connection, here we use c = f, and
	 * j1-f-c = j1-2f incompatible region A: its AFPs overlap with given AFP(i1,j1)
	 * incompatible region B: the gaps between its AFP with given AFP is larger than
	 * g incompatible region C: its AFPs connect with given AFP but cross a given
	 * threshold. compatible region 1: [i1-f-c,i1-f+1>,[j1-f-c,j1-f+1> or
	 * [a2,a3],[b2,b3] compatible region 2: [i1-G,i1-f-c],[j1-f-c,j1-f] or
	 * [a1,a2],[b2,b3] combine 1 and 2 : [i1-G,i1-f],[j1-f-c,j1-f] or
	 * [a1,a3],[b2,b3] compatible region 3: [i1-f-c,i1-f],[j1-G,j1-f-c] or
	 * [a2,a3],[b1,b2] c->misCut f->fragLen G->fragLen+maxGap->maxGapFrag
	 *
	 *
	 */
	private static int getCompatibleAfps(int afp, int[] list, FatCatParameters params, AFPChain afpChain) {
		...
		for (i = a1; i <= a3; i++) {// i <= a3 instead of i < a3
			...
			if (s1 < 0) {
				continue;// no AFP for the given i with j > b2
			}
			s2 = afpBefIndex[i][b3]; // afps is sorted by j given a i,it's sparse matrix
			if (s2 < 0) {
				continue;// no AFP for the given i with j < b3
			}
	...
	/**
	 * //Key function: calculate the connectivity of AFP pairs //no compatibility
	 * criteria is executed //note: afp1 is previous to afp2 in terms of the
	 * position //this module must be optimized
	 *
	 * @param afp1
	 * @param afp2
	...
	/**
	 * return the gaps between this and afp requiring afp1 > afp2 ( operator % in C)
	 */
	...
	/**
	 * return the mis-matched between this and afp requiring AFP1 > afp2 (operator /
	 * in C)
	 * 
	 * @param afp1
	 * @param afp2
	 ...
	 * //from the segments that form the given AFP list //this value can be a
	 * measurement (2) for the connectivity of the AFPs //and its calculation is
	 * quicker than the measurement (1), rmsd //currently only deal with AFP pair //
	 * // |-d1--| // |--d2---| // |---d3----|
	 * //-----------------------------------------------------------------------
	 ...
	 */
	private static void traceBack(int[] pre, int currafp0, int twist, FatCatParameters params, AFPChain afpChain,
			Atom[] ca1, Atom[] ca2) {
		...
		if (afpChainTwiNum != twist) {
			logger.error(String.format("AFPChainer Warning: the twists number is not consistent %d %d%n",
		...
		if (Math.abs(checkscore - alignScore) > 1e-4) {
			logger.error(String.format("AFPChainer Warning: fail in alignment score checking %.4f %.4f%n", alignScore,
					checkscore));
		}

		if (debug) {
			logger.info(new StringBuilder().append("traceBack:").append(afpChainLen).append(" ")
					.append(afpChainList.length).toString());
		}
		double rmsd = calAfpRmsd(afpChainLen, afpChainList, 0, afpChain, ca1, ca2);
		afpChain.setChainRmsd(rmsd);
		if (debug) {
			logger.info("Chain RMSD: " + rmsd);
		}
		int b1 = 0;
		int bk = 0;
		int a;
		...
		for (i = 0; i < afpChainLen; i++) {
			...
			if (afpChainTwiBin[i] == 1) {
				if (debug) {
					logger.info(new StringBuilder().append(" ** calAfpTmsd : afpChainWtiBin == 1 : i: ").append(i)
							.append(" i-b1: ").append(i - b1).append(" b1: ").append(b1).append(" afpChainList.len: ")
							.append(afpChainList.length).toString());
				}
				...
				// }
				if (debug) {
					logger.error(new StringBuilder().append("calculation calAfpRmsd ").append(i).append(" ").append(b1)
							.append(" ").toString());
				}
				...
				// System.out.println("block2Afp.length:"+ block2Afp.length + " " + bk + " " + i
				// + " " + afpChain.getMaxTra() );
		...
		if (debug) {
			logger.info(new StringBuilder().append("after loop over all afpChainList ").append(i - b1).append(" ")
					.append(b1).toString());
		}

		rmsd = calAfpRmsd(i - b1, afpChainList, b1, afpChain, ca1, ca2);
		if (debug) {
			logger.info(new StringBuilder().append("*** i:").append(i).append(" b1: ").append(b1).append(" i-b1 ")
					.append(i - b1).toString());
		}
	...
	/**
	 * //return the rmsd of the residues from the segments that form the given AFP
	 * list //this value can be a measurement (1) for the connectivity of the AFPs
	 *
	 * @param afpn
	...
	protected static double calAfpRmsd(int afpn, int[] afpPositions, int listStart, AFPChain afpChain, Atom[] ca1,
			Atom[] ca2) {

		if (debug) {
			logger.info(new StringBuilder().append("XXX calling afp2res ").append(afpn).append(" ")
		...
		if (debug) {
			logger.info(new StringBuilder().append("XXX calculating calAfpRmsd: ").append(focusResn).append(" ")
					.append(focusRes1.length).append(" ").append(focusRes2.length).append(" ")
					.append(afpChain.getMinLen()).append(" ").toString());
		}
	 ...
	 * @return
	 */
	private static double getRmsd(int focusResn, int[] focusRes1, int[] focusRes2, AFPChain afpChain, Atom[] ca1,
			Atom[] ca2) {
		...
		for (int i = 0; i < focusResn; i++) {
			...
			if (tmp1[i].getCoords() == null) {
				logger.error(new StringBuilder().append("tmp1 got null: ").append(i).append(" pos: ")
						.append(focusRes1[i]).toString());
			}
			if (tmp2[i].getCoords() == null) {
				logger.error(new StringBuilder().append("tmp1 got null: ").append(i).append(" pos: ")
						.append(focusRes2[i]).toString());
			}
			// XX
	...
	/**
	 * Calculate the RMSD for two sets of atoms. Rotates the 2nd atom set so make
	 * sure this does not cause problems later
	 *
	 *
	 * @param catmp1
	 * @return
