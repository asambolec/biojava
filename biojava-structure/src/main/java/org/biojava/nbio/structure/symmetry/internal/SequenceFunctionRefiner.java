 ...
 * Needs the order of symmetry and assumes that the last repeat aligns with the
 * first, being thus a CLOSE symmetry.
 *
...
public class SequenceFunctionRefiner implements SymmetryRefiner {
	 ...
	 *
	 * The resulting alignment will have a one-to-one correspondance between aligned
	 * residues of each symmetric part.
	 *
	 * @param afpChain Input alignment from CE-Symm
	 * @param k        Symmetry order. This can be guessed by
	 *                 {@link CeSymm#getSymmetryOrder(AFPChain)}
	...
	public static Map<Integer, Integer> refineSymmetry(Map<Integer, Integer> alignment, int k)
		...
		/*
		 * For future heap implementation Comparator<Integer> scoreComparator = new
		 * Comparator<Integer>() {
		 * 
		 * @Override public int compare(Integer o1, Integer o2) {
		 * if(scores.containsKey(o1)) { if(scores.containsKey(o2)) { // If both have
		 * defined scores, compare the scores return
		 * scores.get(o1).compareTo(scores.get(o2)); } else { // o2 has infinite score,
		 * so o1 < o2 return -1; } } else { //o1 has infinite score
		 * if(scores.containsKey(o2)) { // o1 > o2 return 1; } else { //both undefined
		 * return 0; } } } }; PriorityQueue<Integer> heap = new
		 * PriorityQueue<Integer>(alignment.size(), scoreComparator);
		 */
		// int step = 0;
		while (!eligible.isEmpty()) {
			// System.out.format("Step %d: %s%n", ++step,
			// AlignmentTools.toConciseAlignmentString(alignment));
			...
			// System.out.format("Modifying %d -> %d. %d now eligible.%n",
			// resK1,bestRes,eligible.size());
		}
		// System.out.format("Step %d: %s%n", ++step,
		// AlignmentTools.toConciseAlignmentString(alignment));
	 ...
	 *
	 * Eligible if: 1. score(x)>0 2. f^K-1(x) is defined 3. score(f^K-1(x))>0 4. For
	 * all y, score(y)==0 implies sign(f^K-1(x)-y) == sign(x-f(y) )
	 * 
	 * @param alignment     The alignment with respect to which to calculate
	 *                      eligibility
	 * @param scores        An up-to-date map from residues to their scores
	 * @param eligible      Starting list of eligible residues. If null will be
	 *                      generated.
	 * @param k
	 * @param backwardLoops
	 * @param forwardLoops
	 ...
	 */
	private static List<Integer> initializeEligible(Map<Integer, Integer> alignment, Map<Integer, Double> scores,
			List<Integer> eligible, int k, NavigableSet<Integer> forwardLoops, NavigableSet<Integer> backwardLoops) {
		// Eligible if:
		// 1. score(x)>0
		// 2. f^K-1(x) is defined
		// 3. score(f^K-1(x))>0
		...
		// Map<Integer, Integer> alignK1 = AlignmentTools.applyAlignment(alignment,
		// k-1);
		...
		while (eligibleIt.hasNext()) {
			...
			// 4. For all y, score(y)==0 implies sign(f^K-1(x)-y) == sign(x-f(y) )
			// Test equivalent: All loop edges should be properly ordered wrt edge f^k-1(x)
			// -> x
	...
	/**
	 * Like {@link AlignmentTools#applyAlignment(Map, int)}, returns a map of k
	 * applications of alignmentMap. However, it also sets loops of size less than k
	 * as ineligible.
	 *
	 * @param alignmentMap f(x)
	 * @param k
	 * @param eligible     Eligible residues. Residues from small cycles are
	 *                     removed.
	 * @return f^k(x)
	 */
	private static Map<Integer, Integer> applyAlignmentAndCheckCycles(Map<Integer, Integer> alignmentMap, int k,
	 ...
	 * @return scores
	 */
	private static Map<Integer, Double> initializeScores(Map<Integer, Integer> alignment, Map<Integer, Double> scores,
			int k) {
		if (scores == null) {
	...
	/**
	 * Calculate the score for a residue, specifically the Absolute Error score(x) =
	 * |x-f^k(x)|
	 *
	 ...
	 * @throws StructureException
	 */
	private static AFPChain partitionAFPchain(AFPChain afpChain, Atom[] ca1, Atom[] ca2, int order)
			throws StructureException {
