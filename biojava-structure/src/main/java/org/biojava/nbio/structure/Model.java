...
/**
 * An internal utility class for StructureImpl to make it easier to manage poly
 * and nonpoly chains. Not to exposed to users through API.
 *
...
public class Model implements Serializable {
	...
	public void setChains(List<Chain> modelChains) {
		...
		modelChains.forEach(this::addChain);
	}
	...
	public void addChain(Chain c) {
		...
		if (info == null || info.getType() == null) {
			logger.info(
					"No entity info could be found while adding chain with asym id {} (author id {}). Will consider it a polymer chain.",
					c.getId(), c.getName());
			polyChains.add(c);
			...
			logger.warn(
					"Chain with asym id {} (author id {}) has entity type 'macrolide', considering it non-polymeric",
					c.getId(), c.getName());
			nonPolyChains.add(c);

		} else {
			logger.warn(
					"Chain with asym id {} (author id {}) has unsupported entity type '{}'. Will not add it to the Structure.",
					c.getId(), c.getName(), info.getType().toString());
			// ignore it
	...
	/**
	 * Returns the total number of chains in this model: polymeric, non-polymeric
	 * and water
	 * 
	 * @return
	 */
	...
	public String toString() {
		return new StringBuilder().append("[").append(polyChains.size()).append(" poly chains, ")
				.append(nonPolyChains.size()).append(" non-poly chains, ").append(waterChains.size())
				.append(" water chains]").toString();
	}
}
