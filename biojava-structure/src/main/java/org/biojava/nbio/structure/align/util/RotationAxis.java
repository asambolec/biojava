...
public final class RotationAxis {
	 ...
	 * @throws NullPointerException if afpChain does not contain a valid rotation
	...
	/**
	 * Determine the location of the rotation axis based on a rotation matrix and a
	 * translation vector
	 * 
	 * @param rotation
	 * @param translation
	...
	/**
	 * Create a rotation axis from a Matrix4d containing a rotational component and
	...
	/**
	 * Returns the rotation axis and angle in a single javax.vecmath.AxisAngle4d
	 * object
	 * 
	 * @return
	 */
	...
	public Matrix getRotationMatrix(double theta) {
		...
		double com = 1 - cos;
		return new Matrix(new double[][] { { com * x * x + cos, com * x * y + sin * z, com * x * z + -sin * y },
	...
	/**
	 * Calculate the rotation axis for the normal case, where there is a significant
	 * rotation angle
	 * 
	 * @param rotation
	 * @param translation
	...
	private void calculateRotationalAxis(Matrix rotation, Atom translation, double c) {
		...
		// Only use biggest d for the sign directly, for numerical stability around
		// 180deg
	...
	/**
	 * Returns a Jmol script which will display the axis of rotation. This consists
	 * of a cyan arrow along the axis, plus an arc showing the angle of rotation.
	 * <p>
	 * As the rotation angle gets smaller, the axis of rotation becomes poorly
	 * defined and would need to get farther and farther away from the protein. This
	 * is not particularly useful, so we arbitrarily draw it parallel to the
	 * translation and omit the arc.
	 * 
	 * @param atoms Some atoms from the protein, used for determining the bounds of
	 *              the axis.
	 *
	 ...
	 * points.
	 * <p>
	 * In the case of a pure translational axis, the axis location is undefined so
	...
	public Pair<Atom> getAxisEnds(Atom[] atoms) {
		...
		// Project the origin onto the axis. If the axis is undefined, use the center of
		// mass
		Atom axialPt;
		if (rotationPos == null) {
	...
	public String getJmolScript(Atom[] atoms, int axisID) {
		...
		result.append(String.format(
				new StringBuilder().append("draw ID rot").append(axisID)
						.append(" CYLINDER {%f,%f,%f} {%f,%f,%f} WIDTH %f COLOR %s ;").toString(),
				axisMin.getX(), axisMin.getY(), axisMin.getZ(), axisMax.getX(), axisMax.getY(), axisMax.getZ(), width,
				axisColor));
		...
		if (positiveScrew) {
			...
			result.append(String.format(
					new StringBuilder().append("draw ID screw").append(axisID)
							.append(" VECTOR {%f,%f,%f} {%f,%f,%f} WIDTH %f COLOR %s ;").toString(),
					axisMax.getX(), axisMax.getY(), axisMax.getZ(), screwTranslation.getX(), screwTranslation.getY(),
			...
			result.append(String.format(
					new StringBuilder().append("draw ID screw").append(axisID)
							.append(" VECTOR {%f,%f,%f} {%f,%f,%f} WIDTH %f COLOR %s ;").toString(),
					axisMin.getX(), axisMin.getY(), axisMin.getZ(), screwTranslation.getX(), screwTranslation.getY(),
					screwTranslation.getZ(), width, screwColor));
		}
		...
		if (rotationPos != null) {
			...
			result.append(String.format(new StringBuilder().append("draw ID rotArc").append(axisID)
					.append(" ARC {%f,%f,%f} {%f,%f,%f} {0,0,0} {0,%f,%d} SCALE 500 DIAMETER %f COLOR %s;").toString(),
					axisMin.getX(), axisMin.getY(), axisMin.getZ(), axisMax.getX(), axisMax.getY(), axisMax.getZ(),
					Math.toDegrees(theta), positiveScrew ? 0 : 1, // draw at the opposite end from the screw arrow
					width, axisColor));
		}
	 ...
	 * 
	 * @param point
	 * @return An atom which lies on the axis, or null if the RotationAxis is purely
	 *         translational
	 */
	 ...
	 * @throws NullPointerException If the alignment doesn't have a rotation matrix
	 *                              set
	 */
