 ...
 *
 * Alternative alignments arise from different seed alignments or seed FPairs.
 * The AltAlg class contains methods for refinement (Dynamic Programming based)
 * and filtering (i.e. removing probably wrongly matched APairs). In the
 * refinement phase, different seed alignments can converge to the same
 * solution.
 *
...
public class AlternativeAlignment implements Serializable {
	...
	public String toString() {
		...
		s.append(new StringBuilder().append("#").append(getAltAligNumber()).append(" cluster:").append(cluster)
				.append(" eqr:").append(getEqr()).append(" rmsd:").append(d2.format(getRmsd())).append(" %id:")
				.append(getPercId()).append(" gaps:").append(getGaps()).append(" score:").append(d2.format(score))
				.toString());
	...
	/**
	 * set the number of the cluster this alignment belongs to. All alignments in a
	 * cluster are quite similar.
	 *
	...
	public void finish(StrucAligParameters params, Atom[] ca1, Atom[] ca2) throws StructureException {
		...
		calcScores(ca1, ca2);
		logger.debug(new StringBuilder().append("eqr ").append(eqr0).append(" ").append(gaps0).append(" ")
				.append(idx1[0]).append(" ").append(idx1[1]).toString());
	...
	/*
	 * private Alignable getAlignableFromSimMatrix (Matrix sim,StrucAligParameters
	 * params){ //System.out.println("align_NPE");
	 * 
	 * float gapOpen = params.getGapOpen(); float gapExtension =
	 * params.getGapExtension();
	 * 
	 * int rows = sim.getRowDimension(); int cols = sim.getColumnDimension();
	 * 
	 * Alignable al = new StrCompAlignment(rows,cols);
	 * al.setGapExtCol(gapExtension); al.setGapExtRow(gapExtension);
	 * al.setGapOpenCol(gapOpen); al.setGapOpenRow(gapOpen);
	 * //System.out.println("size of aligmat: " + rows+1 + " " + cols+1);
	 * //AligMatEl[][] aligmat = new AligMatEl[rows+1][cols+1]; AligMatEl[][]
	 * aligmat = al.getAligMat();
	 * 
	 * for (int i = 0; i < rows; i++) { for (int j = 0; j < cols; j++) {
	 * 
	 * int e=0; //if ( ( i < rows) && // ( j < cols)) { //TODO: the ALIGFACTOR calc
	 * should be hidden in Gotoh!!
	 * 
	 * e = (int)Math.round(Gotoh.ALIGFACTOR * sim.get(i,j)); //}
	 * //System.out.println(e); //AligMatEl am = new AligMatEl();
	 * aligmat[i+1][j+1].setValue(e); //.setValue(e); //am.setTrack(new
	 * IndexPair((short)-99,(short)-99)); //igmat[i+1][j+1] = am;
	 * 
	 ...
	 */
	/**
	 * Refinement procedure based on superposition and dynamic programming. Performs
	 * an iterative refinement. Several methods apply such a procedure, e.g. CE or
	 * ProSup. Here we additionally test for circular permutation, which are in the
	 * same frame of superposition as the optimal alignment. This feature may be
	 * switched off by setting permsize to -1.
	 *
	...
	public void refine(StrucAligParameters params, Atom[] ca1, Atom[] ca2) throws StructureException {
		// System.out.println("refine Alternative Alignment #"+ getAltAligNumber()+"
		// l1:" + ca1.length + " l2:" + ca2.length);
		...
		for (int iter = 0; iter < maxiter; iter++) {
			...
			if (permsize > 0 && (quada_end - quada_beg >= permsize) && (quadb_end - quadb_beg >= permsize)) {
				...
				// System.out.println(quada_beg + " " + quada_end + " " +quadb_beg+" " +
				// quadb_end);
			...
			// if ( j > 0)
			// System.out.println("do new superimpos " + idx1.length + " " + idx2.length + "
			// " + idx1[0] + " " + idx2[0]);
		...
		// new ... get rms...
		// now use the original atoms to get the rotmat relative to the original
		// structure...
		...
		getPdbRegions(ca1, ca2);
		// System.out.println("eqr " + eqr0 + " aligpath len:"+aligpath.length+ " gaps:"
		// + gaps0 + " score " + score);
	}
	 ...
	 * Does not change the original coordinates. as an internal result the rotation
	 * matrix and shift vectors for are set
	 *
	...
	private void super_pos_alig(Atom[] ca1, Atom[] ca2, int[] idx1, int[] idx2, boolean getRMS)
		...
		Matrix4d trans = SuperPositions.superpose(Calc.atomsToPoints(ca1subset), Calc.atomsToPoints(ca2subset));
	...
	/**
	 * returns the rotation matrix that needs to be applied to structure 2 to rotate
	 * on structure 1
	 *
	...
	/**
	 * returns the shift vector that has to be applied on structure to to shift on
	 * structure one
	 *
	...
	/**
	 * create an artifical Structure object that contains the two structures
	 * superimposed onto each other. Each structure is in a separate model. Model 1
	 * is structure 1 and Model 2 is structure 2.
	 *
	 * @param s1 the first structure. its coordinates will not be changed
	 * @param s2 the second structure, it will be cloned and the cloned coordinates
	 *           will be rotated according to the alignment results.
	 * @return composite structure containing the 2 aligned structures as a models 1
	 *         and 2
	 */
	...
	/**
	 * converts the alignment to a PDB file each of the structures will be
