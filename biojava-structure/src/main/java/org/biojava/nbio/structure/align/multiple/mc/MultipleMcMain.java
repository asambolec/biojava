...
/**
 * Main class of the Java implementation of the Combinatorial Extension - Monte
 * Carlo (CEMC) Algorithm, as it was originally described by C.Guda,
 * E.D.Scheeff, P.E. Bourne and I.N. Shindyalov (2001). The original CEMC paper
 * is available from
...
public class MultipleMcMain implements MultipleStructureAligner {
	...
	/**
	 * Default constructor. Default parameters are used.
	 * 
	...
	public MultipleMcMain(StructureAlignment pairwiseAlgo) {
		...
		if (pairwise == null) {
			pairwise = new CeCPMain();
		}
	}

	/**
	 * Creates a MultipleAlignment seed for MC optimization from the representative
	 * Atoms of the structures. If there are N structures:
	 * <ul>
	 * <li>Generate (N*(N-1))/2 all-to-all alignments in parallel using the Java
	 * API.
	 * <li>Choose the closest structure to all others as the reference.
	 * <li>Generate a MultipleAlignment by combining all the alignments to the
	 * reference, that will be used as a seed for the MC optimization.
	 * </ul>
	 *
	...
	private MultipleAlignment generateSeed(List<Atom[]> atomArrays)
			throws InterruptedException, ExecutionException, StructureException {
		...
		for (int i = 0; i < size; i++) {
			for (int j = i + 1; j < size; j++) {
				...
				Callable<AFPChain> worker = new CallableStructureAlignment(atomArrays.get(i), atomArrays.get(j),
	...
	/**
	 * This method takes the all-to-all pairwise alignments Matrix (as a double List
	 * of AFPChain) and calculates the structure with the lowest average RMSD
	 * against all others. The index of this structure is returned.
	 *
	...
	/**
	 * This method takes a list of pairwise alignments to the reference structure
	 * and calculates a MultipleAlignment resulting from combining their residue
	 * equivalencies.
	 * <p>
	 * It uses the blocks in AFPChain as {@link Block}s in the MultipleAlignment, so
	 * considers non-topological alignments, if the alignment is rigid. If the
	 * alignment is flexible, it considers the blocks as {@link BlockSets}.
	 *
	 ...
	 * @throws StructureException
	 */
	private static MultipleAlignment combineReferenceAlignments(List<AFPChain> afpList, List<Atom[]> atomArrays,
		...
		for (int str = 0; str < size; str++) {
			...
			for (int res = 0; res < length; res++) {
				if (str == ref) {
					equivalencies.get(str).add(res); // identity
				} else {
		...
		for (int pos = 0; pos < length; pos++) {
			...
			if (flexibleBoundaries.contains(pos)
					&& blockSet.getBlocks().get(blockSet.getBlocks().size() - 1).getAlignRes() != null) {
			...
			for (int str = 0; str < size; str++) {
				...
				if (lastB.getAlignRes() == null) {
					...
					List<List<Integer>> alnRes = new ArrayList<>(size);
		...
		logger.info(new StringBuilder().append("Seed alignment has ").append(seed.getBlocks()).append(" Blocks.")
				.toString());
		return seed;
	}
	...
	public MultipleAlignment align(List<Atom[]> atomArrays, Object parameters) throws StructureException {
		...
		/*
		 * int threads = params.getNrThreads(); ExecutorService executor =
		 * Executors.newFixedThreadPool(threads); List<Future<MultipleAlignment>>
		 * msaFuture = new ArrayList<Future<MultipleAlignment>>();
		 * 
		 * for (int i=0; i<params.getNrThreads(); i++){ //Change the random seed for
		 * each parallelization MultipleMcParameters paramsMC = (MultipleMcParameters)
		 * params; paramsMC.setRandomSeed(paramsMC.getRandomSeed()+i);
		 * 
		 * Callable<MultipleAlignment> worker = new MultipleMcOptimizer( result,
		 * paramsMC, reference);
		 * 
		 * Future<MultipleAlignment> submit = executor.submit(worker);
		 * msaFuture.add(submit); }
		 * 
		 * double maxScore = Double.NEGATIVE_INFINITY; //Take the one with the best
		 * result (best MC-Score) for (int i=0; i<msaFuture.size(); i++){
		 * MultipleAlignment align = msaFuture.get(i).get(); double s =
		 * align.getScore(MultipleAlignmentScorer.MC_SCORE); if (s > maxScore){ result =
		 * align; maxScore = s; } } Long runtime =
		 * System.currentTimeMillis()-ensemble.getIoTime();
		 * ensemble.setCalculationTime(runtime);
		 * 
		 * result.setEnsemble(ensemble); ensemble.addMultipleAlignment(result);
		 * executor.shutdown();
		 */

		MultipleMcOptimizer optimizer = new MultipleMcOptimizer(result, params, reference);
