...
public class AFPChainXMLConverter {
	 ...
	 * @throws IOException
	 */
	public synchronized static void toXML(AFPChain afpChain, StringWriter swriter, Atom[] ca1, Atom[] ca2)
			throws IOException {
	...
	private static void printXMLEQRKnownPositions(PrettyXMLWriter xml, AFPChain afpChain, int blockNr)
		...
		if (pdbAln == null) {
			throw new StructureException(
					"Can't convert to XML without known the PDB coordinates. Please provide Ca atoms and call toXML(afpChain,ca1, ca2)");
		}
	...
	public static void printXMLEQRInferPositions(PrettyXMLWriter xml, AFPChain afpChain, int bk, Atom[] ca1, Atom[] ca2)
			throws IOException {
		...
		for (int pos = 0; pos < optLen[bk]; pos++) {
			...
			// ca2[pos2].getGroup().getResidueNumber().toString() + " " +
			// ca2[pos2].getParent().getPDBName());

		}

	}

	private static void printXMLBlockHeader(PrettyXMLWriter xml, AFPChain afpChain, int blockNr) throws IOException {
	...
	public static void printXMLHeader(PrettyXMLWriter xml, AFPChain afpChain) throws IOException {
		...
		xml.attribute("alnLength", Integer.toString(afpChain.getAlnLength()));
		xml.attribute("blockNum", Integer.toString(afpChain.getBlockNum()));
		xml.attribute("gapLen", Integer.toString(afpChain.getGapLen()));
		xml.attribute("optLength", Integer.toString(afpChain.getOptLength()));
		xml.attribute("totalLenIni", Integer.toString(afpChain.getTotalLenIni()));
		...
		xml.attribute("similarity1", Integer.toString(afpChain.getCoverage1()));
		xml.attribute("similarity2", Integer.toString(afpChain.getCoverage2()));
		...
		xml.attribute("ca1Length", Integer.toString(afpChain.getCa1Length()));
		xml.attribute("ca2Length", Integer.toString(afpChain.getCa2Length()));
		xml.attribute("afpNum", Integer.toString(afpChain.getAfpSet().size()));
