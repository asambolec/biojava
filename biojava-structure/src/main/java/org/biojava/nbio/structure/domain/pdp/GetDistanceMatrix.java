...
public class GetDistanceMatrix {
	...
	public PDPDistanceMatrix getDistanceMatrix(Atom[] protein) throws StructureException {
		...
		if (protein.length >= PDPParameters.MAXLEN) {
			logger.error(String.format("%d protein.len > MAXLEN %d%n", protein.length, PDPParameters.MAXLEN));
			return null;
		}
				...
				if (dist[i][j] >= 2 && j - i > 5) {
					...
					/* alpha-helices */
						...
						if (dist[i - 3][j - 3] >= 1 && dist[i + 3][j + 3] >= 1
							...
							if ((dist[i - 3][j - 3] >= 1 || dist[i - 3][j - 4] >= 1 || dist[i - 4][j - 3] >= 1
									|| dist[i - 4][j - 4] >= 1)
									&& (dist[i + 4][j + 4] >= 1 || dist[i + 4][j + 3] >= 1 || dist[i + 3][j + 3] >= 1
											|| dist[i + 3][j + 4] >= 1)
									|| (dist[i - 4][j + 4] >= 1 || dist[i - 4][j + 3] >= 1 || dist[i - 3][j + 4] >= 1
											|| dist[i - 3][j + 3] >= 1)
											&& (dist[i + 4][j - 4] >= 1 || dist[i + 4][j - 3] >= 1
