 ...
 * {@link AbstractBean}. By default, all fields from the bean are taken as the
 * CIF labels. Fields may be omitted by annotating them as
 * {@link IgnoreField @IgnoreField}. The CIF label for a field may be changed
 * (for instance, for fields that are not valid Java identifiers) by defining a
 * function <tt>static Map<String,String> getCIFLabelMap()</tt> mapping from the
 * field's name to the correct label.
 * 
 * @author Jose Duarte
...
public class MMCIFFileTools {
	...
	/**
	 * The character to be printed out in cases where a value is not assigned in
	 * mmCIF files
	 */
	public static final String MMCIF_MISSING_VALUE = "?";

	/**
	 * The character to be printed out as a default value in mmCIF files, e.g. for
	 * the default alt_locs
	 */
	 ...
	 * className must be one of the beans in the
	 * {@link org.biojava.nbio.structure.io.mmcif.model} package
	 * 
	 * @param categoryName
	 * @param className
	 * @return
	...
	public static String toLoopMmCifHeaderString(String categoryName, String className) throws ClassNotFoundException {
		...
		for (Field f : getFields(c)) {
			str.append(new StringBuilder().append(categoryName).append(".").append(f.getName()).append(newline)
					.toString());
		}
	...
	/**
	 * Converts a mmCIF bean (see {@link org.biojava.nbio.structure.io.mmcif.model}
	 * to a String representing it in mmCIF (single-record) format.
	 * 
	 * @param categoryName
	 * @param o
	 * @return
	...
	/**
	 * Gets all fields for a particular class, filtering fields annotated with
	 * {@link IgnoreField @IgnoreField}.
	 * 
	 * As a side effect, calls {@link Field#setAccessible(boolean)
	 * setAccessible(true)} on all fields.
	 * 
	 * @param c
	 * @return
	 */
	...
	/**
	 * Gets the mmCIF record name for each field. This is generally just the name of
	 * the field or the value specified by the {@link CIFLabel @CIFLabel}
	 * annotation.
	 * 
	 * As a side effect, calls {@link Field#setAccessible(boolean)
	 * setAccessible(true)} on all fields.
	 ...
	 * {@link org.biojava.nbio.structure.io.mmcif.model} to a String representing
	 * them in mmCIF loop format with one record per line.
	 * 
	 * @param list
	 * @return
	 */
	public static <T> String toMMCIF(List<T> list, Class<T> klass) {
		...
		list.forEach(o -> sb.append(toSingleLoopLineMmCifString(o, fields, sizes)));
	...
	/**
	 * Given a mmCIF bean produces a String representing it in mmCIF loop format as
	 * a single record line
	 * 
	 * @param record
	 * @param fields Set of fields for the record. If null, will be calculated from
	...
	private static String toSingleLoopLineMmCifString(Object record, Field[] fields, int[] sizes) {
		...
		for (Field f : fields) {
			...
			try {
				...
				str.append(String.format(new StringBuilder().append("%-").append(sizes[i]).append("s ").toString(),
						addMmCifQuoting(val)));
	...
	private static String addMmCifQuoting(String val) {
		...
		if (val.contains("'")) {
			// double quoting for strings containing single quotes (not strictly necessary
			// but it's what the PDB usually does)
			newval = new StringBuilder().append("\"").append(val).append("\"").toString();
			...
			if (val.contains(" ") && val.contains("'")) {
				...
				logger.warn(
						"Value contains both spaces and single quotes, won't format it: {}. CIF ouptut will likely be invalid.",
						val);
			}
			newval = val;
		}
	 ...
	 * @param chainId   the internal chain identifier (asym id) for the output
	 *                  AtomSites
	 * @return
	 */
	...
	public static AtomSite convertAtomToAtomSite(Atom a, int model, String chainName, String chainId, int atomId) {
		...
		/*
		 * ATOM 7 C CD . GLU A 1 24 ? -10.109 15.374 38.853 1.00 50.05 ? ? ? ? ? ? 24
		 * GLU A CD 1 ATOM 8 O OE1 . GLU A 1 24 ? -9.659 14.764 37.849 1.00 49.80 ? ? ?
		 * ? ? ? 24 GLU A OE1 1 ATOM 9 O OE2 . GLU A 1 24 ? -11.259 15.171 39.310 1.00
		 * 50.51 ? ? ? ? ? ? 24 GLU A OE2 1 ATOM 10 N N . LEU A 1 25 ? -5.907 18.743
		 * 37.412 1.00 41.55 ? ? ? ? ? ? 25 LEU A N 1 ATOM 11 C CA . LEU A 1 25 ? -5.168
		 * 19.939 37.026 1.00 37.55 ? ? ? ? ? ? 25 LEU A CA 1
		 */
	...
	/**
	 * Converts a Group into a List of {@link AtomSite} objects. Atoms in other
	 * altloc groups (different from the main group) are also included, removing
	 * possible duplicates via using the atom identifier to assess uniqueness.
	 * 
	 * @param g         the group
	 * @param model     the model number for the output AtomSites
	 * @param chainName the chain identifier (author id) for the output AtomSites
	...
	public static List<AtomSite> convertGroupToAtomSites(Group g, int model, String chainName, String chainId) {
		...
		// The alt locs can have duplicates, since at parsing time we make sure that all
		// alt loc groups have
	...
	public static List<AtomSite> convertChainToAtomSites(Chain c, int model, String chainName, String chainId) {
		...
		if (c.getEntityInfo() == null) {
			logger.warn(
					"No Compound (entity) found for chain {}: entity_id will be set to 0, label_seq_id will be the same as auth_seq_id",
					c.getName());
		}
	...
	/**
	 * Finds the max length of each of the String values contained in each of the
	 * fields of the given list of beans. Useful for producing mmCIF loop data that
	 * is aligned for all columns.
	 * 
	 * @param list   list of objects. All objects should have the same class.
	 * @param fields Set of fields for the record. If null, will be calculated from
	 *               the class of the first record
	 * @return
	...
	/**
	 * Finds the max length of a list of strings Useful for producing mmCIF
	 * single-record data that is aligned for all values.
	 * 
	 * @param names
	 * @return
