...
/**
 * A class containing static methods to parse the symop.lib file from the CCP4
 * package. The file contains the transformations belonging to all protein
...
public class SymoplibParser {
	...
	/**
	 * Gets the space group for the given standard identifier. See for example
	...
	private static TreeMap<Integer, SpaceGroup> parseSpaceGroupsXML() {
		...
		// NOTE: if the space group file is requested by some part of the code (i.e.
		// this method is called) and
		// there is a problem in reading it, then that's truly a FATAL problem, since
		// this is not a user file
		// but a file that's part of the distribution: it MUST be there and MUST have
		// the right format. A failure
		// to read it is more of a "compilation" error than a runtime error. That's the
		// reason that System.exit
		// is called (which otherwise usually is not a good idea).
		//
		// The rest of the application will simply not work: there are 3 options to
		// handle it
		// a) returning null and then a NullPointer will happen down the line and thus a
		// not very clear
		// error message will be printed
		// b) throw the exception forward and catch it in the final main but that would
		// also be bad because
		// this is a file that the user didn't input but that should be part of the
		// distribution
		// c) call System.exit(1) and "crash" the application with a
		...
		if (spaceGroupIS == null) {
			logger.error(new StringBuilder().append("Fatal error! Could not find resource: ").append(SPACE_GROUPS_FILE)
		...
		try {
			...
			logger.error(new StringBuilder().append("Fatal error! Could not parse resource: ").append(SPACE_GROUPS_FILE)
					.append(". Error: ").append(e.getMessage()).toString());
			System.exit(1);
		} catch (JAXBException e) {
			logger.error(new StringBuilder().append("Fatal error! Could not parse resource: ").append(SPACE_GROUPS_FILE)
					.append(". Problem in xml formatting: ").append(e.getMessage()).toString());
			System.exit(1);
		}

		name2sgs = new HashMap<>();

		map.values().forEach(sg -> {
	 ...
	 * @return a map providing information for all spacegroups
	 */
	public static TreeMap<Integer, SpaceGroup> parseSpaceGroupsXML(InputStream spaceGroupIS)
	...
	public static SpaceGroup getSpaceGroup(String shortName) {
		...
		// PDB uses group "P 1-" for 13 racemic mixture entries (as of Sep2011), e.g.
		// 3e7r
		// they call the space group "P 1-" unusually (symop.lib and everyone else call
		// it "P -1")
		if ("P 1-".equals(shortName)) {
			shortName = "P -1";
		}

		// enantiomorphic space groups contain sometime letters indicating glide planes
		// which should always be lower case
		// in some PDB entries like 4gwv they are in upper case, we fix that here:
		// convert any non-first letter to lower case
	...
	/**
	 * A parser for the symop.lib file provided by CCP4. Note: this file is not
	 * getting re-distributed by BioJava. It can be downloaded from:
	 *
	 * http://www.ccp4.ac.uk/cvs/viewvc.cgi/libccp4/data/symop.lib?revision=1.10&view=markup
	 *
	 * Note: this file is not needed by BioJava. BioJava loads equivalent
	 * information from the file spacegroups.xml
	 *
	 * @param symoplibIS
	 * @return
	 */
	public static TreeMap<Integer, SpaceGroup> parseSymopLib(InputStream symoplibIS) {
		TreeMap<Integer, SpaceGroup> map = new TreeMap<>();
		name2sgs = new HashMap<>();
		try (BufferedReader br = new BufferedReader(new InputStreamReader(symoplibIS))) {
			String line;
			...
			while ((line = br.readLine()) != null) {
				if (!line.startsWith(" ")) {
					...
					currentSG = new SpaceGroup(id, multiplicity, primitiveMultiplicity, shortSymbol, altShortSymbol,
							BravaisLattice.getByName(brav));
				} else {
			...
			logger.error(new StringBuilder().append("Fatal error! Can't read symop.lib file. Error: ")
					.append(e.getMessage()).append(". ").toString());
			System.exit(1);
		}

		map.values().forEach(SpaceGroup::initializeCellTranslations);
		return map;
	}
