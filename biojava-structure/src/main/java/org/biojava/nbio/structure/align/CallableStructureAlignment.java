...
public class CallableStructureAlignment implements Callable<AFPChain> {
	...
	/**
	 * Default constructor. Used in DB search. Instantiates an empty object,
	 * everything has to be set independently.
	 */
	public CallableStructureAlignment() {
	}

	/**
	 * Constructor for all-to-all alignment calculation. Used for MultipleMC seed
	 * alignment calculation, for example.
	 *
	 * @param ca1           Atoms to align of the first structure
	 * @param ca2           Atoms to align of the second structure
	 * @param algorithmName the pairwise aligner algorithm to use, a new instance
	 *                      will be created for each thread.
	...
	public AFPChain call() throws Exception {
		...
		try {
			// Download the Atoms if they are not provided from the outisde (DB searches
			// usually)
			if (ca1 == null) {
	...
	private void writeXML(File outFileF, String name1, String name2, String xml) {
		try {
			// Create file
			File newF = new File(outFileF, new StringBuilder().append("dbsearch_").append(name1).append("_")
					.append(name2).append(".xml.gz").toString());
