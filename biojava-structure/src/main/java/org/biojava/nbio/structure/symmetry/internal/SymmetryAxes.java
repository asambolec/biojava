...
/**
 * Data Structure that stores all the symmetry axis that describe the symmetry
 * of a structure. Generalizes to all types of symmetry, the classic ones (Cn,
 * Dn) and any hierarchical or local symmetries.
 * <p>
 * Hierarchical symmetry can be visualized as a tree, where each level has a
 * fixed branching factor. Each level of the tree is associated with a
 * transformation operator, whose order determines the degree of nodes at that
 * level of the tree. Leaves of the tree implicitly represent aligned repeats
 * (indexed 0 to n-1), so care must be taken to keep external references to the
 * repeats (e.g. rows of a {@link MultipleAlignment} in the same order implied
 * by the tree.
 * <p>
 * Each node of the tree specifies an alignment between those repeats below each
 * of its children. It is also associated with a symmetry axis, which is
 * calculated based on the associated operator as well as any parent operators.
 * It also stores the parts of the structure (symmetric units) involved in each
 * axis, in addition to the way to calculate them.
 * <p>
 * This is intended to provide a general axis support for the multiple repeat
 * alignment optimization and the axis display in Jmol. This object is related
 * to a MultipleAlignment object that defines the symmetric units.
 *
 * @author Aleix Lafita
 * @since 4.2.0
 *
...
public class SymmetryAxes {
	/**
	 * List of all symmetry axis. They are sorted from higher to lower in the
	 * symmetry hierarchy, where higher means that they apply more globally and
	 * lower means that they apply to a local region of the higher axis division.
	 */
	...
	/**
	 * Return a list giving the number of times each axis must be applied to
	 * generate the given repeat.
	 * <P>
	 * For instance, for a D3 case <tt>getAxisCounts(4)</tt> would return [2,0],
	 * indicating that repeat 4 is generated by two applications of the 3-fold axis
	 * followed by 0 applications of the two-fold axis.
	 * 
	 * @param repeat Index of the desired repeat
	 * @return array of the same length as axes giving the number of times to apply
	 *         each axis.
	 */
	...
	/**
	 * Return the operator for all elementary axes of symmetry of the structure,
	 * that is, the axes stored in the List as unique and from which all the
	 * symmetry axes are constructed.
	 *
	...
	public List<Matrix4d> getElementaryAxes() {
		...
		axes.forEach(axis -> ops.add(axis.getOperator()));
		return ops;
	}

	/**
	 * Return all elementary axes of symmetry of the structure, that is, the axes
	 * stored in the List as unique and from which all the symmetry axes are
	 * constructed.
	 *
	 * @return axes elementary axes of symmetry.
	 */
	 ...
	 * <p>
	 * Returns two lists of the same length. The first gives a list of all repeat
	 * indices which are aligned at the specified level of symmetry (e.g. 0 through
	 * the degree of this level). The second list gives the corresponding repeats
	 * after applying the operator once.
	 *
	 * @param level the axis index
	 * @return the double List of repeat relations, or null if the level is invalid
	 * @see #getRepeatsCyclicForm(int, int) for an equivalent specification with
	 *      half the memory
	 */
	...
	public List<List<Integer>> getRepeatRelation(int level, int firstRepeat) {
		...
		if (firstRepeat % n != 0) {
			throw new IllegalArgumentException(
					String.format("Repeat %d cannot start a block at level %s of this tree", firstRepeat, level));
		}
	...
	/**
	 * Return the transformation that needs to be applied to a repeat in order to
	 * superimpose onto repeat 0.
	 *
	...
	/**
	 * Return the transformation that needs to be applied to repeat x in order to
	 * superimpose onto repeat y.
	 *
	 ...
	 * all parts of the structure. This combines the elementary axes to generate all
	 * possible axes. The axes are returned in the repeat degrees.
	 * 
	 ...
	 * @param prior    transformation aligning the first repeat of this axis with
	 *                 the first overall
	...
	/**
	 * Get the number of leaves from a node at the specified level. This is equal to
	 * the product of all degrees at or below the level.
	 * 
	...
	/*
	 * Implementation note: The tree is a nice explanation and a good image for
	 * developing algorithms, but it is not constructed explicitly. Instead, we just
	 * store one elementary axis for each level and reconstruct which operators
	 * apply to a particular leaf based on that leaf's index.
	 */
	...
	public static class Axis {
		...
		/**
		 * Get the transformation operator as a rotation axis. For open symmetry this
		 * will have a non-zero screw component.
		 * 
		...
		/**
		 * 
		 * @param level The level of this axis within it's parent hierarchy. Must be
		 *              positive
		 */
		...
		// /**
		// *
		// * @param indexInLevel the index of this axis relative to others at the same
		// level
		// */
