...
/**
 * A map from {@link ResidueNumber ResidueNumbers} to ATOM record positions in a
 * PDB file.
 *
 * <p>
 * To use:
 ...
 *
 * <p>
 * Note: The getLength() methods were introduced in BioJava 4.0.0 to replace the
 * calcLength methods. The new method returns the number of residues between two
 * residues, inclusive, whereas the previous method returned 1 less than that.
 * 
...
public class AtomPositionMap {
	 ...
	 */
	public static final GroupMatcher AMINO_ACID_MATCHER = (Group group) -> {
		if (group == null) {
			return false;
		}
		...
		return PolymerType.PROTEIN_ONLY.contains(polyType) && group.hasAtom(StructureTools.CA_ATOM_NAME);
	};

	/**
	 * Matches all atoms
	 */
	public static final GroupMatcher ANYTHING_MATCHER = (Group group) -> true;
	...
	/**
	 * Creates a new AtomPositionMap containing representative atoms from a
	 * structure.
	 * 
	 * @param s
	 */
	 ...
	 * @return The number of atoms between A and B inclusive belonging to the given
	 *         chain
	 */
	public int getLength(int positionA, int positionB, String startingChain) {
		...
		for (Map.Entry<ResidueNumber, Integer> entry : treeMap.entrySet()) {
			if (entry.getKey().getChainName().equals(startingChain) && positionStart <= entry.getValue()
	 ...
	 * @param startingChain Case-sensitive chain
	 * @return The number of atoms from A to B inclusive belonging to the given
	 *         chain
	 */
	...
	/**
	 * Calculates the number of atoms between two ResidueNumbers, inclusive. Both
	 * residues must belong to the same chain.
	 * 
	 * @param start First residue
	 * @param end   Last residue
	 * @return The number of atoms from A to B inclusive
	 * @throws IllegalArgumentException if start and end are on different chains, or
	 *                                  if either of the residues doesn't exist
	 */
	public int getLength(ResidueNumber start, ResidueNumber end) {
		if (!start.getChainName().equals(end.getChainName())) {
			throw new IllegalArgumentException(
					String.format("Chains differ between %s and %s. Unable to calculate length.", start, end));
		}
		Integer startPos = getPosition(start);
		Integer endPos = getPosition(end);
		if (startPos == null) {
			throw new IllegalArgumentException(
					new StringBuilder().append("Residue ").append(start).append(" was not found.").toString());
		}
		if (endPos == null) {
			throw new IllegalArgumentException(
					new StringBuilder().append("Residue ").append(start).append(" was not found.").toString());
	...
	/**
	 * Calculates the number of atoms between two ResidueNumbers, inclusive. Both
	 * residues must belong to the same chain. Will return a negative value if the
	 ...
	 * 
	 * @param residueNumber
	 * @return The position of the ATOM record in the PDB file corresponding to the
	 *         {@code residueNumber}, or null if the residueNumber was not found
	 */
	...
	/**
	 * @param chainId
	 * @return The first {@link ResidueNumber} of the specified chain (the one
	 *         highest down in the PDB file)
	 */
	public ResidueNumber getFirst(String chainId) {
		Map.Entry<ResidueNumber, Integer> entry = treeMap.firstEntry();
		while (true) {
			if (entry.getKey().getChainName().equals(chainId)) {
				return entry.getKey();
			}
	...
	/**
	 * @param chainId
	 * @return The last {@link ResidueNumber} of the specified chain (the one
	 *         farthest down in the PDB file)
	 */
	...
	/**
	 * @return The first {@link ResidueNumber} of any chain (the one farthest down
	 *         in the PDB file)
	 */
	...
	/**
	 * @return The last {@link ResidueNumber} of any chain (the one farthest down in
	 *         the PDB file)
	 */
	...
	/**
	 * Returns a list of {@link ResidueRange ResidueRanges} corresponding to this
	 * entire AtomPositionMap.
	 */
	public List<ResidueRangeAndLength> getRanges() {
		...
		for (ResidueNumber rn : treeMap.keySet()) {
			if (!rn.getChainName().equals(currentChain)) {
				if (first != null) {
					ResidueRangeAndLength newRange = new ResidueRangeAndLength(currentChain, first, prev,
							this.getLength(first, prev));
					ranges.add(newRange);
				}
				first = rn;
			}
