...
public class StructureSequenceMatcher {
	...
	/**
	 * Get a substructure of {@code wholeStructure} containing only the {@link Group
	 * Groups} that are included in {@code sequence}. The resulting structure will
	 * contain only {@code ATOM} residues; the SEQ-RES will be empty. The
	 * {@link Chain Chains} of the Structure will be new instances (cloned), but the
	 * {@link Group Groups} will not.
	 * 
	...
	public static Structure getSubstructureMatchingProteinSequence(ProteinSequence sequence, Structure wholeStructure) {
		...
		for (ResidueNumber rn : rns) {
			...
			try {
				...
				throw new IllegalArgumentException(new StringBuilder().append("Could not find residue ").append(rn)
						.append(" in structure").toString(), e);
			}
	...
	/**
	 * Generates a ProteinSequence corresponding to the sequence of struct, and
	 * maintains a mapping from the sequence back to the original groups.
	 *
	 * Chains are appended to one another. 'X' is used for heteroatoms.
	 *
	 * @param struct             Input structure
	 * @param groupIndexPosition An empty map, which will be populated with (residue
	 *                           index in returned ProteinSequence) -> (Group within
	 *                           struct)
	 ...
	 *
	 * @see {@link SeqRes2AtomAligner#getFullAtomSequence(List, Map)}, which does
	 *      the heavy lifting.
	 *
	 */
	public static ProteinSequence getProteinSequenceForStructure(Structure struct,
		...
		struct.getChains().stream().map(Chain::getAtomGroups).forEach(groups -> {
			...
			// fix up the position to include previous chains, and map the value back to a
			// Group
			chainIndexPosition.keySet().forEach(seqIndex -> {
	...
	/**
	 * Given a sequence and the corresponding Structure, get the ResidueNumber for
	 * each residue in the sequence.
	 *
	 * <p>
	 * Smith-Waterman alignment is used to match the sequences. Residues in the
	 * sequence but not the structure or mismatched between sequence and structure
	 * will have a null atom, while residues in the structure but not the sequence
	 ...
	 * @return A list of ResidueNumbers of the same length as seq, containing either
	 *         the corresponding residue or null.
	 */
	public static ResidueNumber[] matchSequenceToStructure(ProteinSequence seq, Structure struct) {

		// 1. Create ProteinSequence for struct while remembering to which group each
		// residue corresponds
		...
		// TODO This should really be semi-global alignment, though global for either
		// sequence OR structure (we don't know which)
		...
		SubstitutionMatrix<AminoAcidCompound> matrix = new SimpleSubstitutionMatrix<>(
				AminoAcidCompoundSet.getAminoAcidCompoundSet(), (short) 1, (short) -1);
		matrix = new SimpleSubstitutionMatrix<>(AminoAcidCompoundSet.getAminoAcidCompoundSet(),
				new InputStreamReader(SimpleSubstitutionMatrix.class.getResourceAsStream("/matrices/blosum100.txt")),
				"blosum100");
		SequencePair<ProteinSequence, AminoAcidCompound> pair = Alignments.getPairwiseAlignment(seq, structSeq,
		...
		for (int pos = alignedSeq.getStart().getPosition(); pos <= alignedSeq.getEnd().getPosition(); pos++) { // 1-indexed
			// skip missing residues from sequence. These probably represent alignment
			// errors
	...
	public static ProteinSequence removeGaps(ProteinSequence gapped) {
		...
		try {
			...
			logger.error(
					"Could not create ungapped protein sequence, found unknown compounds: {}. This is most likely a bug.",
					e.getMessage());
		}
	...
	/**
	 * Creates a new list consisting of all columns of gapped where no row contained
	 * a null value.
	 *
		...
		for (int i = 0; i < nProts; i++) {
			if (gapped[i].length != protLen) {
				throw new IllegalArgumentException(String.format(
						"Expected a rectangular array, but row 0 has %d elements " + "while row %d has %d.", protLen, i,
						gapped[i].length));
