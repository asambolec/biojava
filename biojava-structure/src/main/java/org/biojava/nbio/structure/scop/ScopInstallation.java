 ...
 * <ul>
 * <li>SCOP: <a href=
 * "http://scop.mrc-lmb.cam.ac.uk/scop/">http://scop.mrc-lmb.cam.ac.uk/scop/</a></li>
 * 
 * <li>Introduction: <a href=
 * "http://scop.mrc-lmb.cam.ac.uk/scop/intro.html">http://scop.mrc-lmb.cam.ac.uk/scop/intro.html</a>
 * </li>
 * 
 * <li>SCOP parsable files: <a href=
 * "http://scop.mrc-lmb.cam.ac.uk/scop/parse/">http://scop.mrc-lmb.cam.ac.uk/scop/parse/</a>
 * </li>
 * </ul>
 *
 * 
 * This class can automatically download missing files from the SCOP
 * classification.
 *
 * @author Andreas Prlic
 *
...
public class ScopInstallation implements LocalScopDatabase {
	 ...
	 *
	 * @param cacheLocation where the SCOP files are stored. If they can't be found
	 *                      at that location they will get automatically downloaded
	 *                      and installed there.
	 */
	 ...
	 * This will first check for system properties or environmental variables called
	 * {@link UserConfiguration#PDB_CACHE_DIR}, or else will use a temporary
	 * directory
	 */
	...
	/**
	 * Removes all of the comments (dir.com file) in order to free memory. The file
	 * will need to be reloaded if {@link #getComments(int)} is called subsequently.
	 */
	 ...
	 * 
	 * @see
	 * org.biojava.nbio.structure.scop.ScopDatabase#getByCategory(org.biojava.nbio.
	 * structure.scop.ScopCategory)
	 */
	@Override
		...
		for (Integer i : sunidMap.keySet()) {
			...
			if (sc.getCategory() == category) {
				try {
					...
					throw new RuntimeException(new StringBuilder().append("Could not clone ")
							.append(ScopDescription.class).append(" subclass").toString(), e);
				}
			}
	 ...
	 * 
	 * @see
	 * org.biojava.nbio.structure.scop.ScopDatabase#filterByClassificationId(java.
	 * lang.String)
	 */
	@Override
	public List<ScopDescription> filterByClassificationId(String query) {
		...
		List<ScopDescription> matches = new ArrayList<>();
		sunidMap.keySet().stream().map(sunidMap::get).forEach(sc -> {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.biojava.nbio.structure.scop.ScopDatabase#getTree(org.biojava.nbio.
	 * structure.scop.ScopDomain)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.biojava.nbio.structure.scop.ScopDatabase#filterByDomainName(java.lang.
	 * String)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.biojava.nbio.structure.scop.ScopDatabase#filterByDescription(java.lang.
	 * String)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.biojava.nbio.structure.scop.ScopDatabase#getScopDescriptionBySunid(int)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.biojava.nbio.structure.scop.ScopDatabase#getDomainsForPDB(java.lang.
	 * String)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.biojava.nbio.structure.scop.ScopDatabase#getDomainByScopID(java.lang.
	 * String)
	 */
	@Override
	public ScopDomain getDomainByScopID(String scopId) {
		...
		return doms.stream().filter(d -> d.getScopId().equalsIgnoreCase(scopId)).findFirst().orElse(null);
	}
	...
	private void parseHierarchy(BufferedReader buffer) throws IOException {
		...
		while ((line = buffer.readLine()) != null) {
			...
			if (spl.length != 3) {
				throw new IOException(new StringBuilder().append("parseHierarchy: Can't parse line ").append(line)
						.append(" (length: ").append(spl.length).append(")").toString());
			}
			counter++;
	...
	private void parseDescriptions(BufferedReader buffer) throws IOException {
		...
		while ((line = buffer.readLine()) != null) {
			...
			if (spl.length != 5) {
				throw new IOException(new StringBuilder().append("parseDescriptions: Can't parse line ").append(line)
						.append(" (length: ").append(spl.length).append(")").toString());
			}
			counter++;
	...
	/**
	 * Converts the SCOP range field into a list of subranges suitable for storage
	 * in a ScopDomain object. Each range should be of a format compatible with
	 * {@link StructureTools#getSubRanges(Structure,String)}.
	 * 
	 * @param range
	 * @return
	 */
	...
	protected void downloadFileFromRemote(URL remoteURL, File localFile) throws IOException {
		logger.info(new StringBuilder().append("Downloading ").append(remoteURL).append(" to: ").append(localFile)
				.toString());
	...
	protected String getClaFilename() {
		return new StringBuilder().append(cacheLocation).append(claFileName).append(scopVersion).toString();
	}

	protected String getDesFilename() {
		return new StringBuilder().append(cacheLocation).append(desFileName).append(scopVersion).toString();

	}

	protected String getHieFilename() {
		return new StringBuilder().append(cacheLocation).append(hieFileName).append(scopVersion).toString();

	}

	protected String getComFilename() {
		return new StringBuilder().append(cacheLocation).append(comFileName).append(scopVersion).toString();
	}
	 ...
	 * 
	 * @see
	 * org.biojava.nbio.structure.scop.ScopDatabase#getScopDomainsBySunid(java.lang.
	 * Integer)
	 */
	@Override
				...
				try {
					if (d.getPx() == sunid) {
						...
						throw new RuntimeException(
								new StringBuilder().append("Type ").append(d).append(" not recognized").toString()); // only
																														// possible
																														// if
																														// SCOP
																														// changes
	...
	public List<String> getComments(int sunid) {
		...
		if (!commentsMap.containsKey(sunid)) {
	...
	private void initScopURLs() {
		...
		ScopMirror alt = new ScopMirror(SCOP_DOWNLOAD_ALTERNATE, "dir.cla.scop.%s.txt", "dir.des.scop.%s.txt",
