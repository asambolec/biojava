...
public class FCAlignHelper {
	...
	private void checkAlign() {
		...
		if (sapp[0] != 0) {
			logger.error(String.format("warn: not a local-alignment result, first operation %d%n", sapp[0]));
		}
		double sco = checkScore();
		if (Math.abs(sco - alignScore) > 1e-3) {
			logger.error(String.format("FCAlignHelper: warn: alignment scores are different %f(check) %f(align)%n", sco,
					alignScore));
		}
	}
	...
	/**
	 * record the aligned pairs in alignList[][0], alignList[][1]; return the number
	 * of aligned pairs
	 * 
	 * @param alignList
