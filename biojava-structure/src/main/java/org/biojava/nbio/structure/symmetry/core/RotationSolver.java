...
public class RotationSolver implements QuatSymmetrySolver {
	...
	private void solve() {
		...
		// for cases with icosahedral symmetry n cannot be higher than 60, should check
		// for spherical symmetry here
		...
		for (int i = 0; i < sphereCount; i++) {
			...
			for (double angle : angles) {
				...
				// System.out.println("Rotation Solver: permutation: " + i + ": " +
				// permutation);
	...
	/**
	 * Combine current rotations to make all possible permutations. If these are all
	 * valid, add them to the rotations
	 * 
	 * @param additionalRots Additional rotations we are considering adding to
	 *                       this.rotations
	...
	private boolean completeRotationGroup(Rotation... additionalRots) {
		...
		// Add rotations
		newRots.forEach(this::addRotation);
		return true;
	}
	...
	/**
	 * Superimpose subunits based on the given permutation. Then check whether the
	 * superposition passes RMSD thresholds and create a Rotation to represent it if
	 * so.
	 * 
	 * @param permutation A list specifying which subunits should be aligned by the
	 *                    current transformation
	 * @return A Rotation representing the permutation, or null if the superposition
	 ...
	 * 
	 * @return The rotation angle corresponding to each fold of
	 *         {@link Subunits#getFolds()}
	 */
	private List<Double> getAngles() {
		...
		// note this loop starts at 1, we do ignore 1-fold symmetry, which is the first
		// entry
		for (int fold : folds) {
			if (fold > 0 && fold <= n) {
	...
	/**
	 * Checks if a particular permutation is allowed and superimposes well. Caches
	 * results.
	 * 
	 * @param permutation
	...
	/**
	 * The permutation must map all subunits onto an equivalent subunit and no
	 * subunit onto itself
	 * 
	 * @param permutation
	 * @return
	...
	private static Rotation createSymmetryOperation(List<Integer> permutation, Matrix4d transformation,
			AxisAngle4d axisAngle, int fold, QuatSymmetryScores scores) {
	...
	/**
	 * Compare this.transformedCoords with the original coords. For each subunit,
	 * return the transformed subunit with the closest position.
	 * 
