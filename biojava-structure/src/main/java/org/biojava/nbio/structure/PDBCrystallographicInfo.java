...
public class PDBCrystallographicInfo implements Serializable {
	...
	/**
	 * Some PDB files contain NCS operators necessary to create the full AU. Usually
	 * this happens for viral proteins. See
	 * http://www.wwpdb.org/documentation/format33/sect8.html#MTRIXn . Note that the
	 * "given" operators (iGiven field =1 in PDB format, "given" string in
	 * _struct_ncs_oper.code in mmCIF format) are not stored.
	 */
	private Matrix4d[] ncsOperators;

	/**
	 * Whether this structure has a non-standard space group not supported by
	 * Biojava. If this is true the sg member will be null.
	 * 
	 * @since 4.2.5
	 */
	...
	/**
	 * Whether this structure is non-standard coordinate frame convention, for which
	 * our scale matrix calculation and thus the crystal reconstruction will be
	 * incorrect. There's ~ 200 old structures in the PDB affected by the
	 * non-standard frame problem, hopefully they will be remediated in the future.
	...
	/**
	 * Gets all symmetry transformation operators corresponding to this structure's
	 * space group (including the identity, at index 0) expressed in the orthonormal
	 * basis. Using PDB axes convention (NCODE=1).
	 * 
	...
	/**
	 * Get the NCS operators. Some PDB files contain NCS operators necessary to
	 * create the full AU. Usually this happens for viral proteins. See
	 * http://www.wwpdb.org/documentation/format33/sect8.html#MTRIXn . Note that the
	 * "given" operators (iGiven field =1 in PDB format, "given" string in
	 * _struct_ncs_oper.code in mmCIF format) are not stored.
	 * 
	...
	/**
	 * Set the NCS operators. Some PDB files contain NCS operators necessary to
	 * create the full AU. Usually this happens for viral proteins. See
	 * http://www.wwpdb.org/documentation/format33/sect8.html#MTRIXn . Note that the
	 * "given" operators (iGiven field =1 in PDB format, "given" string in
	 * _struct_ncs_oper.code in mmCIF format) are not stored.
	...
	/**
	 * Whether this structure has a non-standard space group not supported by
	 * Biojava. If this is true {@link #getSpaceGroup()} will be null.
	 * 
	 * @since 4.2.5
	 */
	...
	public String toString() {
		return new StringBuilder().append("[").append(sg == null ? "no SG" : sg.getShortSymbol()).append(" - ")
				.append(cell == null ? "no Cell"
						: String.format("%.2f %.2f %.2f, %.2f %.2f %.2f", cell.getA(), cell.getB(), cell.getC(),
								cell.getAlpha(), cell.getBeta(), cell.getGamma()))
				.append(ncsOperators == null ? "" : String.format(" - %d NCS operators", ncsOperators.length))
				.append("]").toString();
	}
