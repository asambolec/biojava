...
/**
 * A class that provides static access methods for easy lookup of protein
...
public class StructureIO {
	...
	// LoggerFactory.getLogger(StructureIO.class);
	 ...
	 * @throws IOException
	 */
	public static Structure getBiologicalAssembly(String pdbId, boolean multiModel)
	...
	/**
	 * Returns the biological assembly for the given PDB id and bioassembly
	 * identifier.
	 * <p>
	 * The output Structure will be different depending on the multiModel parameter:
	 * <li>the symmetry-expanded chains are added as new models, one per
	 ...
	 * 
	 * @param pdbId
	 * @param biolAssemblyNr - the ith biological assembly that is available for a
	 *                       PDB ID (we start counting at 1, 0 represents the asym
	 *                       unit).
	 ...
	 *                       originalAuthId_transformId).
	 * @return a Structure object or null if that assembly is not available
	 * @throws StructureException if there is no bioassembly available for given
	 *                            biolAssemblyNr or some other problems encountered
	 *                            while loading it
	 * @throws IOException
	 */
	public static Structure getBiologicalAssembly(String pdbId, int biolAssemblyNr, boolean multiModel)
			throws IOException, StructureException {
	...
	/**
	 * Returns the biological assembly for the given PDB id and bioassembly
	 * identifier, using multiModel={@value AtomCache#DEFAULT_BIOASSEMBLY_STYLE}
	 * 
	 * @param pdbId
	 * @param biolAssemblyNr - the ith biological assembly that is available for a
	 *                       PDB ID (we start counting at 1, 0 represents the asym
	 ...
	 * @throws IOException
	 */
	public static Structure getBiologicalAssembly(String pdbId, int biolAssemblyNr)
			throws IOException, StructureException {
	 ...
	 * <li>as original with symmetry-expanded chains added with renamed chain ids
	 * and names (in the form originalAsymId_transformId and
	 * originalAuthId_transformId)</li> If only one biological assembly is required
	 * use {@link #getBiologicalAssembly(String)} or
	 * {@link #getBiologicalAssembly(String, int)} instead.
	 * 
	 * @param pdbId
	 * @param multiModel if true the output Structure will be a multi-model one with
	 *                   one transformId per model, if false the outputStructure
	 ...
	 * @since 5.0
	 */
	public static List<Structure> getBiologicalAssemblies(String pdbId, boolean multiModel)
			throws IOException, StructureException {
	...
	/**
	 * Returns all biological assemblies for the given PDB id, using
	 * multiModel={@value AtomCache#DEFAULT_BIOASSEMBLY_STYLE}
	 * <p>
	 * If only one biological assembly is required use
	 * {@link #getBiologicalAssembly(String)} or
	 * {@link #getBiologicalAssembly(String, int)} instead.
	 * 
	 * @param pdbId
	 * @return
	...
	public static enum StructureFiletype {
		PDB((new PDBFileReader()).getExtensions()), CIF(new MMCIFFileReader().getExtensions()),
