...
public class AFPPostProcessor {
	...
	/**
	 * in some special cases, there is no maginificent twists in the final chaining
	 * result; however, their rmsd (original and after optimizing) are very large.
	 * Therefore, a post-process is taken to split the blocks further at the
	 * ralative bad connections ( with relative high distance variation) to be
	 * tested: split or not according to optimized or initial chaining???
	 */
	...
	private static void splitBlock(FatCatParameters params, AFPChain afpChain, Atom[] ca1, Atom[] ca2) {
		...
		while (blockNum < maxTra + 1) {
			...
			if (debug) {
				logger.info(String.format("block %d original size %d rmsd %.3f maxt %.2f cut at %d%n", bk,
						blockSize[bk], maxs, maxt, cut));
			}
			...
			if (debug) {
				logger.info(String.format("  split into %d and %d sizes%n", blockSize[bk], blockSize[bk + 1]));
			}

			int[] afpChainList = afpChain.getAfpChainList();
			// int[] subrange1 = getSubrange(afpChainList, block2Afp[bk + 1] );
			blockRmsd[bk + 1] = AFPChainer.calAfpRmsd(blockSize[bk + 1], afpChainList, block2Afp[bk + 1], afpChain, ca1,
					ca2);
		...
		if (blockNum - blockNum0 > 0) {
			if (debug) {
				logger.info(String.format("Split %d times:%n", blockNum - blockNum0));
			}
			for (i = 0; i < blockNum; i++) {
				if (debug) {
					logger.info(String.format("  block %d size %d from %d rmsd %.3f%n", i, blockSize[i], block2Afp[i],
							blockRmsd[i]));
				}
			}
		}
	...
	/**
	 * remove the artifical small rigid-body superimpose in the middle clust the
	 * similar superimpositions (caused by the small flexible region, which is
	 * detected as a seperate rigid superimposing region by adding two twists before
	 * and after it(artifically!) one possible solution: allowing long enough loops
	 * in the chaining process, which however increase the calculation complexity
	 */
	private static void deleteBlock(FatCatParameters params, AFPChain afpChain, Atom[] ca1, Atom[] ca2) {
		...
		if (blockNumOld > blockNum) {
			if (debug) {
				logger.info(String.format("Delete %d small blocks%n", blockNumOld - blockNum));
			}
		}

		if (debug) {
			logger.error("deleteBlock: end blockNum:" + blockNum);
		}
		afpChain.setBlock2Afp(block2Afp);
		afpChain.setBlockSize(blockSize);
		afpChain.setAfpChainList(afpChainList);
	...
	private static void mergeBlock(FatCatParameters params, AFPChain afpChain, Atom[] ca1, Atom[] ca2) {
		...
		while (blockNum > 1) {
			...
			// maxrmsd = (blockRmsd[minb1] >
			// blockRmsd[minb2])?blockRmsd[minb1]:blockRmsd[minb2];
			if (minrmsd < badRmsd) {
				if (debug) {
					logger.info(String.format("merge block %d (rmsd %.3f) and %d (rmsd %.3f), total rmsd %.3f%n", minb1,
							blockRmsd[minb1], minb2, blockRmsd[minb2], minrmsd));
				}
			...
			} // not merge, modify the rmsdlist so that this combination is not considered in
				// next iteration
		}

		if (merge > 0) {
			if (debug) {
				logger.info(String.format("Merge %d blocks, remaining %d blocks%n", merge, blockNum));
			}
		}

		if (debug) {
			logger.error("AFPPostProcessor: mergeBlock end blocknum:" + blockNum);
		}
		afpChain.setBlock2Afp(block2Afp);
		afpChain.setBlockSize(blockSize);
		afpChain.setBlockNum(blockNum);
