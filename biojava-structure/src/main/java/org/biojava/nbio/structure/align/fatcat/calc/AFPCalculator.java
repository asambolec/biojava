...
public class AFPCalculator {
	...
	public static final void extractAFPChains(FatCatParameters params, AFPChain afpChain, Atom[] ca1, Atom[] ca2)
		...
		if (debug) {
			logger.error(new StringBuilder().append("nr of atoms ca1: ").append(ca1.length).append(" ca2: ")
					.append(ca2.length).toString());
		}
		...
		String msg = String.format("possible AFP-pairs %d, remain %d after filter 1 remove %d; filter 2 remove %d%n",
				n0, afpNum, n1, n2);
		logger.error(msg);
	...
	private static final boolean filterTerminal(Atom[] ca1, Atom[] ca2, int p1b, int p1e, int p2b, int p2e, int fragLen,
			int minLen) {
	...
	private static final double getRmsd(Atom[] ca1, Atom[] ca2, int fragLen, int p1, int p2, Matrix m, Atom t)
		...
		if (catmp1 == null) {
			logger.error(new StringBuilder().append("could not get fragment for ca1 ").append(p1).append(" ")
					.append(fragLen).toString());
			return rmsd;
		}

		if (catmp2 != null) {
			return SuperPositions.getRmsd(Calc.atomsToPoints(catmp1), Calc.atomsToPoints(catmp2));
		}
		logger.error(new StringBuilder().append("could not get fragment for ca2 ").append(p2).append(" ")
				.append(fragLen).toString());
		return rmsd;
	}
	 ...
	 * @param                clone: returns a copy of the atom (in case the
	 *                       coordinate get manipulated...)
	...
	private static final double scoreAfp(AFP afp, double badRmsd, double fragScore) {
		...
		double s;
		double w;
		// s = (rmsdCut - afptmp.rmsd) * afptmp.len; //the same scroing strategy as that
	...
	public static final void sortAfps(AFPChain afpChain, Atom[] ca1, Atom[] ca2) {
		...
		if (debug) {
			logger.error("entering sortAfps");
		}
		...
		afpChain.setAfpAftIndex(new int[pro1Len][pro2Len]); // the index of AFP (i,j*) nearest to (i,j), j*<j. if a AFP
															// exits for (i,j), it equals to afpIndex
		afpChain.setAfpBefIndex(new int[pro1Len][pro2Len]); // the index of AFP (i,j*) nearest to (i,j), j*>j. if a AFP
															// exits for (i,j), it equals to afpIndex
			...
			if (a == afpNum - 1 || afpSet.get(a).getP1() != afpSet.get(a + 1).getP1()) {
				...
				for (int b = b0; b <= a; b++) {
					...
					if (afpSet.get(b).getP1() != i) {
						logger.error(String.format("Warning: wrong afp index %d %d%n", i, afpSet.get(b).getP1()));
						return;
					}
				}
