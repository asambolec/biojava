...
/**
 * This module calculates the el Hassan-Calladine Base Pairing and Base-pair
 * Step Parameters for any nucleic acid containing structure that has the
 * information about the core base-pair rings. Citation:
 * https://www.ncbi.nlm.nih.gov/pubmed/11601858
 *
 * The method that is usually overridden is findPairs(), this base
 * implementation is used for a large-scale analysis of the most proper helical
 * regions in almost 4000 protein-DNA structures, almost 2000 structures
 * containing only DNA, or almost 1300 structures containing only RNA. (as of
 * 7/2017). Those who study tertiary structures for RNA folding should use the
 * TertiaryBasePairParameters, because this base class is only looking for base
 * pairs between separate strands that exactly line up. To relax the lining up
 * policy and allow for non-canonical base pairs, use the
 * MismatchedBasePairParameters class, which will not consider intra-strand base
 * pairing.
 *
 * @author Luke Czapla
 * @since 5.0.0
...
public class BasePairParameters implements Serializable {
	...
	// See URL
	// http://ndbserver.rutgers.edu/ndbmodule/archives/reports/tsukuba/Table1.html
	...
	public static final String[] STANDARD_BASES = new String[] {
			new StringBuilder().append("SEQRES   1 A    1  A\n")
					.append("ATOM      2  N9    A A   1      -1.291   4.498   0.000\n")
					.append("ATOM      3  C8    A A   1       0.024   4.897   0.000\n")
					.append("ATOM      4  N7    A A   1       0.877   3.902   0.000\n")
					.append("ATOM      5  C5    A A   1       0.071   2.771   0.000\n")
					.append("ATOM      6  C6    A A   1       0.369   1.398   0.000\n")
					.append("ATOM      8  N1    A A   1      -0.668   0.532   0.000\n")
					.append("ATOM      9  C2    A A   1      -1.912   1.023   0.000\n")
					.append("ATOM     10  N3    A A   1      -2.320   2.290   0.000\n")
					.append("ATOM     11  C4    A A   1      -1.267   3.124   0.000\n").append("END").toString(),
			new StringBuilder().append("SEQRES   1 A    1  G\n")
					.append("ATOM      2  N9    G A   1      -1.289   4.551   0.000\n")
					.append("ATOM      3  C8    G A   1       0.023   4.962   0.000\n")
					.append("ATOM      4  N7    G A   1       0.870   3.969   0.000\n")
					.append("ATOM      5  C5    G A   1       0.071   2.833   0.000\n")
					.append("ATOM      6  C6    G A   1       0.424   1.460   0.000\n")
					.append("ATOM      8  N1    G A   1      -0.700   0.641   0.000\n")
					.append("ATOM      9  C2    G A   1      -1.999   1.087   0.000\n")
					.append("ATOM     11  N3    G A   1      -2.342   2.364   0.001\n")
					.append("ATOM     12  C4    G A   1      -1.265   3.177   0.000\n").append("END").toString(),
			new StringBuilder().append("SEQRES   1 A    1  T\n")
					.append("ATOM      2  N1    T A   1      -1.284   4.500   0.000\n")
					.append("ATOM      3  C2    T A   1      -1.462   3.135   0.000\n")
					.append("ATOM      5  N3    T A   1      -0.298   2.407   0.000\n")
					.append("ATOM      6  C4    T A   1       0.994   2.897   0.000\n")
					.append("ATOM      8  C5    T A   1       1.106   4.338   0.000\n")
					.append("ATOM     10  C6    T A   1      -0.024   5.057   0.000\n").append("END").toString(),
			new StringBuilder().append("SEQRES   1 A    1  C\n")
					.append("ATOM      2  N1    C A   1      -1.285   4.542   0.000\n")
					.append("ATOM      3  C2    C A   1      -1.472   3.158   0.000\n")
					.append("ATOM      5  N3    C A   1      -0.391   2.344   0.000\n")
					.append("ATOM      6  C4    C A   1       0.837   2.868   0.000\n")
					.append("ATOM      8  C5    C A   1       1.056   4.275   0.000\n")
					.append("ATOM      9  C6    C A   1      -0.023   5.068   0.000\n").append("END").toString(),
			new StringBuilder().append("SEQRES   1 A    1  U\n")
					.append("ATOM      2  N1    U A   1      -1.284   4.500   0.000\n")
					.append("ATOM      3  C2    U A   1      -1.462   3.131   0.000\n")
					.append("ATOM      5  N3    U A   1      -0.302   2.397   0.000\n")
					.append("ATOM      6  C4    U A   1       0.989   2.884   0.000\n")
					.append("ATOM      8  C5    U A   1       1.089   4.311   0.000\n")
					.append("ATOM      9  C6    U A   1      -0.024   5.053   0.000\n").toString() };

	// this is also hard-coded data about standard WC base pairs for both DNA and
	// RNA
	...
	/**
	 * This constructor takes a Structure object, finds base pair and base-pair step
	 * parameters for double-helical regions within the structure.
	 * 
	 * @param structure  The already-loaded structure to analyze.
	 * @param useRNA     whether to look for canonical RNA pairs. By default (false)
	 *                   it analyzes DNA.
	 * @param removeDups whether to only look for base-pair parameters for each
	 *                   unique sequence in the structure (if set to <i>true</i>)
	...
	/**
	 * This constructor takes a Structure object, whether to use RNA, and whether to
	 * remove duplicate sequences.
	 * 
	 * @param structure  The already-loaded structure to analyze.
	 * @param useRNA     if true, the RNA standard bases will be used. Otherwise, if
	 *                   false, it will work on standard DNA bases.
	 * @param removeDups if true, duplicate sequences will not be considered. This
	 *                   is for the analysis of X-ray structures from RCSB, where
	 *                   there may be identical or similar units.
	 */
	...
	/**
	 * This constructor takes a Structure object, and whether to use the RNA
	 * standard bases.
	 * 
	 * @param structure The already-loaded structure to analyze.
	 * @param useRNA    if true, the RNA standard bases will be used. Otherwise, if
	...
	/**
	 * This constructor takes a Structure object, finds base pair and base-pair step
	 * parameters for double-helical regions within the structure for only canonical
	 * DNA pairs.
	 * 
	 * @param structure The already-loaded structure to analyze.
	 */
	...
	/**
	 * This method is the main function call to extract all step parameters, pairing
	 * parameters, and sequence information from the Structure object provided to
	 * the constructor.
	 * 
	 * @return This same object with the populated data, convenient for output (e.g.
	 *         <i>log.info(new BasePairParameters(structure).analyze());</i>)
	 */
	public BasePairParameters analyze() {
		...
		for (int i = 0; i < pairs.size(); i++) {
			...
			for (int j = 0; j < 6; j++) {
				pairingParameters[i][j] = pairParameters[j];
			}
			if (i != 0) {
				lastStep.invert();
	...
	/**
	 * This method returns the total number of base pairs that were found, used
	 * after the call to analyze().
	 * 
	...
	/**
	 * This method reports all the pair parameters, in the order of: buckle,
	 * propeller, opening (in degrees), shear, stagger, stretch (in Å).
	 * 
	 * @return A double[][] with length equal to number of base pairs for rows, and
	 *         6 columns
	 */
	...
	/**
	 * This method reports all the base-pair step parameters, in the order of: tilt,
	 * roll, twist (in degrees), shift, slide, rise (in Å).
	 * 
	 * @return A double[][] with length equal to number of base pairs (the first row
	 *         0 has no step and therefore is six zeroes), and 6 columns.
	 */
	...
	/**
	 * This method returns the primary strand's sequence where parameters were
	 * found. There are spaces in the string anywhere there was a break in the helix
	 * or when it goes from one helix to another helix in the structure. (the "step"
	 * is still returned)
	 * 
	...
	/**
	 * This method returns the names of the pairs in terms of A, G, T/U, and C for
	 * each base pair group in the list. The first character is the leading strand
	 * base and the second character is the complementary base
	 * 
	 * @return
	 */
	...
	/**
	 * This method is an internal test that the base pair specified is within a
	 * valid range. If not, it throws an exception with a message.
	 * 
	...
	/**
	 * This method returns the propeller ("propeller-twist") in degrees for the
	 * given base pair
	 * 
	 * @param bp the number of the base pair (starting with 0)
	 * @return the value as a double (in degrees)
	...
	/**
	 * This method returns the tilt for the given base pair, relative to the one
	 * before it.
	 * 
	 * @param bp the number of the base pair (starting with 0)
	 * @return the value as a double (in degrees)
	...
	/**
	 * This method returns the roll for the given base pair, relative to the one
	 * before it.
	 * 
	 * @param bp the number of the base pair (starting with 0)
	 * @return the value as a double (in degrees)
	...
	/**
	 * This method returns the twist for the given base pair, relative to the one
	 * before it.
	 * 
	 * @param bp the number of the base pair (starting with 0)
	 * @return the value as a double (in degrees)
	...
	/**
	 * This method returns the slide for the given base pair, relative to the one
	 * before it.
	 * 
	 * @param bp the number of the base pair (starting with 0)
	 * @return the value as a double (in Å)
	...
	/**
	 * This method returns the rise for the given base pair, relative to the one
	 * before it.
	 * 
	 * @param bp the number of the base pair (starting with 0)
	 * @return the value as a double (in Å)
	...
	/**
	 * This method reports all the nucleic acid chains and has an option to remove
	 * duplicates if you are considering an analysis of only unique DNA or RNA
	...
	public List<Chain> getNucleicChains(boolean removeDups) {
		...
		result.addAll(chains.stream().filter(Chain::isNucleicAcid).collect(Collectors.toList()));
		if (removeDups) {
	...
	/**
	 * This method performs a search for base pairs in the structure. The criteria
	 * is alignment of sequences and the canonical base pairs of DNA or RNA. Use
	 * MismatchedBasePairParameters or TertiaryBasePairParameters for finding
	 * higher-order associations.
	 * 
	 * @param chains The list of chains already found to be nucleic acids
	 * @return The list of corresponding Watson-Crick groups as pairs, as a Pair of
	 *         nucleic acid Groups
	 */
		...
		for (int i = 0; i < chains.size(); i++) {
			...
			for (int j = i + 1; j < chains.size(); j++) {
				...
				if (log.isDebugEnabled()) {
					log.debug(new StringBuilder().append(c.getAtomSequence()).append(" ")
							.append(chains.get(j).getAtomSequence()).append(" ").append(match).toString());
				}
				...
				for (int k = 0; k < match.length(); k++) {
					...
					if (Math.abs(distance - 10.0) < 4.0) {
						...
						if (valid) {
							...
							pairingNames.add((useRNA ? BASE_LIST_RNA[type1] + BASE_LIST_RNA[type2]
									: BASE_LIST_DNA[type1] + BASE_LIST_DNA[type2]));
							pairSequence += c.getAtomSequence().charAt(index1 + k);
						} else if (pairSequence.length() != 0
								&& pairSequence.charAt(pairSequence.length() - 1) != ' ') {
							pairSequence += ' ';
						}
	...
	/**
	 * This method calculates the central frame (4x4 transformation matrix) of a
	 * single base pair.
	 * 
	...
	public Matrix4d basePairReferenceFrame(Pair<Group> pair) {
		...
		// calculate pairing parameters: buckle, propeller, opening, shear, stretch,
		// stagger
		temp2.invert();
		temp2.mul(ref2);
	...
	// The following methods are just helper classes for the rapid analyze of
	// base-pair geometry.
	/**
	 * This method calculates pairing and step parameters from 4x4 transformation
	 * matrices (used internally) that comes out as a Matrix4d.
	 * 
	 * @param input the 4x4 matrix representing the transformation from strand II ->
	 *              strand I or pair i to pair i+1
	...
	/**
	 * Simple helper method for quickly checking the complement of a sequence, see
	 * also DNASequence nad RNASequence classes for more extensively useful
	 * functions not used in this narrow context of structural biology of base
	 * pairs. (Used internally)
	 */
	...
	/**
	 * This does a 3D Vector cross product of two vectors as double arrays. (used
	 * internally)
	 *
	...
	/**
	 * This method removes any component of vector a that is along vector b. (used
	 * internally)
	 * 
	...
	/**
	 * This method finds the longest common substring between two strings. (used
	 * internally)
	 * 
	...
	/**
	 * This returns true if a is the complement of b, false otherwise. (used
	 * internally)
	 * 
