...
/**
 * This is a port of the R survival code used for doing Cox Regression. The
 * algorithm was a fairly easy port from C code to Java where the challenge was
 * making the code a little more object friendly. In the R code everything is
 * passed around as an array and a large portion of the code is spent extracting
 * data from the array for use in different calculations. By organizing the data
 * in a class for each data point was able to simplify much of the code. Not all
 * variants of different methods that you can select for doing various
 * statistical calculations are implemented. Wouldn't be difficult to go back in
 * add them in if they are important.
 *
 * <p>
 * In R you can pass in different paramaters to override defaults which requires
 * parsing of the paramaters. In the Java code tried to be a little more exact
 * in the code related to paramaters where using strata, weighting, robust and
 * cluster are advance options. Additionaly code is implemented from Bob Gray to
 * do variance correction when using weighted paramaters in a data set.
 ...
 * <li>In robust linear_predictor set to 0 which implies score = 1 but previous
 * score value doesn't get reset
 * </ul>
 *
 * Cox regression fit, replacement for coxfit2 in order to be more frugal about
 * memory: specificly that we don't make copies of the input data.
 *
 *
 *
 *
...
public class CoxR {
	 ...
	 * @throws Exception
	 */
	public CoxInfo process(ArrayList<String> variables, ArrayList<SurvivalInfo> DataT, boolean useStrata,
			boolean useWeighted, boolean robust, boolean cluster) throws Exception {
		...
		return process(variables, DataT, maxiter2, CoxMethod.Efron, eps2, toler2, beta, doscale2, useStrata,
				useWeighted, robust, cluster);
	 ...
	 * @throws Exception
	 */
	public CoxInfo process(ArrayList<String> variables, ArrayList<SurvivalInfo> data, int maxiter, CoxMethod method,
			double eps, double toler, double[] beta, int doscale, boolean useStrata, boolean useWeighted,
			boolean robust, boolean cluster) throws Exception {
		...
		variables.stream().filter(variable -> variable.contains(":")).forEach(variable -> {
		...
		double[][] cmat; /* ragged arrays covar[][], */
		double[][] imat;
		double wtave;
		double[] a;
		double[] newbeta;
		...
		/*
		 ** Subtract the mean from each covar, as this makes the regression much more
		 * stable.
		 */
		...
		for (person = 0; person < nused; person++) {
			...
			if (cluster) {
				if (si.getClusterValue() == null && si.getClusterValue().isEmpty()) {
		...
		for (person = nused - 1; person >= 0;) {
			...
			if (ndead > 0) { /* we need to add to the main terms */
				if (method == CoxMethod.Breslow) { /* Breslow */
					...
					/*
					 ** If there are 3 deaths we have 3 terms: in the first the three deaths are all
					 * in, in the second they are 2/3 in the sums, and in the last 1/3 in the sum.
					 * Let k go from 0 to (ndead -1), then we will sequentially use denom -
					 * (k/ndead)*efronwt as the denominator a - (k/ndead)*a2 as the "a" term cmat -
					 * (k/ndead)*cmat2 as the "cmat" term and reprise the equations just above.
					 */
					for (k = 0; k < ndead; k++) {
						temp = (double) k / ndead;
						wtave = deadwt / ndead;
		...
		/*
		 * am I done? update the betas and test for convergence
		 */
		...
		/*
		 ** Never, never complain about convergence on the first step. That way, if
		 * someone HAS to they can force one iterations at a time.
		 */
		for (i = 0; i < nvar; i++) {
		...
		if (!gotofinish) {
			...
			for (iter = 1; iter <= maxiter; iter++) {
				...
				/*
				 ** The data is sorted from smallest time to largest Start at the largest time,
				 * accumulating the risk set 1 by 1
				 */
				for (person = nused - 1; person >= 0;) {
		...
		// finish:
		/*
		 * for (j = 0; j < numCovariates; j++) { b[j] = b[j] / SD[j]; ix = j *
		 * (numCovariates + 1) + j SE[j] = Math.sqrt(a[ix(j, j, numCovariates + 1)]) /
		 * SD[j]; // o = o + ("   " + variables.get(j) + "    " + Fmt(b[j]) + Fmt(SE[j])
		 * + Fmt(Math.exp(b[j])) + Fmt(Norm(Math.abs(b[j] / SE[j]))) + Fmt(Math.exp(b[j]
		 * - 1.95 * SE[j])) + Fmt(Math.exp(b[j] + 1.95 * SE[j])) + NL); CoxCoefficient
		 * coe = coxInfo.getCoefficient(variables.get(j)); coe.coeff = b[j];
		 * coe.stdError = SE[j]; coe.hazardRatio = Math.exp(b[j]); coe.pvalue =
		 * Norm(Math.abs(b[j] / SE[j])); coe.hazardRatioLoCI = Math.exp(b[j] - 1.95 *
		 * SE[j]); coe.hazardRatioHiCI = Math.exp(b[j] + 1.95 * SE[j]); }
		 * 
		 */
		...
		for (int n = 0; n < beta.length; n++) {
			...
			// System.out.println(beta[n] + "," + se[n] + "," + means[n] + "," + sd[n] + ","
			// + u[n]); //+ "," + imat[n] "," + loglik[n] + "," + sctest[n] + "," +
			// iterations[n] + "," + flag[n]
		...
		// System.out.println("?overall model p-value=" + chiSq(sctest[0],
		// beta.length));
	 ...
	 * @throws Exception
	 */
	public void coxphfitSCleanup(CoxInfo ci, boolean useWeighted, boolean robust, ArrayList<String> cluster)
			throws Exception {
		...
		for (SurvivalInfo si : ci.survivalInfoList) {
			...
			// System.out.println("lp score " + si.order + " " + si.time + " " +
			// si.getWeight() + " " + si.getClusterValue() + " " + lp + " " + Math.exp(lp));
		}
		// ci.dump();
	...
	public static void main(String[] args) {
		...
		//
		// WorkSheet worksheet =
		// WorkSheet.readCSV("/Users/Scooter/NetBeansProjects/AssayWorkbench/src/edu/scripps/assayworkbench/cox/uis-complete.txt",
		// '\t');
		...
		//
		// CoxR cox = new CoxR();
		// // cox.coxfit6(maxiter2, time2, status2, covar2, offset2, weights2, strata2,
		// method2, eps2, toler2, time2, doscale2);
		//
		//
		//
		//
	 ...
	 **
	 ** input **matrix, which contains the chol decomp of an n by n matrix in its
	 * lower triangle.
	 **
	 ** returned: the upper triangle + diagonal contain (FDF')^{-1} below the
	 * diagonal will be F inverse
	 **
	 ** Terry Therneau
	 */
	void chinv2(double[][] matrix, int n) {
		...
		/*
		 ** invert the cholesky in the lower triangle take full advantage of the
		 * cholesky's diagonal of 1's
		 */
		for (i = 0; i < n; i++) {
			if (matrix[i][i] > 0) {
		...
		/*
		 ** lower triangle now contains inverse of cholesky calculate F'DF (inverse of
		 * cholesky decomp process) to get inverse of original matrix
		 */
		for (i = 0; i < n; i++) {
	 ...
	 **
	 ** Solve the equation Ab = y, where the cholesky decomposition of A and y are
	 * the inputs.
	 **
	 ** Input **matrix, which contains the chol decomp of an n by n matrix in its
	 * lower triangle. y[n] contains the right hand side
	 **
