...
public class SurvFitKM {
	...
	private static final Logger logger = LoggerFactory.getLogger(SurvFitKM.class);
	 ...
	 * @throws Exception
	 */
	public SurvFitInfo process(LinkedHashMap<String, ArrayList<CensorStatus>> survivalData, boolean useWeights)
			throws Exception {
		...
		for (String strata : survivalData.keySet()) {
			...
			for (CensorStatus cs : csList) {
				...
				si.addUnknownDataTypeVariable("VALUE", Double.toString(cs.value));
	 ...
	 * @throws Exception
	 */
	public SurvFitInfo process(String datafile, String timeColumn, String statusColumn, String weightColumn,
			String variableColumn, boolean useWeights) throws Exception {
	...
	public SurvFitInfo process(String variable, ArrayList<SurvivalInfo> dataT, boolean useWeighted) throws Exception {
		return this.process(variable, dataT, Method.kaplanMeier, Error.greenwood, true, .95, ConfType.log,
				ConfLower.usual, null, null, useWeighted);
	}

	public LinkedHashMap<String, StrataInfo> processStrataInfo(String variable, ArrayList<SurvivalInfo> dataT,
			SurvFitKM.Method method, SurvFitKM.Error error, boolean seFit, double confInt, ConfType confType,
			ConfLower confLower, Double startTime, Double newTime, boolean useWeighted) throws Exception {
		...
		for (String strata : strataHashMap.keySet()) {
			...
			if (method == Method.kaplanMeier) {
				...
				for (int i = 0; i < strataInfo.getNrisk().size(); i++) {
					double t = (strataInfo.getNrisk().get(i) - strataInfo.getNevent().get(i))
			...
			if (seFit) {
				if (error == Error.greenwood) {
					for (int i = 0; i < strataInfo.getNrisk().size(); i++) {
						double t = (strataInfo.getNevent().get(i)) / (strataInfo.getNrisk().get(i)
								* (strataInfo.getNrisk().get(i) - strataInfo.getNevent().get(i)));
						if (i == 0) {
					...
					for (int i = 0; i < strataInfo.getNrisk().size(); i++) {
						double t = (strataInfo.getNevent().get(i))
								/ (strataInfo.getNrisk().get(i) * strataInfo.getNrisk().get(i));
						if (i == 0) {
							strataInfo.getVarhaz().add(t);
						} else {
							strataInfo.getVarhaz().add(t + strataInfo.getVarhaz().get(i - 1));
		...
		strataInfoHashMap.values().forEach(strataInfo -> {
		...
		if (confLower == ConfLower.usual) {
			strataInfoHashMap.values().forEach(strataInfo -> strataInfo.setStdlow(strataInfo.getStderr()));
		...
		if (confType == ConfType.plain) {
			strataInfoHashMap.values().forEach(strataInfo -> {
				for (int j = 0; j < strataInfo.getSurv().size(); j++) {
					double upper = strataInfo.getSurv().get(j)
							+ zvalue * strataInfo.getStderr().get(j) * strataInfo.getSurv().get(j);
					double lower = strataInfo.getSurv().get(j)
							- zvalue * strataInfo.getStdlow().get(j) * strataInfo.getSurv().get(j);
	 ...
	 * @throws Exception
	 */
	public SurvFitInfo process(String variable, ArrayList<SurvivalInfo> dataT, SurvFitKM.Method method,
			SurvFitKM.Error error, boolean seFit, double confInt, ConfType confType, ConfLower confLower,
			Double startTime, Double newTime, boolean useWeighted) throws Exception {
		SurvFitInfo si = new SurvFitInfo();

		LinkedHashMap<String, StrataInfo> strataInfoHashMap = this.processStrataInfo(variable, dataT, method, error,
				seFit, confInt, confType, confLower, startTime, newTime, useWeighted);
		si.setStrataInfoHashMap(strataInfoHashMap);
		LinkedHashMap<String, StrataInfo> unweightedStrataInfoHashMap = this.processStrataInfo(variable, dataT, method,
				error, seFit, confInt, confType, confLower, startTime, newTime, false);
