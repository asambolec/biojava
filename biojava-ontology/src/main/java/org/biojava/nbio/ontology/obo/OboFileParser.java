 ...
 *
 * This file contains parts of the OBO-Edit file OBOParseEngine, (particularly
 * the encoding and decoding part)
 *
 ...
 * Thanks to the OboEdit developers for giving permission to release this in
 * BioJava.
 *
 *
...
public class OboFileParser {
	...
	static {
		...
		escapeChars.put(Character.valueOf('W'), Character.valueOf(' '));
			...
			if (line.charAt(0) == '[') {
				if (line.charAt(line.length() - 1) != ']') {
					throw new IOException(
							new StringBuilder().append("Unclosed stanza: \"").append(line).append("\"").toString());
				}
				String stanzaname = line.substring(1, line.length() - 1);
				if (stanzaname.length() < 1) {
					throw new IOException(
							new StringBuilder().append("Empty stanza: \"").append(line).append("\"").toString());
				}
				...
				/*
				 * if (nv != null) logger.warn("nv = "+nv+", value = |"+value+"|");
				 */
				if (value.isEmpty()) {
	...
	/**
	 * parse the Synonym String from the Term. value can be:
	 * 
	 * <pre>
	...
	private Synonym parseSynonym(String key, String value) throws IOException {
		...
		if (startIndex == -1) {
			throw new IOException(
					new StringBuilder().append("Expected \"").append(line).append(" ").append(linenum).toString());
		}
		SOPair p = unescape(value, '"', startIndex + 1, value.length(), true);
		int defIndex = findUnescaped(value, '[', p.index, value.length());
		if (defIndex == -1) {
			throw new IOException(new StringBuilder().append("Badly formatted synonym. ")
					.append("No dbxref list found.").append(line).append(" ").append(linenum).toString());
		}
		...
		for (int i = 0; tokenizer.hasMoreTokens(); i++) {
			...
			if (i == 0) {
				if ("RELATED".equals(token)) {
					...
					throw new IOException(new StringBuilder().append("Found unexpected scope ").append("identifier ")
							.append(token).append(line).toString());
				}
			} else if (i == 1) {
				catID = token;
			} else {
				throw new IOException(new StringBuilder().append("Expected dbxref list,").append(" instead found ")
						.append(token).append(line).toString());
			}
		}
		...
		// logger.info("SYNONYM: " + p.str +" " + synonym.getCategory() + " " +
		// synonym.getScope());
	...
	protected Map<String, Object>[] getDbxrefList(String line, int startoffset, int endoffset) throws IOException {
		...
		while (!stop) {
			...
			Map<String, Object> pair = parseXref(line, startoffset, endIndex);
			if (pair == null) {
				startoffset++;
				continue;
			}
			...
			if (trailing) {
				...
				if (endIndex == -1) {
					throw new IOException(new StringBuilder().append("Badly formatted ").append("trailing properties ")
	...
	private void triggerNewStanza(String stanza) {
		listeners.forEach(li -> li.newStanza(stanza));
	}

	private void triggerNewKey(String key, String value) {
		listeners.forEach(li -> li.newKey(key, value));
	}

	private void triggerNewSynonym(Synonym synonym) {
		listeners.forEach(li -> li.newSynonym(synonym));
	}
	...
	public static String escape(String str, boolean escapespaces) {
		...
		for (int i = 0; i < str.length(); i++) {
			char c = str.charAt(i);
			Object o = unescapeChars.get(Character.valueOf(c));
			if (o == null) {
				out.append(c);
			} else {
	...
	public SOPair unescape(String str, char toChar, int startindex, int endindex, boolean mustFindChar)
			throws IOException {
		StringBuilder out = new StringBuilder();
		int endValue = -1;
		for (int i = startindex; i < endindex; i++) {
			...
			if (c == '\\') {
				i++;
				c = str.charAt(i);
				Character mapchar = escapeChars.get(Character.valueOf(c));
				if (mapchar == null) {
					throw new IOException(new StringBuilder().append("Unrecognized escape").append(" character ")
							.append(c).append(" found.").toString());
				}
				out.append(mapchar);
			} else if (c == toChar) {
				endValue = i;
		...
		if (endValue == -1 && mustFindChar) {
			throw new IOException(
					new StringBuilder().append("Expected ").append(toChar).append(".").append(str).toString());
		}
	...
	protected SOPair readQuotedString(String value, int startIndex, int stopIndex, char terminatingChar,
			boolean requireQuotes, boolean legalEndOfLine) throws IOException {
		...
		for (; i < stopIndex; i++) {
			...
			if (Character.isWhitespace(value.charAt(i))) {
				...
				if (requireQuotes) {
					throw new IOException(new StringBuilder().append("Expected start of quoted string. ").append(line)
							.append(" ").append(value).append(" at linenr ").append(linenum).toString());
				}
				useQuotes = false;
				break;
			} else {
		...
		while (startIndex < str.length()) {
			...
			for (; startIndex < str.length(); startIndex++) {
				if (Character.isWhitespace(str.charAt(startIndex))) {
					...
					throw new IOException(new StringBuilder().append("Expected comma in trailing modifier. ")
							.append(line).append(" linenr: ").append(linenum).toString());
				}
			}
		}
		return str.length();
...
class NestedValue {
	...
	private static final Logger logger = LoggerFactory.getLogger(NestedValue.class);
	...
	public String toString() {
		...
		for (Object key1 : keys) {
			...
			txt += new StringBuilder().append(" [").append(key).append(":").append(value).append("]").toString();
		}
