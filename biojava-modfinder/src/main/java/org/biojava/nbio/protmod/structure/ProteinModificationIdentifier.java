...
public class ProteinModificationIdentifier {
	...
	/**
	 *
	 * @param bondLengthTolerance tolerance of error (in Angstroms) of the covalent
	 *                            bond length, when calculating the atom distance
	 *                            threshold.
	 */
	...
	/**
	 *
	 * @param recordUnidentifiableModifiedCompounds true if choosing to record
	 *                                              unidentifiable atoms; false,
	 *                                              otherwise.
	...
	/**
	 *
	 * @return true if choosing to record unidentifiable atoms; false, otherwise.
	...
	/**
	 *
	 * @param recordAdditionalAttachments true if choosing to record additional
	 *                                    attachments that are not directly attached
	 *                                    to a modified residue.
	...
	/**
	 *
	 * @return true if choosing to record additional attachments that are not
	 *         directly attached to a modified residue.
	...
	/**
	 *
	 * @return a set of identified {@link ModifiedCompound}s from the last parse
	 *         result.
	...
	/**
	 *
	 * @return a set of atom linkages, which represent the atom bonds that were not
	 *         covered by the identified {@link ModifiedCompound}s from the last
	 *         parse result. Each element of the list is a array containing two
	 *         atoms.
	...
	/**
	 *
	 * @return a set of modified residues that were not covered by the identified
	 ...
	 */
	public void identify(final Structure structure, final Set<ProteinModification> potentialModifications) {
		if (structure == null) {
	...
	public void identify(final List<Chain> chains, final Set<ProteinModification> potentialModifications) {
		...
		if (recordAdditionalAttachments) {
			...
			modComps.forEach(mc -> identifyAdditionalAttachments(mc, ligands, chains));
		}
	...
	private void processMultiCrosslink(Map<Component, Set<Group>> mapCompGroups, List<ModifiedCompound> modComps,
	...
	private void processCrosslink1(Map<Component, Set<Group>> mapCompGroups, List<ModifiedCompound> modComps,
			ProteinModification mod, List<Component> components) {
		// modified residue
		...
		if (modifiedResidues != null) {
			modifiedResidues.stream().map(residue -> StructureUtil.getStructureGroup(residue, true))
					.forEach(strucGroup -> {
			...
			for (Group group1 : ligands) {
				for (int i = start; i < n; i++) {
					...
					if (!identifiedGroups.contains(group1)) {
						List<Atom[]> linkedAtoms = StructureUtil.findAtomLinkages(group1, group2, false,
								bondLengthTolerance);
						if (!linkedAtoms.isEmpty()) {
							linkedAtoms.forEach(atoms -> mc.addAtomLinkage(
	...
	private void recordUnidentifiableAtomLinkages(List<ModifiedCompound> modComps, List<Group> ligands) {
		...
		modComps.forEach(mc -> identifiedLinkages.addAll(mc.getAtomLinkages()));
		...
		for (int i = 0; i < nRes - 1; i++) {
			...
			for (int j = i + 1; j < nRes; j++) {
				...
				linkages.stream().map(atoms -> StructureUtil.getStructureAtomLinkage(atoms[0], true, atoms[1], true))
						.forEach(unidentifiableAtomLinkages::add);
			}
		}
		...
		for (int i = 0; i < nRes; i++) {
			...
			for (int j = 0; j < nLig; j++) {
				...
				linkages.stream().map(atoms -> StructureUtil.getStructureAtomLinkage(atoms[0], true, atoms[1], false))
						.forEach(unidentifiableAtomLinkages::add);
			}
		}
	}
	...
	private void recordUnidentifiableModifiedResidues(List<ModifiedCompound> modComps) {
		...
		modComps.forEach(mc -> identifiedComps.addAll(mc.getGroups(true)));

		// TODO: use the ModifiedAminoAcid after Andreas add that.
		residues.stream().filter(group -> group.getType() == GroupType.HETATM).forEach(group -> {
	 ...
	 */
	private void addModificationGroups(final Set<ProteinModification> modifications, final List<Group> residues,
			final List<Group> ligands, final Map<Component, Set<Group>> saveTo) {
		if (residues == null || ligands == null || modifications == null) {
			throw new IllegalArgumentException("Null argument(s).");
		}

		Map<Component, Set<Component>> mapSingleMultiComps = new HashMap<>();
		modifications.stream().map(ProteinModification::getCondition)
				.forEach(condition -> condition.getComponents().forEach(comp -> comp.getPdbccIds().forEach(pdbccId -> {
					Component single = Component.of(Collections.singleton(pdbccId), comp.isNTerminal(),
							comp.isCTerminal());
...

		{
			...
			residues.forEach(group -> {
				...
				unionComponentSet(residuesWildCard, comps).forEach(comp -> {
					Set<Group> gs = saveTo.get(comp);
					if (gs == null) {
						gs = new LinkedHashSet<>();
						saveTo.put(comp, gs);
	 ...
	 */
	private List<List<Atom[]>> getMatchedAtomsOfLinkages(ModificationCondition condition,
			Map<Component, Set<Group>> mapCompGroups) {
		List<ModificationLinkage> linkages = condition.getLinkages();
		int nLink = linkages.size();

		List<List<Atom[]>> matchedAtomsOfLinkages = new ArrayList<>(nLink);
		...
		for (int iLink = 0; iLink < nLink; iLink++) {
			...
			for (Group g1 : groups1) {
				for (Group g2 : groups2) {
					...
					boolean ignoreNCLinkage = potentialNamesOfAtomOnGroup1 == null
							&& potentialNamesOfAtomOnGroup2 == null && residues.contains(g1) && residues.contains(g2);

					Atom[] atoms = StructureUtil.findNearestAtomLinkage(g1, g2, potentialNamesOfAtomOnGroup1,
	 ...
	 */
	private void assembleLinkages(List<List<Atom[]>> matchedAtomsOfLinkages, ProteinModification mod,
		...
		while (indices[0] < matchedAtomsOfLinkages.get(0).size()) {
			...
			if (matchLinkages(modLinks, atomLinkages)) {
				...
				for (int i = 0; i < n; i++) {
					...
					StructureAtomLinkage link = StructureUtil.getStructureAtomLinkage(linkage[0],
							residues.contains(linkage[0].getGroup()), linkage[1],
							residues.contains(linkage[1].getGroup()));
					linkages.add(link);
				}
	...
	private boolean matchLinkages(List<ModificationLinkage> linkages, List<Atom[]> atomLinkages) {
		...
		for (int i = 0; i < nLink - 1; i++) {
			...
			for (int j = i + 1; j < nLink; j++) {
				...
				// check components
				if (((link1.getIndexOfComponent1() == link2.getIndexOfComponent1()) != (atoms1[0].getGroup()
						.equals(atoms2[0].getGroup())))
						|| ((link1.getIndexOfComponent1() == link2.getIndexOfComponent2()) != (atoms1[0].getGroup()
								.equals(atoms2[1].getGroup())))
						|| ((link1.getIndexOfComponent2() == link2.getIndexOfComponent1()) != (atoms1[1].getGroup()
								.equals(atoms2[0].getGroup())))
						|| ((link1.getIndexOfComponent2() == link2.getIndexOfComponent2()) != (atoms1[1].getGroup()
								.equals(atoms2[1].getGroup())))) {
					return false;
				}
				...
				if ((label11 != null && label21 != null && label11.equals(label21)) != (atoms1[0].equals(atoms2[0]))
						|| (label11 != null && label22 != null && label11.equals(label22)) != (atoms1[0]
								.equals(atoms2[1]))
						|| (label12 != null && label21 != null && label12.equals(label21)) != (atoms1[1]
								.equals(atoms2[0]))
						|| (label12 != null && label22 != null && label12.equals(label22)) != (atoms1[1]
								.equals(atoms2[1]))) {
					return false;
				}
			}
