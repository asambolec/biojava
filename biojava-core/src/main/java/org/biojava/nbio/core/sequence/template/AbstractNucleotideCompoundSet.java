...
public abstract class AbstractNucleotideCompoundSet<C extends NucleotideCompound> extends AbstractCompoundSet<C> {
	...
	/**
	 * Loops through all known nucleotides and attempts to find which are equivalent
	 * to each other. Also takes into account lower casing nucleotides as well as
	 * upper-cased ones.
	 */
	...
	protected void calculateIndirectAmbiguities() {
		...
		ambiguousCompounds.forEach(sourceCompound -> {
			Set<NucleotideCompound> compoundConstituents = sourceCompound.getConstituents();
			ambiguousCompounds.forEach(targetCompound -> {
		...
		equivalentsMap.keySet().forEach(key -> {
	...
	private void checkAdd(Map<NucleotideCompound, List<NucleotideCompound>> equivalentsMap, NucleotideCompound key,
	...
	/**
	 * Calculates the best symbol for a collection of compounds. For example if you
	 * gave this method a AC it will return a M which is the ambiguity symbol for
	 * these compounds.
	 *
	...
	public NucleotideCompound getAmbiguity(NucleotideCompound... compounds) {
		...
		for (NucleotideCompound compound : compounds) {
			compound.getConstituents().forEach(
					subCompound -> settedCompounds.add(getCompoundForString(subCompound.getBase().toUpperCase())));
		}
		return getAllCompounds().stream().filter(compound -> compound.getConstituents().equals(settedCompounds))
				.findFirst().map(NucleotideCompound::valueOf).orElse(null);
	}
