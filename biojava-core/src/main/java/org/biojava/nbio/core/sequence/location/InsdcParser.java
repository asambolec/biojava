...
/**
 * Parser for working with INSDC style locations. This class supports the full
 * range of location types generated by Genbank, INSDC and ENA.
 *
 * @author ayates
 * @author jgrzebyta
 * @author Paolo Pavan
...
public class InsdcParser<S extends AbstractSequence<C>, C extends Compound> {
	...
	// Pattern.compile("(?:[A-Z]([A-Za-z\\.0-9_]*?):)?(<?)(\\d+)(\\.{2}|\\^)?(>?)(\\d+)?(>?)?");

	// fixed issue #254
	protected static final Pattern singleLocationPattern = Pattern
			.compile("(?:([A-Za-z\\.0-9_]*?):)?(<?)(\\d+)(\\.{2}|\\^)?(>?)(\\d+)?(>?)?");

	/**
	 * Decodes a split pattern. Split patterns are a composition of multiple
	 * locationsString qualified by actions: join(location,location, ... location):
	 * The indicated elements should be joined (placed end-to-end) to form one
	 * contiguous sequence. order(location,location, ... location): The elements can
	 * be found in the specified order (5' to 3' direction), nothing is implied
	 * about their reasonableness bond(location,location...location): Found in
	 * protein files. These generally are used to describe disulfide bonds.
	 * complement(location,location...location): consider locations in their
	 * complement versus
	 *
	 * takes in input a comma splitted location string. The split must be done for
	 * outer level commas group(1) is the qualifier group(2) is the location string
	 * to getFeatures. In case of complex splits it will contain the nested
	 * expression
	 *
	...
	// Pattern.compile("^\\s?(join|order|bond|complement|)\\(?([^\\)]+)\\)?");
	protected static final Pattern genbankSplitPattern = Pattern
			.compile("^\\s?(join|order|bond|complement|)\\(?(.+)\\)?");

	/**
	 * designed to recursively split a location string in tokens. Valid tokens are
	 * those divided by coma that are not inside a bracket. I. e. split on the comma
	 * only if that comma has zero, or an even number of quotes ahead of it.
	 */
	...
	// private S referenceSequence = new
	// org.biojava.nbio.core.sequence.DNASequence();
	private AbstractSequence referenceSequence = new DNASequence();

	/**
	 * define the mode in which complex features should be appended in FLATTEN mode
	 * their will be appended to a single feature in HIERARCHICAL mode, the single
	 * mother feature will have a tree of features that will reflect the
	...
	public Location parse(String locationString) {
		...
		if (ll.size() == 1) {
			...
			l = new SimpleLocation(featureGlobalStart, featureGlobalEnd, Strand.UNDEFINED, ll);
		}
		return l;
	}
	 ...
	 *
	 * @param reader The source of the data; assumes that end of the reader stream
	 *               is the end of the location string to parse
	 * @return The parsed location
	 * @throws IOException     Thrown with any reader error
	 * @throws ParserException Thrown with any error with parsing locations
	 */
	public List<AbstractLocation> parse(Reader reader) throws IOException {
	...
	private List<Location> parseLocationString(String string, int versus) {
		...
		for (String t : tokens) {
			...
			if (!m.find()) {
				throw new ParserException(new StringBuilder().append("Cannot interpret split pattern ").append(t)
						.append("\nin location string:").append(string).toString());
			}
			...
			if (!splitQualifier.isEmpty()) {
				...
				if (!m.find()) {
					throw new ParserException(new StringBuilder().append("Cannot interpret location pattern ")
							.append(splitString).append("\nin location string:").append(string).toString());
				}
