...
public class SingleLinkageClusterer {
	 ...
	 * {@link #getDendrogram()} to get the full tree or {@link #getClusters(double)}
	 * to get the clusters at a certain cutoff in the tree Please note that the
	 * matrix will be altered during the clustering procedure. A copy must be made
	 * before by the user if needing to use the original matrix further.
	 * 
	 * @param matrix        the distance matrix with distance values in j>i half,
	 *                      all other values will be ignored
	 * @param isScoreMatrix if false the matrix will be considered a distance
	 *                      matrix: lower values (distances) mean closer objects, if
	 *                      true the matrix will be considered a score matrix:
	 *                      larger values (scores) mean closer objects
	...
	/**
	 * Calculate the hierarchical clustering and store it in dendrogram array This
	 * is the naive implementation (o(n3)) of single linkage clustering as outlined
	 * in wikipedia: http://en.wikipedia.org/wiki/Single-linkage_clustering
	 */
	private void clusterIt() {
		...
		for (int m = 0; m < numItems - 1; m++) {
			...
			// if (debug) {
			// System.out.println("Matrix after iteration "+m+" (merged
	...
	/**
	 * Merge 2 rows/columns of the matrix by the linkage function (see
	 * {@link #link(double, double)}
	 * 
	 * @param closestPair
	 */
	private void merge(LinkedPair closestPair) {
		...
		for (int other = 0; other < numItems; other++) {
			matrix[Math.min(first, other)][Math.max(first, other)] = link(getDistance(first, other),
	...
	/**
	 * The linkage function: minimum of the 2 distances (i.e. single linkage
	 * clustering)
	 * 
	 * @param d1
	 * @param d2
	...
	public Map<Integer, Set<Integer>> getClusters(double cutoff) {
		...
		for (int i = 0; i < numItems - 1; i++) {
			...
			if (isWithinCutoff(i, cutoff)) {
				...
				if (firstClusterId == -1 && secondClusterId == -1) {
					// neither member is in a cluster yet, let's assign a new cluster and put them
					// both in
					...
					// we need to join them: necessarily one must be of size 1 and the other of
					// size>=1
					...
					if (firstCluster.size() < secondCluster.size()) {
						logger.debug(new StringBuilder().append("Joining cluster ").append(firstClusterId)
								.append(" to cluster ").append(secondClusterId).toString());
						// we join first onto second
						firstCluster.stream().mapToInt(Integer::valueOf).forEach(secondCluster::add);
						clusters.remove(firstClusterId);
					} else {
						logger.debug(new StringBuilder().append("Joining cluster ").append(secondClusterId)
								.append(" to cluster ").append(firstClusterId).toString());
						// we join second onto first
						secondCluster.stream().mapToInt(Integer::valueOf).forEach(firstCluster::add);
		...
		// reassigning cluster numbers by creating a new map (there can be gaps in the
		// numbering if cluster-joining happened)
		...
		// anything not clustered is assigned to a singleton cluster (cluster with one
		// member)
		for (int i = 0; i < numItems; i++) {
	...
	private String clustersToString(Map<Integer, Set<Integer>> finalClusters) {
		...
		finalClusters.keySet().stream().mapToInt(Integer::valueOf).forEach(cId -> {
			sb.append(cId).append(": ");
			finalClusters.get(cId).stream().mapToInt(Integer::valueOf).forEach(member -> sb.append(member).append(" "));
			sb.append("\n");
		});
		return sb.toString();
	}
			...
			for (int j = 0; j < numItems; j++) {
				if (i == j) {
					...
					if (matrix[j][i] == Double.MAX_VALUE) {
						...
						sb.append(String.format("%6.2f ", matrix[j][i]));
					}
				}
			}
			sb.append("\n");
	...
	private class LinkedPair {
		...
		public String toString() {
			...
			return new StringBuilder().append("[").append(first).append(",").append(second).append("-")
					.append(closestDistStr).append("]").toString();
		}
