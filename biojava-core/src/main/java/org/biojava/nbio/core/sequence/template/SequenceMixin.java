 ...
 *
 * It also provides a place to put utility methods whose application can be to a
 * single class of Sequence e.g. {@link NucleotideCompound} {@link Sequence}; or
 * to any Sequence e.g. looking for the {@link #getComposition(Sequence)} or
 * {@link #getDistribution(Sequence)} for any type of Sequence.
 *
...
public class SequenceMixin {
	...
	/**
	 * For the given vargs of compounds this method counts the number of times those
	 * compounds appear in the given sequence
	 *
	...
	/**
	 * Analogous to {@link #getComposition(Sequence)} but returns the distribution
	 * of that {@link Compound} over the given sequence.
	 *
	 * @param          <C> The type of compound to look for
	 * @param sequence The type of sequence to look over
	 * @return Returns the decimal fraction of the compounds in the given sequence.
	 *         Any compound not in the Map will return a fraction of 0.
	 */
	...
	/**
	 * Does a linear scan over the given Sequence and records the number of times
	 * each base appears. The returned map will return 0 if a compound is asked for
	 * and the Map has no record of it.
	 *
	 * @param          <C> The type of compound to look for
	 * @param sequence The type of sequence to look over
	...
	/**
	 * For the given Sequence this will return a {@link StringBuilder} object filled
	 * with the results of {@link Compound#toString()}. Does not used
	 * {@link #write(java.lang.Appendable, org.biojava.nbio.core.sequence.template.Sequence) }
	...
	/**
	 * Shortcut to
	 * {@link #toStringBuilder(org.biojava.nbio.core.sequence.template.Sequence)}
	...
	/**
	 * Performs a linear search of the given Sequence for the given compound. Once
	 * we find the compound we return the position.
	 */
	 ...
	 * {@link ReversedSequenceView} and passing it into
	 * {@link #indexOf(Sequence, Compound)}. We then inverse the index coming out of
	 * it.
	 */
	...
	/**
	 * Creates a simple sequence iterator which moves through a sequence going from
	 * 1 to the length of the Sequence. Modification of the Sequence is not allowed.
	 */
	 ...
	 * {@link Collections#shuffle(List)} and then returning the shuffled elements in
	 * a new instance of {@link SequenceBackingStore} which behaves as a
	 * {@link Sequence}.
	 */
	...
	/**
	 * Produces kmers of the specified size e.g. ATGTGA returns two views which have
	 * ATG TGA
	 *
	...
	/**
	 * Used to generate overlapping k-mers such i.e. ATGTA will give rise to ATG,
	 * TGT & GTA
	 *
	 * @param          <C> Compound to use
	 * @param sequence Sequence to build from
	...
	/**
	 * A case-insensitive manner of comparing two sequence objects together. We will
	 * throw out any compounds which fail to match on their sequence length &
	 * compound sets used. The code will also bail out the moment we find something
	 * is wrong with a Sequence. Cost to run is linear to the length of the
	 * Sequence.
	 *
	 * @param        <C> The type of compound
	...
	/**
	 * A case-sensitive manner of comparing two sequence objects together. We will
	 * throw out any compounds which fail to match on their sequence length &
	 * compound sets used. The code will also bail out the moment we find something
	 * is wrong with a Sequence. Cost to run is linear to the length of the
	 * Sequence.
	...
	private static <C extends Compound> boolean baseSequenceEquality(Sequence<C> source, Sequence<C> target,
			boolean ignoreCase) {
		boolean equal = true;
	 ...
	 * biological index. This assumes your sequence supports random access and
	 * performs well when doing these operations.
	 *
	 * @author ayates
	 *
