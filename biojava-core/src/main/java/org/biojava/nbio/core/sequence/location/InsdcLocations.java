...
/**
 * A collection of locations which are used whenever we work with INSDC; some of
 * which could be deprecated (from INSDC's point of view) yet appear in records.
 *
 * @author ayates
 * @author Paolo Pavan
 */
public class InsdcLocations {
	...
	/**
	 * Used to represent bond locations equivalent to bond(7,8) or bond(7). Bond
	 * locations are single position complex locations
	 */
	public static class BondLocation extends AbstractLocation {
		...
		protected final void assertLocation() {
			for (Location l : this) {
				...
				Point end = l.getEnd();
				if (!start.equals(end)) {
					throw new IllegalStateException(new StringBuilder().append("The start ").append(start)
							.append(" is not equal to the end ").append(end)
							.append(". bond locations must be a single ").append("compound long").toString());
				}
			}
		}
	}
	...
	/**
	 * Deprecated in INSDC yet still appears; equivalent to the order() directive
	 * except no 5' to 3' ordering is defined. The constructor reflects this
	 * relationship and only allows the creation of complex locations
	 */
	...
	/**
	 * Deprecated in INSDC; refers to a set of locations of which one location could
	 * be valid e.g. one-of(location, location, location). Originally used to
	 * describe split locations in alternative splicing or variations. Now these are
	 * dealt with in their own feature fields.
	 *
	 * The default location is chosen to be the first however if you think you need
	 * to work with this location you should use the sub-locations.
	 */
