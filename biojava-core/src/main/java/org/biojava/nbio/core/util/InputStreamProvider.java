...
/**
 * A class that provides an InputStream from a File. The file can be compressed
 * or uncompressed.
 *
 * Currently supported compressions:
 * <ul>
 * <li>Gzip (extension .gz)</li>
 * <li>Zip (extension .zip) in this case a stream to the first entry in the zip
 * file is returned</li>
 * <li>Jar (extension .jar) same as .Zip; only stream to first entry is returned
 * </li>
...
public class InputStreamProvider {
	...
	// LoggerFactory.getLogger(InputStreamProvider.class);
	...
	/**
	 * Get an InputStream for given file path. The caller is responsible for closing
	 * the stream or otherwise a resource leak can occur.
	 * 
	...
	/**
	 * open the file and read the magic number from the beginning this is used to
	...
	/**
	 * Get an InputStream for the file. The caller is responsible for closing the
	 * stream or otherwise a resource leak can occur.
	 * 
	 * @param f a File
	...
	/**
	 * Wrapper for new FileInputStream. if System.property biojava.cache.files is
	 * set, will try to load files from memory cache.
	 *
	 * @param f
	 * @return
