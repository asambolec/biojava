...
/**
 * Static utility to access substitution matrices that come bundled with
 * BioJava. All matrices were downloaded from
...
public class SubstitutionMatrixHelper implements Serializable {
	...
	/**
	 * Returns Nuc 4.2 matrix by Lowe Only the first nucleotide sequence to align
	 * can contain ambiguous nucleotides
	 * 
	...
	/**
	 * Returns Nuc 4.4 matrix by Lowe Both of the nucleotide sequences to align can
	 * contain ambiguous nucleotides
	 * 
	 ...
	 * by the name {@code name}. Searches first in the default AAINDEX file
	 * (see @link {@link #getMatrixFromAAINDEX(String)}), then in the classpath. If
	 * the required matrix does not exist, null is returned. Example names:
	 * <ul>
	 * <li>blosum62</li>
	 * <li>JOND920103</li>
	 * <li>pam250</li>
	...
	private static SubstitutionMatrix<AminoAcidCompound> getAminoAcidMatrix(String file) {
		aminoAcidMatrices.putIfAbsent(file,
				new SimpleSubstitutionMatrix<>(AminoAcidCompoundSet.getAminoAcidCompoundSet(), getReader(file), file));
	...
	private static SubstitutionMatrix<NucleotideCompound> getNucleotideMatrix(String file) {
		nucleotideMatrices.putIfAbsent(file,
				new SimpleSubstitutionMatrix<>(AmbiguityDNACompoundSet.getDNACompoundSet(), getReader(file), file));
	...
	private static InputStreamReader getReader(String file) {
		...
		return new InputStreamReader(SubstitutionMatrixHelper.class
				.getResourceAsStream(String.format("/%s.txt", resourcePathPrefix + file)));
	}
