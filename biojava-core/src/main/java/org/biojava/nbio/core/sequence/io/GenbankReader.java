...
/**
 * Use GenbankReaderHelper as an example of how to use this class where
 * GenbankReaderHelper should be the primary class used to read Genbank files
 *
 */
public class GenbankReader<S extends AbstractSequence<C>, C extends Compound> {
	 ...
	 */
	public GenbankReader(final File file, final SequenceHeaderParserInterface<S, C> headerParser,
			final SequenceCreatorInterface<C> sequenceCreator) throws FileNotFoundException {
	 ...
	 * This method tries to process all the available Genbank records in the File or
	 ...
	 * 
	 * @see #process(int)
	 * @return {@link HashMap} containing all the parsed Genbank records present,
	 ...
	 * @return {@link HashMap} containing maximum <code>max</code> parsed Genbank
	...
	public LinkedHashMap<String, S> process(final int max) throws IOException, CompoundNotFoundException {
		...
		while (true) {
			...
			genbankParser.getFeatures().keySet().forEach(k -> {
				for (AbstractFeature f : genbankParser.getFeatures(k)) {
					// f.getLocations().setSequence(sequence); // can't set proper sequence source
					// to features. It is actually needed? Don't think so...
			...
			if (dbQualifier != null) {
				...
				sequence.setTaxonomy(new TaxonomyID(
						new StringBuilder().append(q.getDatabase()).append(":").append(q.getId()).toString(),
						DataSource.GENBANK));
			}
	...
	public static void main(String[] args) throws Exception {
		...
		GenbankReader<ProteinSequence, AminoAcidCompound> proteinReader = new GenbankReader<>(is,
				new GenericGenbankHeaderParser<>(),
				new ProteinSequenceCreator(AminoAcidCompoundSet.getAminoAcidCompoundSet()));
		LinkedHashMap<String, ProteinSequence> proteinSequences = proteinReader.process();
		logger.info(String.valueOf(proteinSequences));

		String inputFile = "src/test/resources/NM_000266.gb";
		is = new FileInputStream(inputFile);
		GenbankReader<DNASequence, NucleotideCompound> dnaReader = new GenbankReader<>(is,
				new GenericGenbankHeaderParser<>(), new DNASequenceCreator(DNACompoundSet.getDNACompoundSet()));
		LinkedHashMap<String, DNASequence> dnaSequences = dnaReader.process();
		logger.info(String.valueOf(dnaSequences));

		String crazyFile = "src/test/resources/CraftedFeature.gb";
		is = new FileInputStream(crazyFile);
		GenbankReader<DNASequence, NucleotideCompound> crazyReader = new GenbankReader<>(is,
				new GenericGenbankHeaderParser<>(), new DNASequenceCreator(DNACompoundSet.getDNACompoundSet()));
		LinkedHashMap<String, DNASequence> crazyAnnotatedSequences = crazyReader.process();

		is.close();
		logger.info(String.valueOf(crazyAnnotatedSequences));
	}
