...
public class GenericInsdcHeaderFormat<S extends AbstractSequence<C>, C extends Compound> {
	...
	private String _write_feature_qualifier(String key, String value, boolean quote) {
		String line = "";
		if (null == value) {
			line = new StringBuilder().append(QUALIFIER_INDENT_STR).append("/").append(key).append(lineSep).toString();
			return line;
		}
		if (quote) { // quote should be true for numerics
			line = new StringBuilder().append(QUALIFIER_INDENT_STR).append("/").append(key).append("=\"").append(value)
					.append("\"").toString();
		} else {
			line = new StringBuilder().append(QUALIFIER_INDENT_STR).append("/").append(key).append("=").append(value)
					.toString();
	...
	private String _wrap_location(String location) {
		...
		return new StringBuilder().append(location.substring(0, index + 1)).append(lineSep).append(QUALIFIER_INDENT_STR)
				.append(_wrap_location(location.substring(index + 1))).toString();
	}
	...
	protected String _write_feature(FeatureInterface<AbstractSequence<C>, C> feature, int record_length) {
		...
		String line = new StringBuilder().append(formatter.toString().substring(0, QUALIFIER_INDENT))
				.append(_wrap_location(location)).append(lineSep).toString();
		formatter.close();
		...
		return line;
		/*
		 * self.handle.write(line) #Now the qualifiers... for key, values in
		 * feature.qualifiers.items(): if isinstance(values, list) or isinstance(values,
		 * tuple): for value in values: self._write_feature_qualifier(key, value) elif
		 * values: #String, int, etc self._write_feature_qualifier(key, values) else:
		 * #e.g. a /psuedo entry self._write_feature_qualifier(key)
		 */
	}
	 ...
	 * "join(complement(20,100),complement(1,10))" instead (but appears to have now
	 * adopted the GenBank convention). Notice that the order of the entries is
	 * reversed! This function therefore uses the first form. In this situation we
	 * expect the parent feature and the two children to all be marked as strand ==
	 * -1, and in the order 0:10 then 19:100.
	 * 
	 ...
	 * 
	 * For mixed this mixed strand feature, the parent SeqFeature should have no
	 * strand (either 0 or None) while the child features should have either strand
	 * +1 or -1 as appropriate, and be listed in the order given here.
	 * 
	 * @param feature
	 * @param record_length
	 */
	private String _insdc_feature_location_string(FeatureInterface<AbstractSequence<C>, C> feature, int record_length) {
		if (feature.getChildrenFeatures().isEmpty()) {
			// Non-recursive.
			String location = _insdc_location_string_ignoring_strand_and_subfeatures(feature.getLocations(),
					record_length);
		...
		if (feature.getLocations().getStrand() == Strand.NEGATIVE) {
			for (FeatureInterface<?, ?> f : feature.getChildrenFeatures()) {
				if (f.getLocations().getStrand() != Strand.NEGATIVE) {
					...
					formatter.format("Inconsistent strands: %s for parent, %s for child",
							feature.getLocations().getStrand(), f.getLocations().getStrand());
					String output = formatter.toString();
					formatter.close();
			...
			feature.getChildrenFeatures().forEach(f -> locations
					.add(_insdc_location_string_ignoring_strand_and_subfeatures(f.getLocations(), record_length)));
	...
	private String _insdc_location_string_ignoring_strand_and_subfeatures(
			...
			AbstractLocation sequenceLocation, int record_length) {
		/*
		 * if location.ref: ref = "%s:" % location.ref else: ref = "" assert not
		 * location.ref_db
		 */
		String ref = "";
		if (!sequenceLocation.getStart().isUncertain() && !sequenceLocation.getEnd().isUncertain()
				&& sequenceLocation.getStart() == sequenceLocation.getEnd()) {
			...
			if (sequenceLocation.getEnd().getPosition() == record_length) {
				...
				StringBuilder sb = new StringBuilder();
				Formatter formatter = new Formatter(sb, Locale.US);
				formatter.format("%s%d^%d", ref, sequenceLocation.getStart().getPosition(),
						sequenceLocation.getEnd().getPosition());
				String output = formatter.toString();
				formatter.close();
				return output;
			}
		...
		if (!sequenceLocation.getStart().isUncertain() && !sequenceLocation.getEnd().isUncertain()
				&& sequenceLocation.getStart().getPosition() + 1 == sequenceLocation.getEnd().getPosition()) {
			...
			if (sequenceLocation.getStart().isUnknown() && sequenceLocation.getEnd().isUnknown()) {
				...
				StringBuilder sb = new StringBuilder();
				Formatter formatter = new Formatter(sb, Locale.US);
				formatter.format("%s<%d..%s", ref, sequenceLocation.getEnd().getPosition(),
						_insdc_feature_position_string(sequenceLocation.getEnd()));
				String output = formatter.toString();
				formatter.close();
				return output;
			} else {
				...
				StringBuilder sb = new StringBuilder();
				Formatter formatter = new Formatter(sb, Locale.US);
				formatter.format("%s%s..>%d", ref, _insdc_feature_position_string(sequenceLocation.getStart()),
						sequenceLocation.getStart().getPosition());
				String output = formatter.toString();
				formatter.close();
				return output;
			}
			...
			return new StringBuilder().append(ref)
					.append(_insdc_feature_position_string(sequenceLocation.getStart(), 0)).append("..")
					.append(_insdc_feature_position_string(sequenceLocation.getEnd())).toString();
		}
	}
	...
	private String _insdc_feature_position_string(Point location, int increment) {
		...
		/*
		 * if isinstance(pos, SeqFeature.ExactPosition): return "%i" %
		 * (pos.position+offset) elif isinstance(pos, SeqFeature.WithinPosition): return
		 * "(%i.%i)" % (pos.position + offset, pos.position + pos.extension + offset)
		 * elif isinstance(pos, SeqFeature.BetweenPosition): return "(%i^%i)" %
		 * (pos.position + offset, pos.position + pos.extension + offset) elif
		 * isinstance(pos, SeqFeature.BeforePosition): return "<%i" % (pos.position +
		 * offset) elif isinstance(pos, SeqFeature.AfterPosition): return ">%i" %
		 * (pos.position + offset) elif isinstance(pos, SeqFeature.OneOfPosition):
		 * return "one-of(%s)" \ % ",".join([_insdc_feature_position_string(p,offset) \
		 * for p in pos.position_choices]) elif isinstance(pos,
		 * SeqFeature.AbstractPosition): raise
		 * NotImplementedError("Please report this as a bug in Biopython.") else: raise
	 ...
	 *
	 * Any single words which are too long get returned as a whole line (e.g. URLs)
