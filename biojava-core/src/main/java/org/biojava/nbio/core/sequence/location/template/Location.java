 ...
 * location can be a single set of bounds or composed of multiple sub-locations.
 * Each sub-location is a Location and therefore subject to the same rules.
 *
 * @author ayates
 */
public interface Location extends Iterable<Location>, Accessioned {
	...
	/**
	 * Gives access to the sub locations for this location. However this does not
	 * return sub-locations of sub-locations. For that functionality use
	...
	/**
	 * An extension to {@link #getSubLocations()} which returns sub-locations of
	 * sub-locations; this will continue until it runs out of those locations.
	 *
	...
	/**
	 * Returns true if the location is considered to be complex meaning the location
	 * is actually composed of sub-locations.
	 */
	boolean isComplex();
	...
	/**
	 * Returns true if the position is meant to represent a point between two points
	 * such as 78^79. Only valid if start and stop are next to each other.
	 */
	boolean isBetweenCompounds();

	/**
	 * Will return a SequenceReader object which represents the outer bounds of this
	 * Location
	 *
	 ...
	 * locations i.e. those locations which are not complex and define the true
		...
		public static Location circularLocation(int start, int end, Strand strand, int length) {
			...
			if (min > length) {
				throw new IllegalArgumentException(new StringBuilder().append("Cannot process a ")
						.append("location whose lowest coordinate is less than ").append("the given length ")
